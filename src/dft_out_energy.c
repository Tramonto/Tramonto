...
double
calc_free_energy (FILE * fp, double *x, double fac_area,
		  double fac_vol, int print_flag)
{
  int icomp, ilist, loc_inode, inode, loc_i, loc_i_psi, i, inode_box;
  ...
  double rho_i, psi_i = 0.0, omega_i, omega_i_b, area, stress_tensor,
    ...
    grand_free_energy[2] = { 0.0, 0.0 }, surface_free_energy[2] =
  {
  0.0, 0.0}, psi_rho_sum[2],
    vext_c_sum[2], hs_energy, ideal_sum[2], lj_sum[2], deltac_sum[2],
    vext_sum[2], ideal, ljterm, deltacterm, vext, ideal_b, lj_b, deltac_b,
  ...
  if (Ipot_ff_n != IDEAL_GAS)
    {
      assemble_HS_free_energy (x, &sum_phispt, &sum_phispt_b,
			       &sum_phispt_b_old);
      if (Proc == 0 && print_flag && Iwrite != NO_SCREEN)
	{
	  printf
	    ("\t===== Hard Sphere Integrals ... integrating over domain volume =======\n");
	  printf ("\t Phispt: %g \n", sum_phispt);
	  printf ("\t Phispt_bulk: %g \n", sum_phispt_b);
	  printf
	    ("\t===== Bulk Hard Sphere Integral ... integrating over fluid volume =======\n");
      ...
      for (icomp = 0; icomp < Ncomp; icomp++)
	{
	  for (i = 0; i < Imax; i++)
	      ...
	      if (Ipot_ff_c == 1)
		{
	      ...
	      if (rho_i > Rho_b[icomp] * exp (-VEXT_MAX)
		  && Vext[loc_inode][icomp] < VEXT_MAX)
		{
		  ...
		  if (Ipot_ff_n == LJ12_6)
		    {		/* LJ fluid case */
		      ljterm =
			0.5 * rho_i * int_stencil (x, inode_box, icomp,
						   U_ATTRACT);
		      omega_i += ljterm;
		    }
		  else
	      ...
	      else
		{
	      ...
	      ideal_b =
		Rho_b[icomp] * (log (Rho_b[icomp]) - 1.0 - Betamu[icomp]);
	      omega_i_b = ideal_b;

	      if (Ipot_ff_n == LJ12_6)
		{		/* LJ fluid case */
		  lj_b = 0.5 * Rho_b[icomp] * Betamu_att[icomp];
		  omega_i_b += lj_b;
		}
	      else
		lj_b = 0.0;
		...
		{
		  deltac_b = -0.5 * Rho_b[icomp] * Deltac_b[icomp];
		  omega_i_b += deltac_b;
		}
	      else
		deltac_b = 0.0;
		...
		* Vol_el / ((double) Nnodes_per_el_V);
	      ideal_sum[i] +=
		(ideal * Nel_hit2[i][loc_i] -
		 ideal_b * Nel_hit[i][loc_i]) * Vol_el /
		((double) Nnodes_per_el_V);
	      lj_sum[i] +=
		(ljterm * Nel_hit2[i][loc_i] -
		 lj_b * Nel_hit[i][loc_i]) * Vol_el /
		((double) Nnodes_per_el_V);
	      vext_sum[i] +=
		vext * Nel_hit2[i][loc_i] * Vol_el /
		((double) Nnodes_per_el_V);
	      if (Type_coul > -1)
		{
		  ...
		  vext_c_sum[i] += vext_c * Nel_hit2[i][loc_i]
		    * Vol_el / ((double) Nnodes_per_el_V);
		  deltac_sum[i] +=
		    (deltacterm * Nel_hit2[i][loc_i] -
		     deltac_b * Nel_hit[i][loc_i]) * Vol_el /
		    ((double) Nnodes_per_el_V);
		}
  ...
  for (i = 0; i < Imax; i++)
	...
	AZ_gsum_double (omega_s_sum[i], Aztec.proc_config);
  ...
  if (fp != NULL && Proc == 0 && print_flag)
	...
	{
	  printf
	    ("\t===== Totals when Integrating over domain volume =====\n");
	...
	{
	  printf
	    ("\t====Contributions of each term to surface free energy =========\n");
	  printf ("\t\t Ideal Gas and Chem.pot term: %9.6f\n", ideal_sum[i]);
	  if (Type_func >= 0)
	    printf ("\t\t Hard Sphere term: %9.6f\n", hs_energy);
	  if (Type_attr >= 0)
	    printf ("\t\t Lennard-Jones term: %9.6f\n", lj_sum[i]);
      ...
      if (Iwrite != NO_SCREEN)
	{
	  ...
	  if (Ipot_ff_n != IDEAL_GAS && Lhard_surf)
	    {
	      printf
		("\t===== Totals when Integrating over fluid volume =====\n");
	      printf ("\t\t Grand Free Energy: %g \n", grand_free_energy[0]);
	      printf ("\t\t Surface Free Energy: %g \n",
	  ...
	  if (Lprint_pmf && fp != NULL)
	    {
	      if (Nwall == 2)
		fprintf (fp,
			 " Energy=%9.6f Surf_ex_energy=%9.6f  Uww[0][1]= %9.6f PMF=%9.6f ",
			 grand_free_energy[i], surface_free_energy[i],
			 Uww[0][1], surface_free_energy[i] + Uww[0][1]);
	      else if (Nwall > 2)
		fprintf (fp,
			 " Eneregy=%9.6f Surf_ex_energy=%9.6f  Total_Uww=%9.6f Total PMF%9.6f  ",
			 grand_free_energy[i], surface_free_energy[i],
			 Uww_sum, surface_free_energy[i] + Uww_sum);
	      else
		fprintf (fp, " Energy=%9.6f Surf_ex_energy=%9.6f  ",
			 grand_free_energy[i], surface_free_energy[i]);
	    }
	  else
	    fprintf (fp, " Energy=%9.6f Surf_ex_energy=%9.6f  ",
      ...
      else
	{
	  if (Lprint_pmf && fp != NULL)
	    {
	      if (Nwall == 2)
		fprintf (fp, " %9.6f %9.6f   %9.6f %9.6f ",
			 grand_free_energy[i], surface_free_energy[i],
			 Uww[0][1], surface_free_energy[i] + Uww[0][1]);
	      else if (Nwall > 2)
		fprintf (fp, " %9.6f %9.6f   %9.6f %9.6f  ",
		...
		fprintf (fp, " %9.6f %9.6f  ", grand_free_energy[i],
			 surface_free_energy[i]);
	    }
	  else
	    fprintf (fp, " %9.6f %9.6f  ", grand_free_energy[i],
...
void
assemble_HS_free_energy (double *x, double *sum_phispt, double *sum_phispt_b,
...
/* integrate phi locally  */
  ...
  for (loc_inode = 0; loc_inode < Nnodes_per_proc; loc_inode++)
      ...
      for (iel = 0; iel < Nnodes_per_el_V; iel++)
	{
	  ...
	  if (ielement != -2)
	    {
	      if (ielement == -1 || Wall_elems[Nlists_HW - 1][iel_box] != -1)
		nel_hit--;
	    }
	}
      ...
      for (iel = 0; iel < Nnodes_per_el_V; iel++)
	{
	  ...
	  if (ielement != -2)
	    {
	      if (ielement == -1 || Wall_elems[0][iel_box] != -1)
		nel_hit_b--;
	    }
...
double
energy_elec_vext_vol (double *x)
{
  int loc_inode, loc_i, inode, iel_box, ijk_box[3], jln, ielement,
    reflect_flag[3];
...
double
energy_elec (double *x, double *sum3)
{
  ...
  double sum = 0.0, wall_avg_psi[NWALL_MAX], sblock = 0.0, sopen = 0.0;
  ...
  *sum3 = 0.;
  ...
  for (loc_inode = 0; loc_inode < Nnodes_per_proc; loc_inode++)
      ...
      for (ilist = 0; ilist < Nlists_HW; ilist++)
	{
      ...
      if (iwall != -1)
	{			/* sum up the boundary element contributions */
...
/*          for (iel_w=0; iel_w<Nelems_S[Nlists_HW-1][loc_inode]; iel_w++){
	  ...
	  for (iel_w = 0; iel_w < Nelems_S[ilist][loc_inode]; iel_w++)
		...
		{
		  deriv_x_A[idim] =
		    calc_deriv_e (idim, inode_box, BFD, &blocked, x, ilist);
		  deriv_x_B[idim] =
		    calc_deriv_e (idim, inode_box, FFD, &blocked, x, ilist);
		}
	      else
		{
		  deriv_x_A[idim] =
...
/*              printf("iel_w: %d  surf_norm: %d  idim: %d\n",iel_w,surf_norm,idim);*/
	      ...
	      sum +=
		x[loc_i] * (deriv_x_A[idim] -
			    deriv_x_B[idim]) * prefac * geom_factor[idim];
...
double
charge_stress (double *x, double *sum2)
{
  int loc_inode, iwall, idim, inode, ijk[3], inode_box, iel_box;
	  ...
	  if (iel >= 0)
	      ...
	      for (idim = 0; idim < Ndim; idim++)
		{
		  ...
		  switch (Ndim)
		    {
		    case 3:
		      if ((idim == 0 && (loc_node_el == 0 || loc_node_el == 2
					 || loc_node_el == 4
					 || loc_node_el == 6)
			   && ijk[0] < Nodes_x[0] - 1) || (idim == 1
							   && (loc_node_el ==
							       0
							       || loc_node_el
							       == 1
							   ...
							   Nodes_x[1] - 1)
			  || (idim == 2
			      && (loc_node_el == 0 || loc_node_el == 1
				  || loc_node_el == 2 || loc_node_el == 3)
			      && ijk[2] < Nodes_x[2] - 1))
			{
			  deriv2_x[idim] =
			    calc_deriv2 (idim, inode_box, FFD, x);
			  sum3 +=
			    (deriv2_x[idim] * deriv2_x[idim] * Vol_el /
			     (0.5 * Nnodes_per_el_V));
...
double
free_energy_charging_up (double *x)
{
  ...
  for (loc_inode = 0; loc_inode < Nnodes_per_proc; loc_inode++)
    {
      ...
      if (Nodes_2_boundary_wall[Nlists_HW - 1][node_to_node_box (inode)] !=
	  -1)
	{
	  ...
	  for (iel_w = 0; iel_w < Nelems_S[ilist][loc_inode]; iel_w++)
		...
		{		/* RHS */
		  ...
		  psi_deriv[i][1] =
		    (x[loc_i] -
		     x[iunk + Nunk_per_node * (loc_inode - 1)]) / Esize_x[0];
		}
	      else
		{		/* LHS */
		  i = 0;
		  psi_deriv[i][0] = -(3. * x[loc_i]
				      - 4. * x[iunk +
					       Nunk_per_node * (loc_inode -
								1)] + x[iunk +
									Nunk_per_node
...
double
int_stencil (double *x, int inode_box, int icomp, int sten_type)
{
  ...
  for (jcomp = 0; jcomp < Ncomp; jcomp++)
      ...
      for (isten = 0; isten < current_sten->Length; isten++)
	{
	  ...
	  if (inode_sten >= 0 && !Zero_density_TF[inode_sten][jcomp])
	    ...
	    {
	      if (inode_sten == -1)
		sum += weight * Rho_b[jcomp];
	      if (inode_sten == -3)
		sum += weight * Rho_coex[1];
	      if (inode_sten == -4)
		sum += weight * Rho_coex[0];
	    }
...
double
phispt_i (struct RB_Struct *rho_bar, int inode)
{
  int idim;
  ...
  if (rb3 < 1.0 && rb2 > 0.0)
	...
	phi_v = -dot_12 / (1.0 - rb3)
	  + POW_DOUBLE_INT (rb2 - dot_22 / rb2,
			    3) / (24.0 * PI * (1.0 - rb3) * (1.0 - rb3));
      return (phi_s + phi_v);
    }
  ...
  for (icomp = 0; icomp < Ncomp; icomp++)
    {
      ...
      four_pi_RSQ[icomp] =
	PI * Sigma_ff[icomp][icomp] * Sigma_ff[icomp][icomp];
      pi_six_dCBD[icomp] =
	(PI / 6.0) * Sigma_ff[icomp][icomp] * Sigma_ff[icomp][icomp] *
...
double
calc_deriv_e (int idim, int inode0, int flag, int *blocked, double *x,
	      int ilist)
{
...
double
calc_deriv2 (int idim, int inode0, int flag, double *x)
{
  int inode1, offset[3], loc_i0, loc_i1, jdim, ijk_box[3], reflect_flag[3];
  double deriv = 0.0;
...
double
calc_free_energy_polymer (FILE * fp, double *x, double fac_area,
			  double fac_vol)
{
  double /*ads[NCOMP_MAX][2], */ area, free_energy, energy =
    0.0, boltz, ideal_nz;
  ...
  for (loc_inode = 0; loc_inode < Nnodes_per_proc; loc_inode++)
      ...
      for (icomp = 0; icomp < Ncomp; icomp++)
	{
	  ...
	  loc_i =
	    Aztec.update_index[Ncomp + icomp + Nunk_per_node * loc_inode];
	  ...
	  if (Type_poly != 3)
		...
		{
		  loc_j =
		    Aztec.update_index[Ncomp + jcomp +
				       Nunk_per_node * loc_inode];
		  ifluid += Rism_cr[icomp][jcomp][0] * x[loc_j];
		  ibulk -=
		    Rism_cr[icomp][jcomp][0] * Rho_b[icomp] * Rho_b[jcomp];
		}
	    }

	  sfluid +=
	    ifluid * x[loc_i] * Nel_hit2[0][loc_i] * Vol_el /
	    ((double) Nnodes_per_el_V);
	  sbulk +=
	    ibulk * Nel_hit[0][loc_i] * Vol_el / ((double) Nnodes_per_el_V);
  ...
  for (icomp = 0; icomp < Ncomp; icomp++)
    {
      if (icomp >= Ntype_mer)
	free_energy -= Ads_ex[icomp][0];
      else
	{
	  pol_number = 0;
	  while (Nmer_t[pol_number][icomp] == 0)
	    pol_number++;
...
/* calc_u_ideal:
             ...
             boltz unknown                      */
double
calc_u_ideal (int itype_mer, int *ijk_box, double *x, double *fluid,
	      double *bulk)
{
      ...
      for (isten = 0; isten < sten->Length; isten++)
	{
	  ...
	  if (jnode_box >= 0)
		...
		weight = weight_bulk;
	      ...
	      *bulk -=
		sign * weight_bulk * Rho_b[jtype_mer] * Rho_b[itype_mer];
	      /*  if (jnode_box > bound)
