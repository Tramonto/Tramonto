...

{
  ...
  if (cgi->method == TURNING_POINT_CONTINUATION ||
    ...
    if (cgi->printproc) printf("\tcon_lib: Adding random"
  ...
  for (n = 0; n <= csi->max_steps; n++) {
    ...
    if (cgi->printproc) print_cont_step1(order, step, step_old, con);
    ...
    if (num_newt_conv < 0) {
       ...
       */
        ...
        if (cgi->printproc) print_cont_step_fail(order, step, con);
      }
    }
      ...
      if (cpi->step_num != 0 && end_passed <= 0.0) last_step = TRUE;
      ...
      if (!last_step && cgi->method == ARC_LENGTH_CONTINUATION) {
        ...
        if (cgi->printproc) printf(" Tangent factor is %9g\n",tang_factor);
      ...
      /* Repeat step if tang_factor is too small */
        ...
        if (cpi->step_num > 1
          ...
          if (cgi->printproc) printf(" Step limit exceeded: Retrying ...");
          tan_flag = FALSE;
          step *= 0.5;
          cgi->param = cpi->param_old + step;
          assign_parameter_conwrap(cgi->param);
       ...
       */
        ...
        if (cgi->printproc) print_cont_step2(order, step, con);
