...
******************************************************************************/
{
  ...
  int sn_old = 0, sn_new;	/* Sign of cpi->dp_ds, to check for a
				   turning point                        */
  ...
  int last_step = FALSE;	/* Signals end of continuation routine       */
  ...
  for (n = 0; n <= csi->max_steps; n++)
	...
	print_cont_step1 (order, step, step_old, con);
      ...
      num_newt_conv =
	nonlinear_solver_conwrap (x, (void *) con, cpi->step_num);
      ...
      else
	{
	  ...
	  if (!last_step && cgi->method == ARC_LENGTH_CONTINUATION)
	    {
	      if (cgi->printproc)
		{
		...
		tang_factor = fabs (scaled_dot_prod (x_tang_old, cpi->x_tang,
						     cpi->scale_vec,
						     cgi->numOwnedUnks)) /
		  sqrt (scaled_dot_prod
			(x_tang_old, x_tang_old, cpi->scale_vec,
			 cgi->numOwnedUnks) * scaled_dot_prod (cpi->x_tang,
							       cpi->x_tang,
							       cpi->scale_vec,
							       cgi->
							       numOwnedUnks));
	  ...
	  if (tan_flag == TRUE)
	      ...
	      else
		{
		  ...
		  /* normal step control */
		  if (!last_step && csi->step_ctrl > 0.0)
		    {
		      if (order == 2)
		...
		case PITCHFORK_CONTINUATION:
		  ...
		  break;
		case HOPF_CONTINUATION:
		  solution_output_conwrap (3, x, cgi->param,
					   con->hopf_info.y_vec,
		...
		case PHASE_TRANSITION_CONTINUATION:
		  solution_output_conwrap (2, x, cgi->param,
					   ...
					   con->phase_transition_info.
					   bif_param, NULL, 0, cpi->step_num,
					   num_newt_conv, con);
		  break;
		}
	      ...
	      if (last_step)
		{
		  ...
		  if (cgi->printproc)
		    {
		      printf
			("Simulation completed continuation in %d steps\n",
			 cpi->step_num);
	      ...
	      if (n < csi->max_steps)
		{
		  ...
		  switch (order)
		      ...
		      if (cgi->printproc)
			{
		      ...
		      calc_rhs_continuation (CONT_TANGENT, x, cpi->x_tang,
					     NULL, NULL, NULL, cgi->param,
					     NULL, cgi->numUnks,
					     cgi->numOwnedUnks);
		      ...
		      else
			{
			  ...
			  if ((cgi->printproc) && sn_old != sn_new)
			    printf
...
static double
solution_scale (struct con_struct *con, struct arc_scale_struct *arc)
{
...
double
scaled_dot_prod (double *x, double *y, double *scale_vec, int n)
{
  int i;
  double sum = 0;
...
/*****************************************************************************/
static double
simple_step_control (int num_newt_conv, int max_Newton_steps,
		     double step_ctrl)
/*
...
/*****************************************************************************/
static void
print_cont_step1 (int order, double step, double step_old,
		  struct con_struct *con)
/*
 ...
 */
{
  ...
  printf ("\nStart of Step: %5d   Continuation Param = %9.5g from "
	  "%9.5g\n", cpi->step_num, cgi->param, cgi->param - step);
  printf ("\tContinuation cgi->method = %s\n", string);
...
static void
print_cont_step2 (int order, double step, struct con_struct *con)
/*
...
static void
print_cont_step_fail (int order, double step, struct con_struct *con)
/*
...
/*****************************************************************************/
static void
print_line (char *charstr, int ntimes)
{
  int i;
