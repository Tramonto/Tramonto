...
void
set_initial_guess (int iguess, double *x, int *niters)
{
  /*
  ...
  if (Proc == 0 && Iwrite == VERBOSE)
    {
      if (Restart == 0)
	printf ("generating guess from scratch\n");
      else
	printf ("setting up guess from existing files\n");
    }

  if (Restart > 0 || Imain_loop > 0
      || (*niters == 1 && Load_Bal_Flag >= LB_TIMINGS))
    {
      start_no_info = FALSE;

      x_new =
	(double *) array_alloc (1, Nnodes * Nunk_per_node, sizeof (double));

      if (Proc == 0)
	{			/* Proc 0 reads in the data file */
	  ...
	  if (Imain_loop == 0 && *niters == 0)
	      ...
	      if (Print_rho_type != PRINT_RHO_0 && Imain_loop > 0)
		{
		  ...
		  if (Lbinodal && iguess == BINODAL_FLAG)
		    ...
		    sprintf (filename, "dft_dens.%0d", i1);
		}
	      else
		{
		  if (Lbinodal && iguess == BINODAL_FLAG)
	  ...
	  if (Nodes_old != Nnodes)
	      ...
	      if (Nwall > 0 && Nwall < 3)
		{
		  if (Nwall == 1)
			...
			sep = Guess_range[1];	/* use old guess */
		    }
		  else if (Nwall == 2)
		    {
		  ...
		  if (sep < Guess_range[0])
		    start_no_info = TRUE;
		  else if (sep > Guess_range[1])
		    fac = 1.0;
		  else if (Guess_range[0] == Guess_range[1]
			   || Guess_range[0] < 0.0 || Guess_range[1] < 0.0)
		    fac = 1.0;
		  else
		    fac =
		      (sep - Guess_range[0]) / (Guess_range[1] -
						Guess_range[0]);
		  ...
		  if (start_no_info)
		    {
		      if (Iwrite != NO_SCREEN)
			printf
			  ("Starting from bulk intial guess everywhere\n");
		    }
		  else
		    {
		      if (Iwrite != NO_SCREEN)
			printf
			  ("Mixing old soln and bulk with %9.6f percent of old profile\n",
			   fac * 100.);
		    }
		}
	      else
  ...
  if (start_no_info)
    {				/* START FROM A SPECIFIED INITIAL GUESS */
  ...
  if (Iwrite == VERBOSE)
	...
	{
	  X_old =
	    (double *) array_alloc (1, Nnodes * Nunk_per_node,
				    sizeof (double));
	  Vext_old =
	    (double *) array_alloc (1, Nnodes * Ncomp, sizeof (double));
	}
      ...
      if (Proc == 0)
	{
	  ...
	  else if (Lbinodal && iguess == BINODAL_FLAG)
	    print_profile ("rho_init2.dat");
	  safe_free ((void *) &X_old);
...
void
setup_polymer_field_step (double *x, int iguess)
{
  ...
  for (loc_inode = 0; loc_inode < Nnodes_per_proc; loc_inode++)
	...
	{
	  loc_RHO =
	    Aztec.update_index[Ncomp + itype_mer + Nunk_per_node * loc_inode];
	  loc_i = Aztec.update_index[itype_mer + Nunk_per_node * loc_inode];
	  if (x[loc_RHO] < 1.e-6)
	    field = VEXT_MAX - 1.;
	  else
	    field = -log (x[loc_RHO] / Rho_b[itype_mer]);
	  x[loc_i] = exp (-field);
	}
    }
  return;
...
/*setup_polymer_field: in this routine set up the field guesses
                       for the polymers variables    */
void
setup_polymer_field (double *x, int iguess)
{
      ...
      for (itype_mer = 0; itype_mer < Ncomp; itype_mer++)
	{
	  ...
	  if (!Zero_density_TF[inode_box][itype_mer])
	      ...
	      for (jtype_mer = 0; jtype_mer < Ncomp; jtype_mer++)
		{
		  ...
		  sten =
		    &(Stencil[sten_type][izone]
		      [itype_mer + Ncomp * jtype_mer]);
		  ...
		  for (isten = 0; isten < sten->Length; isten++)
		      ...
		      if (jnode_box >= 0)
			{
			  if (Lhard_surf)
			    {
			      if (Nodes_2_boundary_wall[jlist][jnode_box] !=
				  -1)
				weight =
				  HW_boundary_weight (jtype_mer, jlist,
						      sten->HW_Weight[isten],
						      jnode_box,
						      reflect_flag);
			      else
				weight = weight_bulk;
			  ...
			  if (iguess == CONST_RHO)
				...
				temp -=
				  (weight - weight_bulk) * Rho_b[jtype_mer];
			      else
			  ...
			  else if (iguess == STEP_PROFILE)
			    {
			      ...
			      for (i = 0; i < Nsteps; i++)
				{
				  if (nodepos[Orientation_step[i]] >=
				      Xstart_step[i]
				      && nodepos[Orientation_step[i]] <=
				      Xend_step[i])
				    {
				      if (!Zero_density_TF[jnode_box]
					  [jtype_mer])
					{
					  temp -=
					    (weight * Rho_step[jtype_mer][i] -
		...
		temp = -1.0;
	      ...
	      if ((Sten_Type[POLYMER_CR] == 1) || (temp > 0.5))
		x[loc_i] = exp (-temp);
	      else
		x[loc_i] = exp (1. - sqrt (1. - 2. * temp));

	    }			/* end if i not zero density  */
	  else
	    x[loc_i] = DENSITY_MIN;	/* zero density - Boltzmann probability = 0 */
...
void
setup_polymer_simple (double *x, int iguess)
{
  ...
  for (loc_inode = 0; loc_inode < Nnodes_per_proc; loc_inode++)
      ...
      for (itype_mer = 0; itype_mer < Ncomp; itype_mer++)
	{
	  ...
	  if (!Zero_density_TF[inode_box][itype_mer])
		...
		{
		  loc_j =
		    Aztec.update_index[jtype_mer + Nunk_per_node * loc_inode];
		      ...
		      for (i = 0; i < Nsteps; i++)
			{
			  if (nodepos[Orientation_step[i]] >= Xstart_step[i]
				...
				temp +=
				  ...
				  (Rho_step[jtype_mer][i] - Rho_b[jtype_mer]);
			      else
				temp -=
				  Rism_cr[itype_mer][jtype_mer][0] *
				  Rho_b[jtype_mer];
	      ...
	      if ((Sten_Type[POLYMER_CR]) || (temp > 0.5))
		x[loc_i] = exp (temp);
	      else
		x[loc_i] = exp (1. - sqrt (1. - 2. * temp));
	    }			/* end if i not zero density  */
...
void
setup_polymer_rho (double *x, int iguess)
{
  ...
  for (loc_inode = 0; loc_inode < Nnodes_per_proc; loc_inode++)
      ...
      if (iguess == CONST_RHO)
	{
	  for (i = 0; i < Ncomp; i++)
	    {
	      loc_i =
		Aztec.update_index[i + Ncomp + Nunk_per_node * loc_inode];
		  ...
		  for (icomp = 0; icomp < Ncomp; icomp++)
		    {
		      if (loc_inode == 20)
			printf
			  ("i=%d Xstart_step=%g  Xend_step=%g nodepos[%d]=%g Rho_step[%d]=%g\n",
			   i, Xstart_step[i], Xend_step[i],
			   Orientation_step[i], nodepos[0], icomp,
			   Rho_step[icomp][i]);
		      loc_i =
			Aztec.update_index[icomp + Ncomp +
	...
	{
	  if (Proc == 0)
	    printf ("invalid initial guess for polymers\n");
	  exit (1);
	}
    }
  return;
	      ...
	      for (ibond = 0; ibond < Nbond[poln][iseg]; ibond++)
		{
		  ...
		  if (fabs (x[Aztec.update_index[iunk]] - 999.0) < 1.e-6)
		      ...
		      if (Bonds[poln][iseg][ibond] == -1)
			{
			  if (Iwrite == VERBOSE)
			    printf
			      ("computing end segment poln=%d iseg=%d ibond=%d this cycle=%d \n",
			       poln, iseg, ibond, cycle);
			  for (loc_inode = 0; loc_inode < Nnodes_per_proc;
			       loc_inode++)
			    {
			      inode_box = L2B_node[loc_inode];
			      node_box_to_ijk_box (inode_box, ijk_box);
			      loc_i =
				Aztec.update_index[iunk +
						   Nunk_per_node * loc_inode];
			      loc_j =
			...
			{
			  jtype_mer =
			    Type_mer[poln][Bonds[poln][iseg][ibond]];
			  junk =
			    Geqn_start[poln] + 2 * Bonds[poln][iseg][ibond];
			  if (iseg < Bonds[poln][iseg][ibond])
			    junk += 1;
			  /* test if this G equation has been generated yet ... if not, go on */
			  if (fabs (x[Aztec.update_index[junk]] - 999.0) >
			      1.e-6)
			    {
			      if (Iwrite == VERBOSE)
				printf
				  ("computing poln=%d iseg=%d ibond=%d this cycle=%d \n",
				   poln, iseg, ibond, cycle);
			      for (loc_inode = 0; loc_inode < Nnodes_per_proc;
				   loc_inode++)
				{
				  ...
				  if (!Zero_density_TF[inode_box][jtype_mer])
					   ...
					   isten++)
					{
					  ...
					  if (jnode_box >= -1)
						...
						{	/* use the value at loc_inode ... an approximation */
						  loc_j =
						    Aztec.update_index[junk +
								       Nunk_per_node
								       *
  ...
  for (loc_inode = 0; loc_inode < Nnodes_per_proc; loc_inode++)
      ...
      for (poln = 0; poln < Npol_comp; poln++)
	{
	  ...
	  for (iseg = 0; iseg < Nmer[poln]; iseg++)
		...
		{
		  loc_j =
		    Aztec.update_index[Type_mer[poln][iseg] +
				       Nunk_per_node * loc_inode];
		  x[loc_i] = x[loc_j];
		  if (Type_poly == 2)
		    x[loc_i] = 1.0;
	      ...
	      else
		{
		  ...
		  /* Boltzmann factor */
		  loc_j =
		    Aztec.update_index[itype_mer + Nunk_per_node * loc_inode];
		  x[loc_i] *= x[loc_j];
	      ...
	      else
		{
		  ...
		  if (!Zero_density_TF[inode_box][jtype_mer])
		      ...
		      for (isten = 0; isten < sten->Length; isten++)
			{
			  ...
			  /* Find the Stencil point */
			  jnode_box =
			    offset_to_node_box (ijk_box, offset,
						reflect_flag);
...
void
setup_const_density (double *x, double *rho, int nloop, int index)
{
	  ...
	  if (Lsteady_state)
	      ...
	      if (!Zero_density_TF[inode_box][i])
		{
		  if (nloop > 1)
		    ...
		    x[loc_i] = Betamu[index];
		}
	      else
		x[loc_i] = -10.0;	/* zero density == -infinite chem.pot */
	    }
	}
    }
  return;
...
/*setup_stepped_profile: in this routine set up a stepped
        density profile wherever Zero_density_TF = FALSE */
void
setup_stepped_profile (double *x)
{
  ...
  if (Lsteady_state)
    {
      printf
	("stepped profile not set up for chemical potentials at this time\n");
      exit (-1);
    }
  return;
}
...
void
setup_exp_density (double *x, double *rho, int nloop, int index)
{
  ...
  for (loc_inode = 0; loc_inode < Nnodes_per_proc; loc_inode++)
      ...
      for (i = 0; i < nloop; i++)
	{
	  ...
	  if (!Zero_density_TF[inode_box][i])
	    {
	      if (nloop > 1)
		x[loc_i] = rho[i] * exp (-Vext[loc_inode][i]);
	      else
		x[loc_i] = rho[index] * exp (-Vext[loc_inode][i]);
	    }
	  else
	    x[loc_i] = 0.0;

	  if (x[loc_i] > 1.0)
	    x[loc_i] = 0.99;	/* may need this to
	  ...
	  if (!Zero_density_TF[inode_box][icomp])
	    {
	      ...
	      if (Lsteady_state)
		x[loc_i] =
		  Rho_b_LBB[icomp] + (Rho_b_RTF[icomp] -
				      Rho_b_LBB[icomp]) * x_dist /
		  Size_x[Grad_dim];
	      else
...
setup_rho_bar (double *x)
{
  ...
  for (loc_inode = 0; loc_inode < Nnodes_per_proc; loc_inode++)
	...
	{
	  loc_i =
	    Aztec.update_index[Ncomp + iunk + Nunk_per_node * loc_inode];
...
void
setup_elec_pot (double *x, int iguess)
{
  ...
  for (loc_inode = 0; loc_inode < Nnodes_per_proc; loc_inode++)
    {
      if (Type_poly == -1)
	loc_i =
	  Aztec.update_index[Ncomp + Nrho_bar + Ngeqn_tot +
			     Nunk_per_node * loc_inode];
      else
	loc_i =
      ...
      if (Lsteady_state && iguess == LINEAR)
	{
	  ...
	  x[loc_i] = Elec_pot_LBB +
	    (Elec_pot_RTF - Elec_pot_LBB) * x_dist / Size_x[Grad_dim];
	}
      else
	x[loc_i] = 1.0;
    }
...
setup_chem_pot (double *x)
{
  ...
  for (loc_inode = 0; loc_inode < Nnodes_per_proc; loc_inode++)
	...
	{
	  if (Ipot_ff_c == 1)
	    iunk = Ncomp + Nrho_bar + 1 + icomp;
	  else
	    iunk = Ncomp + Nrho_bar + icomp;
	  loc_i = Aztec.update_index[iunk + Nunk_per_node * loc_inode];
	  if (!Zero_density_TF[inode_box][icomp])
	    {
	      if (Ipot_ff_c == 1)
		{
		  x[loc_i] =
		    log (x
			 [Aztec.
			  update_index[icomp + Nunk_per_node * loc_inode]]) +
		    Charge_f[icomp] *
		    (x
		     [Aztec.update_index[Ncomp + Nunk_per_node * loc_inode]]);

		}
	      x[loc_i] =
		Betamu_LBB[icomp] + (Betamu_RTF[icomp] -
				     Betamu_LBB[icomp]) * x_dist /
		Size_x[Grad_dim];
	    }
	  else
	    x[loc_i] = -VEXT_MAX;
...
int
find_length_of_file (char *filename)
{
  int c, nodes_old = 0;
  FILE *fp;
  if (Proc == 0)
...
void
read_in_a_file (int iguess, char *filename)
{
  int c;
  int i, iunk, idim, inode, nunk_tot, itype_mer, ipol, iseg, index, dim_tmp;
  int ijk_old[3], ijk_old_max[3];
  ...
  for (index = 0; index < Nodes_old; index++)
	...
	{
	  if (Sten_Type[POLYMER_CR])
	    nunk_tot = 2 * Ncomp + Ngeqn_tot;
	  else
	    nunk_tot = Ncomp;
	}
      else
	nunk_tot = Nunk_per_node;
      for (iunk = 0; iunk < nunk_tot; iunk++)
	{
	  if (Sten_Type[POLYMER_CR])
	      ...
	      if (iunk < Ncomp)
		{
		  if (Lbinodal && iguess == BINODAL_FLAG)
		    fscanf (fp5, "%lf",
			    &X2_old[Ncomp + inode * Nunk_per_node + iunk]);
		  else
		    fscanf (fp5, "%lf",
	      ...
	      else if (iunk < 2 * Ncomp)
		{
		  ...
		  if (Lbinodal && iguess == BINODAL_FLAG)
		    ...
		    {
		      X_old[inode * Nunk_per_node + iunk - Ncomp] =
			exp (-tmp);
		    }
		}
	      else if (iunk < 2 * Ncomp + Ngeqn_tot)
		{
		  ...
		  if (Type_poly == 2)
			...
			X2_old[inode * Nunk_per_node + iunk] =
			  tmp / X2_old[inode * Nunk_per_node + itype_mer];
		      else
			X_old[inode * Nunk_per_node + iunk] =
			  tmp / X_old[inode * Nunk_per_node + itype_mer];
		    }
		  else
		    {
		      if (Lbinodal && iguess == BINODAL_FLAG)
...
void
shift_the_profile (double *x_new, double fac)
{
      ...
      for (iunk = 0; iunk < Nunk_per_node; iunk++)
	{
	  ...
	  switch (Pos_new_nodes)
	    ...
	    case 0:		/*ADDING NODES TO CENTER OF BOX */
	      ...
	      if (ijk[idim] < Nodes_x_old[idim] / 2)
		{		/*NODE LBB OF NEW PLANE */
		  inode_old = locate_inode_old (ijk);
		  unk_old = X_old[inode_old * Nunk_per_node + iunk];
		}
	      else if (ijk[idim] > Nodes_x_old[idim] / 2 + Nadd)
		{		/*NODE RTF OF NEW PLANE */
		  for (jdim = 0; jdim < Ndim; jdim++)
		    ijk_tmp[jdim] = ijk[jdim];
		  ijk_tmp[idim] -= Nadd;
		  inode_old = locate_inode_old (ijk_tmp);
		  unk_old = X_old[inode_old * Nunk_per_node + iunk];
		}
	      else
		{		/*NODE IN CENTER OF NEW PLANE */
		  for (jdim = 0; jdim < Ndim; jdim++)
		    ijk_tmp[jdim] = ijk[jdim];
	  ...
	  if (iunk < Ncomp)
	    x_test =
	      min (Rho_max, fac * (unk_old - Rho_b[iunk]) + Rho_b[iunk]);

	  else if (iunk < Ncomp + Nrho_bar)
	    {
	      x_test =
		fac * (unk_old - Rhobar_b[iunk - Ncomp]) + Rhobar_b[iunk -
								    Ncomp];
	      if (iunk == Ncomp && x_test >= 1.0)
		x_test = Rhobar_b[iunk - Ncomp];
	    }
...
locate_inode_old (int *ijk)
{
  ...
  if (Ndim == 1)
    ...
    inode_old = ijk[0] + ijk[1] * Nodes_x_old[0];
  else if (Ndim == 3)
    inode_old = ijk[0] + ijk[1] * Nodes_x_old[0]
...
void
communicate_profile (double *x_new, double *x)
{
...
check_zero_densities (double *x)
{
  ...
  for (loc_inode = 0; loc_inode < Nnodes_per_proc; loc_inode++)
      ...
      for (icomp = 0; icomp < Ncomp; icomp++)
	{
	  ...
	  if (Sten_Type[POLYMER_CR])
	    loc_j =
	      Aztec.update_index[icomp + Ncomp + Nunk_per_node * loc_inode];
	...
	{
	  loc_i =
	    Aztec.update_index[Ncomp + Nrho_bar + Nunk_per_node * loc_inode];
	  x[loc_i] = 1.0;
	}
    }
  return;
...
void
chop_profile (double *x, int iguess)
{
  ...
  for (loc_inode = 0; loc_inode < Nnodes_per_proc; loc_inode++)
	...
	for (iwall = 0; iwall < Nwall; iwall++)
	  if (X_wall[loc_inode][iwall] > Xstart_step[0])
	    check++;
	if (check == Nwall)
	  {
	    loc_i = Aztec.update_index[icomp + Nunk_per_node * loc_inode];
	    if (iguess == CHOP_RHO_L)
	      x[loc_i] = Rho_coex[1];
	    else if (iguess == CHOP_RHO_V)
	      x[loc_i] = Rho_coex[0];
	    else if (iguess == CHOP_RHO)
	      x[loc_i] = Rho_b[icomp];
	    else if (iguess == CHOP_RHO_STEP)
	      x[loc_i] = Rho_step[icomp][0];
	  }
      }
  return;
}
...
double
interpolate (double *pos, int icomp, int nodes_old,
	     double **pos_old, double *Esize_old)
{
  ...
  for (idim = 0; idim < Ndim; idim++)
	...
	if (pos_old[i][idim] > Lmax[idim])
	  Lmax[idim] = pos_old[i][idim];
    }
	  ...
	  for (idim = 0; idim < Ndim; idim++)
	    {
		  ...
		  && x_corner[npoints][idim] > Lmax[idim])
		pos_test = pos_old[i][idim] + Size_x[idim];
	      else
		pos_test = pos_old[i][idim];
	      if (fabs (x_corner[npoints][idim] - pos_test) < 1.e-8)
		match++;
	    }
	  if (match == Ndim)
	    func[npoints] = X_old[i * Nunk_per_node + icomp];
	}
    }
  ...
  for (npoints = 0; npoints < Nnodes_per_el_V; npoints++)
    {
      if (func[npoints] == -1.0)
	{
	  if (npoints == 0 && func[1] != -1.0)
	    func[0] = func[1];
	  else if (npoints == 1 && func[0] != -1.0)
	    func[1] = func[0];
	  else if (npoints == 2 && func[3] != -1.0)
	    func[2] = func[3];
	  else if (npoints == 3 && func[2] != -1.0)
	    func[3] = func[2];
	  else if (npoints == 4 && func[5] != -1.0)
	    func[4] = func[5];
	  else if (npoints == 5 && func[4] != -1.0)
	    func[5] = func[4];
	  else if (npoints == 6 && func[7] != -1.0)
	    func[6] = func[7];
	  else if (npoints == 7 && func[6] != -1.0)
	    func[7] = func[6];
	  else
	    func[npoints] = 1.0e-5;
/*        else{
  ...
  if (Ndim == 1)
    ...
    interp =
      (1.0 - fac[0]) * (1.0 - fac[1]) * func[0] + fac[0] * (1.0 -
							    fac[1]) *
      func[1] + fac[1] * (1.0 - fac[0]) * func[3] + fac[1] * fac[0] * func[2];
  else if (Ndim == 3)
    {
