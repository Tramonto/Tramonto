...
void
rcb (struct rcb_dot **dots,	/* array of dots, may be extended */
  ...
  )
{
  ...
  if (dotmax > 0)
    {
      ...
      if (dotmark == NULL || dotlist == NULL)
	rcb_error (dotmax * sizeof (int));
    }
  else
    {
      dotmark = NULL;
  ...
  if (RCB_CHECK)
    {
      j = 0;
      for (i = 0; i < dotnum; i++)
	if (dotpt[i].weight <= 0.0)
	  j++;
      MPI_Allreduce (&j, &k, 1, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
      if (k > 0)
	{
	  if (proc == 0)
	    printf ("RCB ERROR: %d dot weights are <= 0\n", k);
	  return;
	}
    }
  ...
  while (proclower != procupper)
      ...
      for (i = 0; i < dotnum; i++)
	{
	  ...
	  if (dotpt[i].weight > wtmax)
	    wtmax = dotpt[i].weight;
	}
      ...
      while (1)
	{
	  ...
	  for (j = 0; j < numlist; j++)
	    {
	      i = dotlist[j];
	      if (dotpt[i].x[dim] <= valuehalf)
		{		/* in lower part */
		  medme.lototal += dotpt[i].weight;
		  dotmark[i] = 0;
		  if (dotpt[i].x[dim] > medme.valuelo)
		    {		/* my closest dot */
		...
		{		/* in upper part */
		  ...
		  dotmark[i] = 1;
		  if (dotpt[i].x[dim] < medme.valuehi)
		    {		/* my closest dot */
	  ...
	  if (weightlo + med.lototal < targetlo)
	    {			/* lower half TOO SMALL */
		...
		{		/* multiple dots to move */
		  ...
		  if (medme.valuehi == med.valuehi)
		    wtok = medme.wthi;
		  if (weightlo + med.wthi >= targetlo)
		    {		/* can't move all of them */
		      MPI_Scan (&wtok, &wtupto, 1, MPI_DOUBLE, MPI_SUM,
				local_comm);
		      wtmax = targetlo - weightlo;
		      if (wtupto > wtmax)
			wtok = wtok - (wtupto - wtmax);
		      breakflag = 1;
		    }		/* wtok = most I can move */
		  for (j = 0, k = 0, wtsum = 0.0;
		       j < numlist && wtsum <= wtok; j++)
		    {
		      i = dotlist[j];
		      if (dotpt[i].x[dim] == med.valuehi)
			{	/* only move if better */
		  ...
		  if (breakflag)
		    break;	/* done if not moved all */
		}

	      weightlo += med.wthi;	/* as close as needed */
	      if (fabs (weight - 2.0 * weightlo) <= tolerance)
		break;
		...
		{		/* multiple dots to move */
	      ...
	      if (fabs (weight - 2.0 * weighthi) <= tolerance)
		break;
	...
	if (dotmark[i] == markactive)
      ...
      if (readnumber)
	{
	  MPI_Recv (&incoming, 1, MPI_INT, procpartner, 0, MPI_COMM_WORLD,
		    &status);
	  if (readnumber == 2)
	    {
	      MPI_Recv (&incoming2, 1, MPI_INT, procpartner2, 0,
      ...
      if (dotnew > dotmax)
	{
	  ...
	  if (dotpt == NULL)
	    rcb_error (dotmax * sizeof (struct rcb_dot));
	  *dots = dotpt;
	  if (RCB_STATS)
	    counters[6]++;
	}
      ...
      if (RCB_STATS)
	{
	  counters[1] += outgoing;
	  counters[2] += incoming;
	  if (dotnew > counters[3])
	    counters[3] = dotnew;
	  if (dotmax > counters[4])
	    counters[4] = dotmax;
	}
      ...
      if (outgoing > 0)
	{
	  ...
	  if (dotbuf == NULL)
	    rcb_error (outgoing * sizeof (struct rcb_dot));
	}
      else
	dotbuf = NULL;
  ...
  if (RCB_CHECK)
    rcb_check (dotpt, dotnum, *pdotnum, rcbbox);
  if (RCB_STATS)
    rcb_stats (timestop - timestart, dotpt, dotnum,
...
rcb_error (int size)
{
  ...
  MPI_Comm_rank (MPI_COMM_WORLD, &proc);
  printf ("RCB ERROR: proc = %d could not malloc/realloc %d bytes", proc,
	  size);
  exit (1);
}
...
void
rcb_box_merge (void *in, void *inout, int *len, MPI_Datatype * dptr)
{
  int i;
...
void
rcb_median_merge (void *in, void *inout, int *len, MPI_Datatype * dptr)
{
...
void
rcb_check (struct rcb_dot *dotpt, int dotnum, int dotorig,
  ...
  for (i = 0; i < dotnum; i++)
    {
      ...
      if (dotpt[i].weight > wtone)
	wtone = dotpt[i].weight;
    }
...
void
rcb_stats (double timetotal, struct rcb_dot *dotpt,
	   ...
	   struct rcb_box *rcbbox, int reuse)
{
  ...
  if (proc == 0 && Iwrite == VERBOSE)
    printf ("RCB total time: %g (secs)\n", timetotal);

  if (proc == 0 && Iwrite == VERBOSE)
    printf ("RCB Statistics:\n");
  ...
  if (proc == 0 && Iwrite == VERBOSE)
    printf (" Start-up time %%: ave = %g, min = %g, max = %g\n",
	    ave / timetotal * 100.0, rmin / timetotal * 100.0,
	    rmax / timetotal * 100.0);
  MPI_Barrier (MPI_COMM_WORLD);
  if (RCB_STATS == 2 && Iwrite == VERBOSE)
...
/* Weights must be greater than zero! (weight > 0)                           */
{
  ...
  for (icomp = 0; icomp < Ncomp; icomp++)
	...
	max = Cut_ff[icomp][icomp];
      else
	max = 0.5 * Sigma_ff[icomp][icomp];

      for (idim = 0; idim < Ndim; idim++)
	{
	  if ((Type_bc[idim][0] == REFLECT
	       && ijk[idim] * Esize_x[idim] <= max)
	      || (Type_bc[idim][1] == REFLECT
		  && (Nodes_x[idim] - ijk[idim]) * Esize_x[idim] <= max))
	    weight += 0.2;
	  ...
	  if ((Type_bc[idim][0] == IN_WALL
	       && ijk[idim] * Esize_x[idim] <= 2.0 * max)
	      || (Type_bc[idim][1] == IN_WALL
		  && (Nodes_x[idim] - ijk[idim]) * Esize_x[idim] <=
		  2.0 * max))
	    weight += 0.5;
	}
    }
  ...
  if (Mesh_coarsening || L1D_bc)
    {
      if (Mesh_coarsen_flag[inode_box] < 0
	  && Mesh_coarsen_flag[inode_box] >= -3)
	{
	  weight = 0.02;	/* trivial residual */
	}
      else if (Mesh_coarsen_flag[inode_box] == FLAG_1DBC
	       || Mesh_coarsen_flag[inode_box] == FLAG_BULK)
	weight = 0.001;
      else if (Mesh_coarsen_flag[inode_box] > 0)
	{
	  ...
	  weight /= POW_INT (2,
			     (int) (Ndim *
				    (Mesh_coarsen_flag[inode_box] - 1.0)));
	}
    }
...
void
load_balance (int flag, double *fill_time)
...
/* otherwise, the routine set_weight_for_node is used.                       */
{
  ...
  if (Proc == 0 && Iwrite == VERBOSE)
    {
      ...
      printf
	("\tBefore load_balance: Max=%d  Min=%d  Avg=%g (Unknowns per Proc)\n",
	 nmax, nmin, navg);
    }
  ...
  for (loc_inode = 0; loc_inode < Nnodes_per_proc; loc_inode++)
	...
	if (flag == 0)
	  ...
	  dots[loc_inode].weight = set_weight_for_node (inode);
      else
  ...
  treept =
    (struct rcb_tree *) array_alloc (1, Num_Proc, sizeof (struct rcb_tree));

  /* call rcb routine, which returns new values of dots, Nnodes_per_proc, etc */

  if (Load_Bal_Flag != LB_LINEAR)
    rcb (&dots, &(Nnodes_per_proc), &pdotmax, 1, 1.5, 0, &pdottop, &rcbbox,
	 treept);
  ...
  if (Proc == 0 && Iwrite == VERBOSE)
    printf
      ("\tBefore load_balance: Max %d  Min %d  Ave %g Unknowns per Proc\n",
       nmax, nmin, navg);
  ...
  if (Proc == 0 && Iwrite == VERBOSE)
    printf
      ("\tAfter load_balance: Max=%d  Min=%d  Avg=%g (Unknowns per Proc)\n",
       nmax, nmin, navg);
  ...
  for (loc_inode = 0; loc_inode < Nnodes_per_proc; loc_inode++)
	...
	Aztec.update[loc_inode * Nunk_per_node + icomp] =
      ...
      for (idim = 0; idim < Ndim; idim++)
	{
	  if (ijk[idim] < Min_IJK[idim])
	    Min_IJK[idim] = ijk[idim];
	  if (ijk[idim] > Max_IJK[idim])
	    Max_IJK[idim] = ijk[idim];
	}
    }
