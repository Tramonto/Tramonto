...
/*
 *  FILE: dft_surfaces_cylindricalTaperedPore3D.c
 *
 ...
 *
 */
#include "dft_surfaces_cylindricalTaperedPore3D.h"
/****************************************************************************/
void surface_cylindricalTaperedPore3D_inSurfaceTest(int iwall,int iwall_type,int loc_inode,int flag_setup_Xwall,
                                  double *fluidEl_center, double **image_pos,
                                  double dist_adjustments, double *delr,
                                  int *logical_inwall, int *logical_nearWallDielec)
{

  double dx,x12,r12_sq_sum,r12,radius,radius1,radius2;
  ...
  *logical_inwall=FALSE;
  ...
  radius1=sgeom_iw->radius1+dist_adjustments-roff;
  ...
  if (Ipot_ff_c==COULOMB && r12 > radius-Dielec_X && r12<=radius && x12<halflength+Dielec_X) *logical_nearWallDielec  = TRUE;
   ...
   if (flag_setup_Xwall){
       printf("error : the Vext_1D_xmin option is not available for finite 3D cylindrical pores\n");
