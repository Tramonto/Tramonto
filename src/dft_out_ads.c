...
double
calc_adsorption (FILE * fp, double *x, double fac_area, double fac_vol)
{
      ...
      for (icomp = 0; icomp < Ncomp; icomp++)
	{
	  for (i = 0; i < Imax; i++)
		...
		loc_i = Aztec.update_index[icomp + Nunk_per_node * loc_inode];
	      ...
	      Ads_ex[icomp][i] +=
		(x[loc_i] * Nel_hit2[i][loc_i] -
		 Rho_b[icomp] * Nel_hit[i][loc_i]) * Vol_el /
		((double) Nnodes_per_el_V);
	      Ads[icomp][i] +=
		(x[loc_i] * Nel_hit2[i][loc_i]) * Vol_el /
		((double) Nnodes_per_el_V);
  ...
  if (Proc == 0 && fp != NULL)
	...
	{
	  for (icomp = 0; icomp < Ncomp; icomp++)
	    fprintf (fp, "ads[%d]=%9.6f ", icomp, Ads[icomp][i]);
	  for (icomp = 0; icomp < Ncomp; icomp++)
	    fprintf (fp, "ads_ex[%d]=%9.6f ", icomp, Ads_ex[icomp][i]);
	  fprintf (fp, " area=%9.4f fac_area=%9.4f fac_vol=%9.4f", area,
		   fac_area, fac_vol);
	  first = FALSE;
	}
      else
	{
	  for (icomp = 0; icomp < Ncomp; icomp++)
	    fprintf (fp, "%9.6f ", Ads[icomp][i]);
	  for (icomp = 0; icomp < Ncomp; icomp++)
	    fprintf (fp, "%9.6f ", Ads_ex[icomp][i]);
...
void
calc_surface_charge (FILE * fp, double *x, double fac_area, double fac_vol)
{
	      ...
	      if (ielement != -2)
		{
		  ...
		  for (jcomp = 0; jcomp < Ncomp; jcomp++)
		    if (iwall >= 0 && Lsemiperm[WallType[iwall]][jcomp])
		      semiperm = TRUE;
		  if (ielement == -1 || (iwall != -1 && !semiperm))
		    nel_hit--;
		}
	    }
	  for (idim = 0; idim < Ndim; idim++)
	    {
	      if ((Type_bc[idim][0] == REFLECT || Type_bc[idim][0] == IN_BULK
		   || Type_bc[idim][0] == LAST_NODE) && ijk[idim] == 0)
		nel_hit /= 2;
	      if ((Type_bc[idim][1] == REFLECT || Type_bc[idim][1] == IN_BULK
