...
struct AZ_SCALING
{				/* Left and right matrices to to scale    */
  /* the problem                            */
  int action;
  double A_norm;
  int mat_name;
...
struct grid
{				/* used to define a grid. Still under */
  /* construction                       */
...
{
#endif
  extern void AZ_oldsolve (double x[], double b[], int options[],
			   double params[], double status[],
...
{
#endif
  extern void AZ_iterate (double x[], double b[], int options[],
			  double params[], double status[], int proc_config[],
...
extern void AZ_fortransolve (double x[], double b[], int options[], double params[], int data_org[], double status[], int proc_config[], int indx[], int bindx[],	/* VBR & MSR arrays. Note: all of these   */
			     ...
			     /* user's matvec:                         */
			     int A_ival0[], int A_ival1[], int A_ival2[], int A_ival3[], double A_dval0[], double A_dval1[], double A_dval2[], double A_dval3[], AZ_FUNCTION_PTR A_fun0, AZ_FUNCTION_PTR A_fun1, AZ_FUNCTION_PTR A_fun2, AZ_FUNCTION_PTR A_fun3, AZ_FUNCTION_PTR user_precond,	/* user's preconditioning routine         */
			     /*                                        */
...
/* BLAS, LAPACK stuff */
  ...
  extern FSUB_TYPE daxpy_ (int *n1, double *a1, double *x, int *skipx1,
			   double *y, int *skipy1);
  ...
  extern FSUB_TYPE dlaswp_ (int *, double *, int *, int *, int *, int *,
			    int *);
  ...
  extern FSUB_TYPE dpotrf_ (char *, int *, double *, int *, int *,
			    unsigned int);
  ...
  extern FSUB_TYPE dlaic1_ (int *, int *, double *, double *, double *,
...
/* Aztec function prototypes that can be called by the user */
  ...
  extern void AZ_solve (double x[],	/* On input 'x' contains the initial guess. On output */
			...
			int options[], double params[], int indx[],	/* The ith element of indx points to the location in */
  ...
  extern void AZ_finalize (double x[], double b[], int options[], int
			   proc_config[], AZ_MATRIX * Amat,
			   AZ_PRECOND * precond, int save_old_values[],
			   struct AZ_SCALING *scaling);

  extern void AZ_iterate_setup (int options[], double params[],
				int proc_config[], AZ_MATRIX * Amat,
				AZ_PRECOND * precond);
  ...
  extern void AZ_oldsolve_finish (double x[], double b[], int options[],
				  int proc_config[], AZ_MATRIX * Amat,
				  int save_old_values[], struct AZ_SCALING *);

  extern void AZ_abs_matvec_mult (double *b, double *c, AZ_MATRIX * Amat,
				  int proc_config[]);

  extern void AZ_add_new_ele (int cnptr[], int col, int blk_row, int bindx[],
			      int bnptr[], int indx[], double val[],
			      int therow, double new_ele, int maxcols,
  ...
  extern void AZ_backsolve (double newa[], double pivot[], double x[],
			    int snr[], int ha[], int iflag[], int *ifail,
			    int *nn, int *n, int *iha);

  extern void AZ_block_diagonal_scaling (int action, AZ_MATRIX * Amat,
					 double val[], int indx[],
					 int bindx[], int rpntr[],
					 int cpntr[], int bpntr[],
  ...
  extern void AZ_broadcast (char *ptr, int length, int proc_config[],
			    int action);
  ...
  extern void AZ_calc_blk_diag_inv (double *val, int *indx, int *bindx,
				    ...
				    int *d_rpntr, int *d_bpntr,
				    int data_org[]);

  extern void AZ_calc_blk_diag_LU (double *val, int *indx, int *bindx,
				   int *rpntr, int *cpntr, int *bpntr,
				   double *d_inv, int *d_indx, int *d_bindx,
  ...
  extern double AZ_calc_precond_flops (int solver_flag, int options[],
				       double daxpy_flops,
				       double matvec_flops, int total_its,
  ...
  extern void AZ_check_vbr (int N_update, int N_external, int option,
			    int bindx[], int bnptr[], int cnptr[],
			    int rnptr[], int proc_config[]);

  extern void AZ_combine_overlapped_values (int sym_flag, int data_org[],
					    int options[], double x[],
					    int map[], double ext_vals[],
					    int name, int proc_config[]);

  extern int AZ_compare_update_vs_soln (int N, double, double alpha,
					double p[], double x[],
					double update_reduction,
					int ouput_flag, int proc_config[],
					int *first_time);
  ...
  extern void AZ_compute_global_scalars (AZ_MATRIX * Amat,
					 ...
					 int proc_config[], int *use_r,
					 double v1[], double v2[],
					 double *value,
					 struct AZ_CONVERGE_STRUCT *);

  extern void AZ_compute_matrix_size (AZ_MATRIX * Amat, int options[],
				      int N_nz_unpadded, int N_unpadded,
				      int *N_nz_padded, int N_external,
				      int *max_row, int *N, int *N_nz,
				      double fill, int *extra_fact_nz_per_row,
  ...
  extern AZ_MATRIX *AZ_create_matrix (int local, int additional,
				      int matrix_type, int local_blks,
				      int *not_using);
  ...
  extern void AZ_exchange_local_info (int N_neighbors,
  ...
  extern int AZ_extract_comm_info (int **idata_org,
				   int (*user_comm) (double *, AZ_MATRIX *),
				   AZ_MATRIX *, int proc_config[], int N_cols,
				   int Nghost);
  ...
  extern void AZ_fact_lu (double x[], AZ_MATRIX * A_overlapped, double *aflag,
			  double *pivot, int *rnr, int *ha, int *iflag, int *,
			  int *, int *, int *, int *);
  ...
  extern int AZ_find_block_in_row (int bindx[], int bnptr[], int i,
				   int blk_col, int indx[], int, double val[],
				   int blk_space, int nz_space);

  extern void AZ_find_MSR_ordering (int bindx2[], int **ordering, int N,
				    int **inv_ordering, int name,
				    struct context *);
  ...
  extern void AZ_flop_rates (int data_org[], int indx[], int bpntr[],
			     int bindx[], int options[], double status[],
			     double total_time, int proc_config[]);
  ...
  extern int AZ_get_block (int j, int k, int bindx[], int bpntr[],
			   int *ptr_j);
  ...
  extern void AZ_init_quick_find (int list[], int length, int *shift,
				  int *bins);

  extern void AZ_init_subdomain_solver (struct context *context);

  extern void AZ_invorder_vec (double vec[], int data_org[],
			       int update_index[], int rpntr[],
			       double newarray[]);

  extern void AZ_list_print (int ivec[], int length, double dvec[],
			     int length2);

  extern void AZ_loc_avg (AZ_MATRIX * Amat, double r[], double newr[],
			  int N_fixed, int fixed_pts[], int proc_config[]);
  ...
  extern struct AZ_MATRIX_STRUCT *AZ_submatrix_create (AZ_MATRIX * Amat,
						       int Nsub_rows,
						       int sub_rows[],
						       int Nsub_cols,
						       int sub_cols[],
  ...
  extern struct AZ_MATRIX_STRUCT *AZ_blockmatrix_create (AZ_MATRIX **
							 submat_list,
							 int Nsub_mats,
							 int **submat_locs,
							 int Nblock_rows,
  ...
  extern struct AZ_PREC_STRUCT *AZ_precond_create (struct AZ_MATRIX_STRUCT
						   *Pmat,
						   void (*prec_fun) (double *,
								     int *,
								     int *,
								     double *,
								     struct
  ...
  extern void AZ_matfree_2_msr (AZ_MATRIX * Amat, double *val, int *bindx,
				int N_nz);
  ...
  extern void AZ_msr2vbr_mem_efficient (int N, int **ibindx, double **ival,
					int **icpntr, int **ibpntr,
					int **iindx, int *N_blk_rows,
					int name, char *label, int);
  ...
  extern void AZ_pad_matrix (struct context *context, int proc_config[],
			     int N_unpadded, int *N, int **map,
			     int **padded_data_org, int *N_nz,
			     int estimated_requirements);

  extern void AZ_pbicgstab (double *, double *, double *, int *, double *,
			    int *, double *, AZ_MATRIX *, AZ_PRECOND *,
			    struct AZ_CONVERGE_STRUCT *);
  ...
  extern void AZ_pgmresr (double *, double *, double *, int *, double *,
			  int *, double *, AZ_MATRIX *, AZ_PRECOND *,
			  struct AZ_CONVERGE_STRUCT *);
  ...
  extern void AZ_print_out (int update_index[], int extern_index[],
			    int update[], int external[], double val[],
			    int indx[], int bindx[], int rpntr[], int cpntr[],
			    int bpntr[], int proc_config[], int choice,
			    int matrix, int N_update, int N_external,
			    int off_set);
  ...
  extern void AZ_random_vector (double u[], int data_org[],
				int proc_config[]);
  ...
  extern void AZ_input_msr_matrix (char datafile[], int update[],
				   double **val, int **bindx, int N_update,
				   int proc_config[]);

  extern void AZ_input_update (char datafile[], int *N_update_blks,
			       int *update_blks[], int proc_config[],
  ...
  extern void AZ_recover_sol_params (int instance, int **sub_options,
				     double **sub_params, double **sub_status,
  ...
  extern void AZ_reorder_vec (double vec[], int data_org[],
			      int update_index[], int rpntr[]);

  extern void AZ_global2local (int data_org[], int bindx[], int update[],
			       int update_index[], int externs[],
			       int extern_index[]);

  extern void AZ_restore_unreordered_bindx (int bindx[], double val[],
  ...
  extern void AZ_scale_f (int action, AZ_MATRIX * Amat, int options[],
			  double b[], double x[], int proc_config[],
			  struct AZ_SCALING *scaling);

  extern struct AZ_SCALING *AZ_scale_matrix_only (AZ_MATRIX * Amat,
						  int options[],
						  int proc_config[]);
  ...
  extern void AZ_unscale_after_iterate (double x[], double b[],
					AZ_MATRIX * Amat, int options[],
					int proc_config[],
					struct AZ_SCALING *scaling);
  ...
  extern void AZ_set_MATFREE_getrow (AZ_MATRIX * Amat, void *data,
				     int (*getrow) (int *, double *, int *,
						    ...
						    int, int *, int),
				     int (*user_comm) (double *, AZ_MATRIX *),
				     int N_ghost, int proc_config[]);

  extern void AZ_set_MSR (AZ_MATRIX * Amat, int bindx[], double val[],
			  int data_org[], int N_update, int update[],
			  int option);

  extern void AZ_set_VBR (AZ_MATRIX * Amat, int rpntr[], int cpntr[],
			  int bpntr[], int indx[], int bindx[], double val[],
			  int data_org[], int N_update, int update[],
			  int option);
  ...
  extern int AZ_set_solver_parameters (double *params, int *options,
				       AZ_MATRIX * Amat, AZ_PRECOND * Pmat,
				       struct AZ_SCALING *S);

  extern void AZ_setup_dd_olap_msr (int N_rows, int *New_N_rows, int *bindx,
				    double *val, int olap_size,
				    int *proc_config, int *data_org[],
				    int **map3, int bindx_length, int name,
				    int *prev_data_org,
				    int estimated_requirements,
				    struct context *context);
  ...
  extern void AZ_sort_dble (char a[], int indx[], int start, int end, int b[],
			    int *mid, int real_lists, char buffer[],
			    int buf_len, int afirst, int);
  ...
  extern void AZ_space_for_factors (double input_fill, int N_nz, int N,
				    int *extra_factor_nonzeros, int options[],
				    int bandwidth, int);

  extern void AZ_space_for_kvecs (int request, int **kvec_sizes,
				  double ***saveme, double **ptap,
				  int *options, int *data_org, char *suffix,
				  int proc, double **);

  extern void AZ_space_for_padded_matrix (int overlap, int N_nonzeros, int N,
					  int *extra_rows,
					  int *extra_nonzeros, int N_external,
					  int *largest);
  ...
  extern void AZ_sym_gauss_seidel_sl (double val[], int bindx[], double x[],
				      ...
				      struct context *, int proc_config[]);
  ...
  extern void AZ_transform (int proc_config[], int *external[], int bindx[],
			    ...
			    int N_update, int indx[], int bnptr[],
...
/*****************************************************************************/
  extern unsigned int md_wrap_iread (void *, unsigned int, int *, int *,
				     MPI_AZRequest *);

  extern unsigned int md_wrap_iwrite (void *, unsigned int, int, int, int *,
				      MPI_AZRequest *);

  extern unsigned int md_wrap_wait (void *, unsigned int, int *, int *, int *,
				    MPI_AZRequest *);
...
#define mdwrap_write(a,b,c,x,y)   md_mpi_write((a),(b),(c),(x),(y),proc_config)
  ...
  extern unsigned int md_mpi_write (void *, unsigned int, int, int, int *,
				    int *);
#else
...
/*****************************************************************************/
  ...
  extern void y12mdf_ (int *n, double val[], int *nn, double b[],
		       double pivot[], int snr[], int ha[], int *iha,
		       int iflag[], int *ifail);
...
/*****************************************************************************/
  ...
  extern void AZ_print_vbr_matrix (int matrix_flag,	/* = 0 no matrix output, = 1 output matrix */
...
/*****************************************************************************/
  ...
  extern void AZ_psymmlq (double *, double *, double *, int *, double *,
			  int *, double *, AZ_MATRIX *, AZ_PRECOND *);



  extern void AZ_gather_mesg_info (double x[], int data_org[], char **,
				   char **, int *, int *);
