...
double
integrate_potential (int flag, double param1, double param2, double param3,
		     int ngp, int ngpu, double *gp, double *gpu, double *gw,
		     double *gwu, double *node_pos, double *node_pos_f)
{
	  ...
	  if (flag == FLAG_LJ)
	    {
	      radius /= cut;
	      weight =
		get_wt_from_sten (radius, sigma, eps, cut, ngpu, gpu, gwu);
	    }
	  else
	    {
	      printf
		("Error - we can't compute coulomb external fields or wall-wall potentials\n");
	      printf
		("        in 1 dimension...would require Ewald summation.\n");
	      exit (-1);
	    }
      ...
      for (ig = 0; ig < ngp; ig++)
	{
	  ...
	  for (jg = 0; jg < ngp; jg++)
	      ...
	      if (flag == FLAG_LJ)
		{
		  radius /= cut;
		  weight =
		    get_wt_from_sten (radius, sigma, eps, cut, ngpu, gpu,
				      gwu);
		}
	      else
		{
		  printf
		    ("Error - we can't compute coulomb external fields or wall-wall potentials\n");
		  printf
		    ("        in 2 dimensions...would require Ewald summation.\n");
		  exit (-1);
		}
	  ...
	  for (jg = 0; jg < ngp; jg++)
	      ...
	      for (kg = 0; kg < ngp; kg++)
		{
		  ...
		  if (flag == FLAG_LJ)
		    ...
		    weight = get_wt_from_sten_coul3D (radius, z1, z2);
...
double
get_wt_from_sten (double r, double sigma, double eps, double rcut,
...
double
get_wt_from_sten_coul3D (double r, double z1, double z2)
{
...
double
uLJ12_6_cut (double r, double sigma, double eps, double rcut)
{
  double u;
  ...
  if (r <= rcut)
    {
      u =
	(POW_DOUBLE_INT (sigma / r, 12) - POW_DOUBLE_INT (sigma / rcut, 12)) -
...
double
uderiv_LJ12_6 (double r, double x, double sigma, double eps, double rcut)
{
  double vext_dash;
  ...
  if (r <= rcut)
    {
      vext_dash =
	(1.0 / sigma) * (-12. * x * POW_DOUBLE_INT (sigma / r, 14) +
			 6. * x * POW_DOUBLE_INT (sigma / r, 8));
    }
  else
    vext_dash = 0.0;
...
double
Vext_1D (double x, int icomp, int iwall_type)
{
  double vext, xmin;
  ...
  if (x <= Cut_wf[icomp][iwall_type])
    {
      ...
      vext =
	(2.0 * PI / 3.0) * (POW_DOUBLE_INT (Sigma_wf[icomp][iwall_type], 3)) *
	((2.0 / 15.0) *
	 (POW_DOUBLE_INT (Sigma_wf[icomp][iwall_type] / x, 9) -
	  POW_DOUBLE_INT (Sigma_wf[icomp][iwall_type] /
			  Cut_wf[icomp][iwall_type],
			  9)) -
	 (POW_DOUBLE_INT (Sigma_wf[icomp][iwall_type] / x, 3) -
	  POW_DOUBLE_INT (Sigma_wf[icomp][iwall_type] /
			  Cut_wf[icomp][iwall_type], 3)));
...
double
Vext_1D_dash (double x, int icomp, int iwall_type)
{
  double vdash, xmin;
  xmin = pow (2. / 5., 1. / 6.);
  if (x <= Cut_wf[icomp][iwall_type])
...
/*    x += xmin;*/
      ...
      vdash =
	(2.0 * PI / 3.0) * (POW_DOUBLE_INT (Sigma_wf[icomp][iwall_type], 2)) *
...
double
uLJ_wp (double r, int icomp, int iwall_type)
{
  double vext;
...
double
uCOULOMB (double r, double z1, double z2)
{
  double u;
...
uLJatt_n (double r, int i, int j)
{
  ...
  if (r <= Cut_ff[i][j])
	...
	r = r_min;
      ...
      uatt = 4.0 * Eps_ff[i][j] * sigma6 * (sigma6 * (r12_inv - rc12_inv)
					    - (r6_inv - rc6_inv));
    }
  else
    uatt = 0.0;
...
double
uLJatt_n_int (double r, int i, int j)
{
  ...
  uatt_int =
    16 * PI * Eps_ff[i][j] * sigma6 * (-sigma6 * r9_inv / 9.0 + r3_inv / 3.0);
...
double
uLJatt_n_noshift (double r, int i, int j)
{
  double uatt, r_min, sigma2, sigma6;
  double r_inv, r2_inv, r6_inv, r12_inv;
...
double
uCOULOMB_att (double r, int i, int j)
{
  double uatt, r_min;
...
double
uCOULOMB_att_int (double r, int i, int j)
{
  double uCOUL_int;
