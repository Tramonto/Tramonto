...
/******************************************************************************/
void
setup_external_field_n (int **nelems_w_per_w, int ***elems_w_per_w)
 ...
 */
{
  ...
  Vext_set =
    (double **) array_alloc (2, Nnodes_per_proc, Ncomp, sizeof (double));
  ...
  for (iwall = 0; iwall < Nwall; iwall++)
	...
	case VEXT_1D_ORIENTATION:	/* a different treatment of 2D or 3D systems using 1D potentials */
	  printf
	    ("This external field option (Ipot_wf_n[%d]=%d\n) has not yet been implemented\n",
	...
	case VEXT_3D_INTEGRATED:	/* a more proper treatment of 2D or 3D systems where the 12-6 LJ 
	  ...
	  elems_w_per_w_global =
	    (int **) array_alloc (1, Nlists_HW, sizeof (int *));
	  nelems_w_per_w_global =
	    (int *) array_alloc (1, Nlists_HW, sizeof (int));
	  comm_wall_els (iwall, nelems_w_per_w, elems_w_per_w,
			 nelems_w_per_w_global, elems_w_per_w_global);
	  setup_integrated_LJ_walls (iwall, nelems_w_per_w_global,
				     elems_w_per_w_global);
...
/*       case HARD_CHARGED_ATOMS :       / * note this case was only written for speed 
	...
	default:
	  ...
	  printf ("iwall=%d  Type=%d Ipot_wf_n=%d\n", iwall, WallType[iwall],
		  Ipot_wf_n[WallType[iwall]]);
	  exit (-1);
	}
    }
  t_vext += MPI_Wtime ();
...
/* Finally, we may still have discrepancies in the Zero_TF array
  ...
  for (inode_box = 0; inode_box < Nnodes_box; inode_box++)
    if (B2L_node[inode_box] >= 0)
      {
	if (!Zero_density_TF[inode_box][0])
	  count++;
      }
  ...
  for (icomp = 0; icomp < Ncomp; icomp++)
      ...
      for (loc_inode = 0; loc_inode < Nnodes_per_proc; loc_inode++)
	{
	  ...
	  if (fabs (xpos[0] + 0.5 * Size_x[0] - 7.25) < 1.e-8
	    ...
	    {
	      printf
		("Proc=%d inode=%d loc_inode=%d icomp=%d zero_density_TF=%d\n",
		 Proc, inode, loc_inode, icomp,
...
void
setup_semiperm (int **nelems_w_per_w, int ***elems_w_per_w)
{
  ...
  for (loc_inode = 0; loc_inode < Nnodes_per_proc; loc_inode++)
    for (icomp = 0; icomp < Ncomp; icomp++)
      Vext_set[loc_inode][icomp] = VEXT_MAX;
	...
	if (Lsemiperm[WallType[iwall]][icomp])
	  {
	    ...
	    for (wall_el = 0; wall_el < nelems_w_per_w[ilist][iwall];
		 wall_el++)
	      {
		...
		for (i = 0; i < Nnodes_per_el_V; i++)
		  {
		    for (idim = 0; idim < Ndim; idim++)
		      ijk_box[idim] = ijk_box_0[idim];
		    if (i > 3)
		      ijk_box[2] = ijk_box_0[2] + 1;
		    if (i == 2 || i == 3 || i == 6 || i == 7)
		      ijk_box[1] = ijk_box_0[1] + 1;
		    if (i == 1 || i == 3 || i == 5 || i == 7)
		      ijk_box[0] = ijk_box_0[0] + 1;
		    flag = TRUE;
		    for (idim = 0; idim < Ndim; idim++)
		      {
			ijk[idim] = ijk_box[idim] + Min_IJK_box[idim];
			if (ijk[idim] >= Nodes_x[idim])
			  flag = FALSE;
		      }
		    if (flag)
		      {
			...
			if (loc_inode >= 0)
			  Vext_set[loc_inode][icomp] =
			    Vext_membrane[WallType[iwall]][icomp];
		      }
		  }
	      }
	  }
...
/* setup_1Dvext: set up the 9-3 lennard jones potential for planar 
                   walls in 1D */
void
setup_1Dvext (int iwall)
{
  int icomp, iwall_type, inode_box, iunk, i;
  int loc_inode, inode;
  int maximum, image;
  double max_cut, **image_pos, node_pos_w[3], node_pos_f[3], x, prefac, sign;
  ...
  image = 0;
  node_pos_w[Orientation[iwall_type]] =
    WallPos[Orientation[iwall_type]][iwall];
  image_pos[image][Orientation[iwall_type]] =
    node_pos_w[Orientation[iwall_type]];
	  ...
	  if (!Zero_density_TF[inode_box][icomp])
		  ...
		  Vext[loc_inode][icomp] == 0.0)
		{
		  ...
		  if (Ipot_wf_n[iwall_type] == VEXT_3D_INTEGRATED ||
		      ...
		      || Type_bc[Orientation[iwall_type]][0] == REFLECT
		      || Type_bc[Orientation[iwall_type]][1] == REFLECT)
		    {
		      find_images_1D (Orientation[iwall_type],
				      Cut_wf[icomp][iwall_type], &image,
		  ...
		  for (i = 0; i < image; i++)
		    {
		      ...
		      x =
			fabs (node_pos_f[Orientation[iwall_type]] -
			      image_pos[i][Orientation[iwall_type]]) -
		      ...
		      if (x > 0.00001)
			{
			  Vext[loc_inode][icomp] +=
			    prefac * Vext_1D (x, icomp, iwall_type);
			}
		      else
			{
			  Vext[loc_inode][icomp] = Vext_set[loc_inode][icomp];
  ...
  for (loc_inode = 0; loc_inode < Nnodes_per_proc; loc_inode++)
    {
      ...
      image = 0;
      image_pos[image][Orientation[iwall_type]] =
	node_pos_f[Orientation[iwall_type]];
      node_pos_w[Orientation[iwall_type]] =
	WallPos[Orientation[iwall_type]][iwall];
	  ...
	  if (!Zero_density_TF[inode_box][icomp])
	      ...
	      if (Vext[loc_inode][icomp] != Vext_set[loc_inode][icomp])
		{
		  ...
		  for (i = 0; i < image; i++)
		    {
		      x =
			fabs (image_pos[i][Orientation[iwall_type]] -
			      node_pos_w[Orientation[iwall_type]]) -
			WallParam[iwall_type];
		      if (x > 0.00001)
			{
			  ...
			  Vext_dash[loc_inode][iunk][Orientation[iwall_type]]
			    +=
			    sign * prefac * Vext_1D_dash (x, icomp,
							  iwall_type);
			}
	  ...
	  if (!Zero_density_TF[inode_box][icomp])
	    {
	      ...
	      prefac =
		eps * Rho_w[iwall_type] *
		POW_DOUBLE_INT (Sigma_wf[icomp][iwall_type] /
				Sigma_ww[iwall_type][iwall_type], 3);
...
/*          if (Vext[loc_inode][icomp] < Vext_set[loc_inode][icomp] ||
		  ...
		  for (i = 0; i < image; i++)
			...
			Vext[loc_inode][icomp] += prefac *
			  (Vext_1D (x, icomp, iwall_type) +
			   Vext_1D (x2, icomp, iwall_type));
		      else
			{
			  Vext[loc_inode][icomp] = Vext_set[loc_inode][icomp];
			  break;
...
void
setup_vext_LJ_atomic (int iwall)
{
  ...
  double max_cut, sign, **image_pos, node_pos_w[3],
    node_pos_f[3], node_pos_w2[3], r_center_sq, r, x[3], node_pos_f2[3];
      ...
      for (icomp = 0; icomp < Ncomp; icomp++)
	{
	  if (!Zero_density_TF[inode_box][icomp])
		  ...
		  Vext[loc_inode][icomp] == 0.0)
		{
		  ...
		  on_ref_bound = FALSE;
		  if ((WallPos[0][iwall] == -0.5 * Size_x[0]
		       && Type_bc[0][0] == REFLECT)
		      || (WallPos[0][iwall] == 0.5 * Size_x[0]
			  && Type_bc[0][1] == REFLECT))
		    on_ref_bound = TRUE;

		  if (!on_ref_bound)
		    find_images (0, Cut_wf[icomp][iwall_type],
				 &image, image_pos, node_pos_w, node_pos_f);

		  if (Ndim > 1)
		    {
		      on_ref_bound = FALSE;
		      if ((WallPos[1][iwall] == -0.5 * Size_x[1]
			   && Type_bc[1][0] == REFLECT)
			  || (WallPos[1][iwall] == 0.5 * Size_x[1]
			      && Type_bc[1][1] == REFLECT))
			on_ref_bound = TRUE;
		      if (!on_ref_bound)
			{
		  ...
		  if (Ndim == 3)
		    {
		      on_ref_bound = FALSE;
		      if ((WallPos[2][iwall] == -0.5 * Size_x[2]
			   && Type_bc[2][0] == REFLECT)
			  || (WallPos[2][iwall] == 0.5 * Size_x[2]
			      && Type_bc[2][1] == REFLECT))
			on_ref_bound = TRUE;
		      if (!on_ref_bound)
			{
		  ...
		  for (i = 0; i < image; i++)
		      ...
		      for (idim = 0; idim < Ndim; idim++)
			{
			  ...
			  r_center_sq +=
			    (node_pos_w2[idim] -
			     node_pos_f[idim]) * (node_pos_w2[idim] -
						  node_pos_f[idim]);
			  x[idim] =
			    fabs (node_pos_f[idim] - node_pos_w2[idim]);
			}
		      r = sqrt (r_center_sq);
			...
			{
			  Vext[loc_inode][icomp] +=
			    uLJ12_6_cut (r, Sigma_wf[icomp][iwall_type],
					 Eps_wf[icomp][iwall_type],
      ...
      for (icomp = 0; icomp < Ncomp; icomp++)
	{
	  ...
	  if (!Zero_density_TF[inode_box][icomp] &&
		...
		on_ref_bound = TRUE;
	      if (!on_ref_bound)
		find_images (0, Cut_wf[icomp][iwall_type],
			     &image, image_pos, node_pos_f, node_pos_w);
		  ...
		  if (r > 0.00001)
		      ...
		      for (idim = 0; idim < Ndim; idim++)
			{
			  if (x[idim] != 0.0)
			    sign =
			      (image_pos[i][idim] -
			       node_pos_w[idim]) / x[idim];
			  else
			    sign = 1.0;

			  Vext_dash[loc_inode][iunk][idim] +=
			    sign * uderiv_LJ12_6 (r, x[idim],
						  Sigma_wf[icomp][iwall_type],
						  Eps_wf[icomp][iwall_type],
						  Cut_wf[icomp][iwall_type]);
			}
		    }
...
/* setup_vext_HS_atomic:  In this routine we have atomic hard sphere
                          surfaces */
void
setup_vext_HS_atomic (int iwall)
{
  ...
  if (Surface_type[iwall_type] == atomic_centers
      || Surface_type[iwall_type] == point_surface)
    {
      ...
      for (loc_inode = 0; loc_inode < Nnodes_per_proc; loc_inode++)
	{
	  ...
	  for (icomp = 0; icomp < Ncomp; icomp++)
		...
		{
		  r_test =
		    0.5 * Sigma_ww[iwall_type][iwall_type] +
		    0.5 * Sigma_ff[icomp][icomp];
...
/* setup_integrated_LJ_walls:  In this routine we assume that materials properties
                            ...
                            Lennard-Jones potentials.*/
void
setup_integrated_LJ_walls (int iwall, int *nelems_w_per_w,
			   int **elems_w_per_w)
{
		  ...
		  if (Vext[loc_inode][icomp] < Vext_set[loc_inode][icomp] ||
		      ...
		      for (i = 0; i < image; i++)
			{
			  ...
			  for (idim = 0; idim < Ndim; idim++)
			    {
			      node_pos_w2[idim] =
				image_pos[i][idim] - 0.5 * Esize_x[idim];
			      r_center_sq +=
				(node_pos_w2[idim] -
				 node_pos_f[idim]) * (node_pos_w2[idim] -
						      node_pos_f[idim]);
			  ...
			  if (Lsemiperm[iwall_type][icomp] &&
			      r_center_sq <
			      1.1224620 * Sigma_wf[icomp][iwall_type])
			    {
			      Vext_set[loc_inode][icomp] =
				Vext_membrane[WallType[iwall]][icomp];
			    }
			  ...
			  vext =
			    integrate_potential (FLAG_LJ,
						 Sigma_wf[icomp][iwall_type],
						 Eps_wf[icomp][iwall_type],
						 Cut_wf[icomp][iwall_type],
						 ngp, ngpu, gp, gpu, gw, gwu,
						 node_pos_w2, node_pos_f);
...
/*comm_wall_els: In this routine we collect all the wall elements
               ...
               external fields. */
void
comm_wall_els (int iwall, int **nelems_w_per_w, int ***elems_w_per_w,
  ...
  for (ilist = 0; ilist < Nlists_HW; ilist++)
    {
      ...
      wall_elem_tmp =
	(int *) array_alloc (1, nelems_w_per_w[ilist][iwall], sizeof (int));
	...
	(int *) array_alloc (1, n_tot, sizeof (int));
...
void
comm_vext_max (int *nnodes_vext_max, int **nodes_vext_max)
{
  ...
  for (icomp = 0; icomp < Ncomp; icomp++)
    {
      ...
      vext_max_nodes_tmp =
	(int *) array_alloc (1, Nnodes_per_proc, sizeof (int));
...
/*comm_loc_to_glob_vec: core communication routine for gathering up a
          ...
          processor. */
void
comm_loc_to_glob_vec (int *n_loc, int *in_loc_vec, int *out_glob_vec)
{
  int ierror, iproc;
  int *range, *displs;
...
void
read_external_field_n (char *filename)
{
  ...
  if (Proc == 0)
    {
      if (Iwrite == VERBOSE)
	printf
	  ("setting up the external field from an existing dft_vext.dat file\n");
      ...
      for (i = 0; i < Nnodes; i++)
	{
	  ...
	  for (icomp = 0; icomp < Ncomp; icomp++)
	    fscanf (fp7, "%lf", &vext_tmp[inode][icomp]);
...
void
read_zero_density_TF (char *filename)
{
  ...
  if (Proc == 0)
    {
      if (Iwrite == VERBOSE)
	printf
	  ("setting up the zero_density array from an existing %s file\n",
	   filename);
      if ((fp7 = fopen (filename, "r")) == NULL)
	{
	  printf ("the file %s does not exist\n", filename);
