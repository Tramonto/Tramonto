...
static void set_multiple_B2L (int *ijk_box, int icomp, int loc_i);
#ifdef NUMERICAL_JACOBIAN
static void do_numerical_jacobian (double *x, double *resid,
				   double *resid_tmp);
#endif
...
int
solve_problem (double **x_internal_ptr, double **x2_internal_ptr,
 ...
 */
{
  ...
  int max_nonzeros =
    1, min_nonzeros, tot_nonzeros, max_ints, max_exts, min_ints, min_exts;
  double time_preproc = 0.0, t_solve_max = 0.0;

  int ijk[3], ijk_box[3] = { 0, 0, 0 }, inode_box, icomp;
  FILE *fp1 = NULL;
  ...
  *niters = iter;
  ...
  for (outer = outer_start; outer <= REGULAR_LOOP; outer++)
      ...
      if (outer == ONE_ITER_LOOP)
	{
	  max_iter = 1;
	  fill_time =
	    (double *) array_alloc (1, Nnodes_per_proc, sizeof (double));
	  fill_non0 =
	    (double *) array_alloc (1, Nnodes_per_proc, sizeof (double));
	}
      else
	{
	  ...
	  if (Load_Bal_Flag >= LB_TIMINGS && !Lbinodal)
	    {			/* need Xold2 for Lbinodal */
		...
		load_balance (1, fill_time);
	      else if (Load_Bal_Flag == LB_NON0)
		load_balance (1, fill_non0);
	      else if (Load_Bal_Flag == LB_MIXED)
		{
		  ...
		  for (i = 0; i < Nnodes_per_proc; i++)
		    fill_time[i] +=
		      fill_non0[i] * t_solve_max / (double) max_nonzeros;
		  load_balance (1, fill_time);
		}
		...
		array_alloc (1, Aztec.N_update, sizeof (double));
	      ...
	      safe_free ((void *) &fill_time);
	      fill_time = NULL;
	      safe_free ((void *) &fill_non0);
	      fill_non0 = NULL;
...
*/
	...
	fill_resid_and_matrix_P (NULL, NULL, bindx_2d, NULL, MSR_PREPROCESS,
				 iter, FALSE);
      else
	fill_resid_and_matrix (NULL, NULL, bindx_2d, NULL, MSR_PREPROCESS,
			       iter, FALSE);
      T_msr_setup += MPI_Wtime ();
      ...
      if (Proc == 0 && Iwrite != NO_SCREEN)
	{
      ...
      Aztec.val =
	(double *) array_alloc (1, Aztec.nonzeros + 1, sizeof (double));
      if (Aztec.val == NULL)
	{
	...
	for (j = 0; j < bindx_2d[Aztec.N_update][i]; j++)
      ...
      for (i = 0; i < Aztec.N_update + 1; i++)
	safe_free ((void *) &bindx_2d[i]);
      ...
      if (Load_Bal_Flag >= LB_NON0 && iter == 0)
	{
	  for (i = 0; i < Nnodes_per_proc; i++)
	    fill_non0[i] =
	      Aztec.bindx[(i + 1) * Nunk_per_node] -
	      Aztec.bindx[i * Nunk_per_node] + Nunk_per_node;
	}
...
*/
      ...
      if (Lbinodal)
	x2 = (double *) array_alloc (1, Nunk_int_and_ext, sizeof (double));

      if (iter == 0 || (iter == 1 && Load_Bal_Flag >= LB_TIMINGS))
	{
	  set_initial_guess (Iguess1, *x_internal_ptr, &iter);
	  if (Lbinodal)
	    set_initial_guess (BINODAL_FLAG, *x2_internal_ptr, &iter);
	  if (Proc == 0 && iter == 1)
 ...
 */
	...
	{
	  for (i = 0; i < Aztec.N_update; i++)
	    x2[Aztec.update_index[i]] =
	      (*x2_internal_ptr)[Aztec.update_index[i]];
      ...
      for (i = 0; i < Aztec.N_update; i++)
	{
	  ...
	  icomp = Aztec.update[i] - Nunk_per_node * ijk_to_node (ijk);
	  ijk_to_ijk_box (ijk, ijk_box);
	  inode_box = ijk_box_to_node_box (ijk_box);
	  B2L_unknowns[inode_box * Nunk_per_node + icomp] =
	    Aztec.update_index[i];
	  ...
	  if (Non_unique_G2B[3])
	    set_multiple_B2L (ijk_box, icomp, Aztec.update_index[i]);
	}

      for (i = 0; i < Aztec.data_org[AZ_N_external]; i++)
	{
	  node_to_ijk (Aztec.external[i] / Nunk_per_node, ijk);
	  icomp = Aztec.external[i] - Nunk_per_node * ijk_to_node (ijk);
	  ijk_to_ijk_box (ijk, ijk_box);
	  inode_box = ijk_box_to_node_box (ijk_box);
	  B2L_unknowns[inode_box * Nunk_per_node + icomp] =
	    Aztec.extern_index[i];
	  ...
	  if (Non_unique_G2B[3])
	    set_multiple_B2L (ijk_box, icomp, Aztec.extern_index[i]);
	}

      max_ints = AZ_gmax_int (Aztec.N_update, Aztec.proc_config);
      max_exts =
	AZ_gmax_int (Aztec.data_org[AZ_N_external], Aztec.proc_config);
      min_ints = AZ_gmin_int (Aztec.N_update, Aztec.proc_config);
      min_exts =
	AZ_gmin_int (Aztec.data_org[AZ_N_external], Aztec.proc_config);
...
/* Call LOCA continuation library if it is requested and compiled in */
	...
	iter +=
	  newton_solver (x, x2, fill_time, NULL, max_iter, &t_solve_max);
...
int
newton_solver (double *x, double *x2, double *fill_time, void *con_ptr,
	       int max_iter, double *t_solve_max_ptr)
{
  ...
  double node_pos[3], residmax;
  int nodesave;
  int ijk[3], icomp, inode;
  ...
  do
	...
	resid_tmp[i] = 0.0;
      ...
      for (i = 0; i < Aztec.nonzeros + 1; i++)
	Aztec.val[i] = 0.0;

      t1 = MPI_Wtime ();
      if (Sten_Type[POLYMER_CR])
	fill_resid_and_matrix_P (x, resid_tmp, NULL, fill_time,
				 Matrix_fill_flag, iter, FALSE);
      else
	fill_resid_and_matrix (x, resid_tmp, NULL, fill_time,
			       Matrix_fill_flag, iter, FALSE);
      t2 = MPI_Wtime () - t1;
...
/*       node_to_ijk(Aztec.update[i]/Nunk_per_node,ijk);
	  ...
	  if (resid[i] > residmax)
	    {
	      residmax = resid[i];
	      nodesave = L2G_node[i / Nunk_per_node];
	      node_to_position (L2G_node[i / Nunk_per_node], node_pos);
	    }
...
/*#define NUMERICAL_JACOBIAN*/
	...
	delta_x[i] = 0.0;
      ...
      AZ_solve (delta_x, resid, Aztec.options, Aztec.params, NULL,
		Aztec.bindx, NULL, NULL, NULL, Aztec.val, Aztec.data_org,
		Aztec.status, Aztec.proc_config);
      ...
      else if (Lbinodal)
	{
	  ...
	  for (i = 0; i < Nnodes_per_proc; i++)
		...
		{
		  if (icomp == 0)
		    resid_tmp[B2L_unknowns
			      [L2B_node[i] * Nunk_per_node + icomp]] =
		      dmu_drb *
		      Nel_hit2[0][Aztec.
				  update_index[icomp +
					       Nunk_per_node * i]] * Vol_el /
		      ((double) Nnodes_per_el_V);
		  else
		    resid_tmp[B2L_unknowns
			      [L2B_node[i] * Nunk_per_node + icomp]] = 0.0;
		}
	    }

	  AZ_solve (b, resid_tmp, Aztec.options, Aztec.params, NULL,
		    Aztec.bindx, NULL, NULL, NULL, Aztec.val, Aztec.data_org,
		    Aztec.status, Aztec.proc_config);
	  ...
	  if (Sten_Type[POLYMER_CR])
	    fill_resid_and_matrix_P (x2, resid_tmp, NULL, fill_time,
				     Matrix_fill_flag, iter, FALSE);
	  else
	    fill_resid_and_matrix (x2, resid_tmp, NULL, fill_time,
				   Matrix_fill_flag, iter, FALSE);
	  l2_resid = 0.0;
	  for (i = 0; i < Aztec.N_update; i++)
	    {
	  ...
	  for (i = 0; i < Aztec.N_update; i++)
	    f2[i] =
	      resid_tmp[i] * Nel_hit2[0][Aztec.update_index[i]] * Vol_el /
	      ((double) Nnodes_per_el_V);
	  for (i = 0; i < Aztec.N_update; i++)
	    resid_tmp[i] *= -1.0;

	  AZ_solve (c, resid_tmp, Aztec.options, Aztec.params, NULL,
		    Aztec.bindx, NULL, NULL, NULL, Aztec.val, Aztec.data_org,
		    Aztec.status, Aztec.proc_config);
	  ...
	  AZ_solve (d, resid_tmp, Aztec.options, Aztec.params, NULL,
		    Aztec.bindx, NULL, NULL, NULL, Aztec.val, Aztec.data_org,
		    Aztec.status, Aztec.proc_config);

	  g =
	    calc_free_energy (NULL, x, 1.0, 1.0,
			      FALSE) - calc_free_energy (NULL, x2, 1.0, 1.0,
							 FALSE);
	  ads =
	    -dmu_drb * (calc_adsorption (NULL, x, 1.0, 1.0) -
...
double g1, g2, ee=1.0e-8, bt, bh, ba;
    ...
    if (iter==2) exit(-1);
	  ...
	  del_rb0 =
	    -(g + dotp (f1, a) - dotp (f2, c)) / (ads + dotp (f1, b) -
						  dotp (f2, d));

	  if ((Imain_loop == 0 && iter < 3) /*|| fabs(g)<1.e-5 */ )
	    del_rb0 = 0.0;
	  if ((iter < 3) /*|| fabs(g)<1.e-5 */ )
	    del_rb0 = 0.0;

	  if (Rho_b[0] + del_rb0 < 0.75 * Rho_b[0])
	    del_rb0 = -0.25 * Rho_b[0];
...
/****************************************************************************/
int
update_solution (int iter, double *x, double *delta_x)
/*
 ...
 */
{
  int loc_i, loc_inode, icomp, itmp;
  ...
  for (icomp = 0; icomp < Nunk_per_node; icomp++)
    {
      if ((Type_poly > -1 && icomp < 2 * Ncomp + Ngeqn_tot)
	  || (icomp < Ncomp && Ipot_ff_n != IDEAL_GAS))
	{
	  for (loc_inode = 0; loc_inode < Nnodes_per_proc; loc_inode++)
	    {
		...
		frac = 1.0;
	      if (Sten_Type[U_ATTRACT] && icomp < Ncomp
		  && !Sten_Type[POLYMER_CR]
		  && x[loc_i] <= Rho_b[icomp] * exp (-VEXT_MAX))
		frac = 1.0;
	  ...
	  if (Type_poly >= 0)
	    {
	      if (icomp < Ncomp)
		itmp = icomp;
	      else if (icomp < 2 * Ncomp)
		itmp = icomp - Ncomp;
	      else if (icomp < 2 * Ncomp + Ngeqn_tot)
		{
		  itmp =
		    Type_mer[Unk_to_Poly[icomp - 2 * Ncomp]][Unk_to_Seg
							     [icomp -
							      2 * Ncomp]];
		}
	    }

	  if (Type_poly == -1 && icomp < Ncomp
	      && Zero_density_TF[L2B_node[loc_inode]][icomp])
	    x[loc_i] = 0.0;
	  else if (Type_poly >= 0 && icomp < 2 * Ncomp + Ngeqn_tot
		   && Zero_density_TF[L2B_node[loc_inode]][itmp])
	    x[loc_i] = 0.0;
	  else
	    {

	      temp =
		delta_x[loc_i] / (Newton_rel_tol * x[loc_i] + Newton_abs_tol);
	      l2_norm += temp * temp;
	      ...
	      if (((Type_poly == -1 && icomp < Ncomp)
		   || (Type_poly >= 0 && icomp < 2 * Ncomp + Ngeqn_tot))
		  && x[loc_i] + delta_x[loc_i] <= 1.e-15)
		{
		  x[loc_i] = x[loc_i] * 0.1;	/* temporary */
		}
	      else if (Matrix_fill_flag >= 3 && Type_coul == -1
		       && icomp == Ncomp && x[loc_i] >= 1.0
		       && Ipot_ff_n != IDEAL_GAS)
		{
	      ...
	      else
		{
	      ...
	      if (Type_poly > -1 && icomp > Ncomp - 1 && icomp < 2 * Ncomp
		  && x[loc_i] > Rho_max)
		x[loc_i] = Rho_max;
	    }
	}
...
/****************************************************************************/
static void
set_multiple_B2L (int *ijk_box, int icomp, int loc_i)
{
...
void
full_to_msr (double **a, int *bindx, double *val, int maxrow, int maxcol)
...
#ifdef NUMERICAL_JACOBIAN
static void
do_numerical_jacobian (double *x, double *resid, double *resid_tmp)
...
/* row, column, and value.                                                */
{
  ...
  if (full == NULL)
    {
      printf ("Not enough memory for full numerical jacobian\n");
      exit (-1);
    }
  for (i = 0; i < N; i++)
    {
	...
	resid_tmp[j] = 0.0;
      if (Sten_Type[POLYMER_CR])
	{
	  fill_resid_and_matrix_P (x, resid_tmp, NULL, NULL, Matrix_fill_flag,
				   1, TRUE);
	}
      else
	{
	  fill_resid_and_matrix (x, resid_tmp, NULL, NULL, Matrix_fill_flag,
				 1, TRUE);
	}
	...
	{
	  if (Iwrite != NO_SCREEN)
	    printf ("Proc: %d :: ith row=%d, out of %d rows.\n", Proc, i,
		    N - 1);
	  count = 0;
	}
      count++;
  ...
  if (Iwrite != NO_SCREEN)
    printf ("Proc: %d finished computing numerical jacobian !!\n", Proc);
  ...
  if (Num_Proc > 1)
    MPI_Barrier (MPI_COMM_WORLD);
  if (Iwrite != NO_SCREEN)
    printf ("Proc: %d KILLING CODE AT END OF NUMERICAL JACOBIAN PRINT\n",
	    Proc);
  exit (-1);
}
#endif
