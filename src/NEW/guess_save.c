       ...
       for (i=0;i<Nsteps;i++){
           if (nodepos[Orientation_step[i]]>=Xstart_step[i] &&
	           ...
	           loc_i = Aztec.update_index[icomp+Ncomp+Nunk_per_node*loc_inode];
       		   if (!Zero_density_TF[inode_box][i]) x[loc_i]= Rho_step[icomp][i];
		   else x[loc_i]=0.0;
                }
            }
       }
  ...
  for (loc_inode=0; loc_inode<Nnodes_per_proc; loc_inode++){
     ...
     for (icomp=0; icomp<Ncomp; icomp++) {
        ...
        if (!Zero_density_TF[inode_box][icomp]){
           ...
           if (Lsteady_state)
             x[loc_i] = Rho_b_LBB[icomp] + (Rho_b_RTF[icomp]-Rho_b_LBB[icomp])*
  ...
  for (loc_inode=0; loc_inode<Nnodes_per_proc; loc_inode++){
     ...
     for (icomp=0; icomp<Ncomp; icomp++){
       ...
       if (!Zero_density_TF[inode_box][icomp]){
        ...
        x[loc_i] = Betamu_LBB[icomp] + (Betamu_RTF[icomp]-Betamu_LBB[icomp])*
                                                    x_dist/Size_x[Grad_dim];
       }
