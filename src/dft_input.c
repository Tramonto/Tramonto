...
void
read_input_file (char *input_file, char *output_file1)
 ...
 */
{
  ...
  int isten, icomp, jcomp, iwall, iwall_save, iwall_type, idim,
    ...
    ***pol_sym_tmp, dim_tmp, Lauto_center, Lauto_size, Lcompare_fastram,
    jmin = 0, jmax = 0, lzeros, latoms, ltrues, jwall_type;
  ...
  if (Proc == 0)
      ...
      for (idim = 0; idim < Ndim; ++idim)
	{
      ...
      for (idim = Ndim; idim < NDIM_MAX; idim++)
	Size_x[idim] = 0.0;
    }
 ...
 * if (Proc==0) fprintf(fp2,"%d",Lmesh_refine);
				 */ Lmesh_refine = FALSE;
				/*parameter not active currently - auto mesh refine */
  ...
  for (idim = 0; idim < Ndim; ++idim)
	...
	{
	  printf
	    ("%s: ERROR: Both BC in dimension %d must be periodic if one is\n",
	     yo, idim);
	  exit (-1);
	}
    }
  ...
  if (Type_func == 0 || Type_func == 1)
    {
      Sten_Type[0] = Sten_Type[1] = TRUE;
    }
  else if (Type_func > 1 || Type_func < -1)
    {
      if (Proc == 0)
	printf ("ERROR Type_hs out of range - should be -1,0,1\n");
      exit (-1);
    }
  ...
  if (Type_attr == 0)
    ...
    {
      if (Proc == 0)
	printf ("ERROR Type_attr out of range - should be -1 or 0\n");
      exit (-1);
    }
  ...
  if (Type_coul == 1)
    ...
    {
      if (Proc == 0)
	printf ("ERROR Type_coul out of range - should be -1,0,1\n");
      exit (-1);
    }
  ...
  if (Type_poly != -1)
    {
      Sten_Type[0] = TRUE;
      Sten_Type[2] = FALSE;	/* attractions handled differently for polymers */
    }

  /* ALF: modify for new SCF Type_poly == 3 */
  if (Type_poly == 0 || Type_poly == 1 || Type_poly == 2)
    Sten_Type[4] = TRUE;
  else if (Type_poly == 3 || Type_poly == 4 || Type_poly == 5)
    Sten_Type[4] = 2;

  else if (Type_poly > 5 || Type_poly < -1)
    {
      if (Proc == 0)
	printf ("ERROR Type_poly out of range - should be -1,0,1,2,3\n");
      exit (-1);
    }
...
/* check if we are trying to compare with FasTram -- for hard systems */
  ...
  if (Type_func == -1 && Type_poly == -1)
    Ipot_ff_n = 0;		/* Ideal Gas */
  else if (Type_func >= 0 && Type_attr == -1)
    Ipot_ff_n = 1;		/* Hard Sphere Fluid */
  else if (Type_func >= 0 && Type_attr == 0)
    Ipot_ff_n = 2;		/* Lennard-Jones Fluid */
  else if (Type_poly >= -1 && Type_attr == 0)
    Ipot_ff_n = 2;		/* Lennard-Jones Chains */
  else if (Type_poly >= -1 && Type_attr == -1)
    Ipot_ff_n = 1;		/* Hard Chain Fluid */
  else
    {
      printf
	("ERROR WITH Type_func and Type_attr selections and conversion to Ipot_ff_n parameter \n");
      exit (-1);
    }
  ...
  if (Type_coul >= 0)
    Ipot_ff_c = 1;		/* Coulombic Fluid */
  else
    Ipot_ff_c = 0;		/* Neutral Fluid */
  ...
  if (Proc == 0)
    {
      read_junk (fp, fp2);
      fscanf (fp, "%d  %d  %d  %d  %d", &Nwall_type, &Nwall, &Nlink,
	      &Lauto_center, &Lauto_size);
      fprintf (fp2, "%d  %d  %d  %d  %d", Nwall_type, Nwall, Nlink,
	       Lauto_center, Lauto_size);
    }
  ...
  if (Nwall_type != 0)
      ...
      if (Proc == 0)
	{
	  ...
	  for (iwall = 0; iwall < Nwall; iwall++)
	      ...
	      for (idim = 0; idim < Ndim; idim++)
		{
		  ...
		  if (fabs (WallPos[dim_tmp][iwall] + 9999.0) < 1.e-6)
		    {		/*random coordinate placement */
		      irand = random ();
		      irand_range = POW_INT (2, 31) - 1;
		      WallPos[dim_tmp][iwall] =
			Size_x[idim] * (-0.5 +
					(((double) irand) /
					 ((double) irand_range)));
		  ...
		  if (Length_ref > 0.0)
		    WallPos[dim_tmp][iwall] /= Length_ref;
		  if (WallPos[dim_tmp][iwall] < minpos[dim_tmp])
		    minpos[dim_tmp] = WallPos[dim_tmp][iwall];
		  if (WallPos[dim_tmp][iwall] > maxpos[dim_tmp])
		    maxpos[dim_tmp] = WallPos[dim_tmp][iwall];
		}
      ...
      MPI_Bcast (WallPos, NDIM_MAX * NWALL_MAX, MPI_DOUBLE, 0,
		 MPI_COMM_WORLD);
	...
	{
	  printf
	    ("Check Nlink in dft_input.dat: %d and assignments in dft_surfaces.dat: %d\n",
	     Nlink, nlink_chk);
	  exit (-1);
	}
	...
	for (iwall_type = 0; iwall_type < Nwall_type; ++iwall_type)
	  {
	    ...
	    if (Surface_type[iwall_type] == point_surface)
	      WallParam[iwall_type] = Esize_x[0];
	  }
      else
	fprintf (fp2, "not relevant");
    }
	...
	for (iwall_type = 0; iwall_type < Nwall_type; ++iwall_type)
	  {
	    ...
	    if (Ipot_wf_n[iwall_type] == VEXT_HARD)
	      Lhard_surf = TRUE;
	...
	for (iwall_type = 0; iwall_type < Nwall_type; ++iwall_type)
	  {
		...
		if (iwall_type == 0 && jwall_type == 0)
		  {
		    if (Ipot_ww_n[iwall_type][jwall_type] == -2)
		      lzeros = TRUE;
		    else if (Ipot_ww_n[iwall_type][jwall_type] == -1)
		      latoms = TRUE;
		  }
		if (lzeros)
		  Ipot_ww_n[iwall_type][jwall_type] = 0;
		else if (latoms)
		  Ipot_ww_n[iwall_type][jwall_type] = 1;
	      }
	  }
      else
	fprintf (fp2, "not relevant");
...
/* New code for interaction potential parameters */
      ...
      if (Type_coul == 2)
	{
	  for (icomp = 0; icomp < Ncomp; ++icomp)
		...
		fprintf (fp2, "%f  ", Pol[icomp]);
	      if (Pol[icomp] != 0.0)
		Lpolarize[icomp] = TRUE;
	      else
		Lpolarize[icomp] = FALSE;
	    }
	}
      else if (Proc == 0)
	fprintf (fp2, "not relevent");
    }
  ...
  if (Mix_type == 1)
    MPI_Bcast (Sigma_ww, NWALL_MAX_TYPE * NWALL_MAX_TYPE, MPI_DOUBLE, 0,
	       MPI_COMM_WORLD);
  else
  ...
  if (Mix_type == 1)
    MPI_Bcast (Eps_ww, NWALL_MAX_TYPE * NWALL_MAX_TYPE, MPI_DOUBLE, 0,
	       MPI_COMM_WORLD);
  else
	  ...
	  for (j = jmin; j < jmax; j++)
	    {
	      ...
	      if (Length_ref > 0.0)
		Cut_ww[i][j] /= Length_ref;
	    }
	}
    }
  MPI_Bcast (Cut_ww, NWALL_MAX_TYPE * NWALL_MAX_TYPE, MPI_DOUBLE, 0,
	     MPI_COMM_WORLD);
      ...
      if (Proc == 0)
	{
	  ...
	  for (i = 0; i < Ncomp; i++)
	      ...
	      for (j = 0; j < Nwall_type; j++)
		{
		  ...
		  if (Length_ref > 0.0)
		    Cut_wf[i][j] /= Length_ref;
		}
	    }
	}
      MPI_Bcast (Sigma_wf, NCOMP_MAX * NWALL_MAX_TYPE, MPI_DOUBLE, 0,
		 MPI_COMM_WORLD);
      MPI_Bcast (Eps_wf, NCOMP_MAX * NWALL_MAX_TYPE, MPI_DOUBLE, 0,
		 MPI_COMM_WORLD);
      MPI_Bcast (Cut_wf, NCOMP_MAX * NWALL_MAX_TYPE, MPI_DOUBLE, 0,
		 MPI_COMM_WORLD);
    }
  else
    {
  ...
  if (Sten_Type[POLYMER_CR])
      ...
      if (Proc == 0)
	{
	  ...
	  for (i = 0; i < Npol_comp; ++i)
		...
		{
		  if (Proc == 0)
		    printf ("Must increase NBLOCK_MAX");
		  Nblock[i] = NBLOCK_MAX;
		}
	    }
	}
      ...
      if (Proc == 0)
	{
	  ...
	  for (pol_number = 0; pol_number < Npol_comp; ++pol_number)
	      ...
	      for (i = 0; i < Nblock[pol_number]; ++i)
		{
		  ...
		  if (block_type[i] > Ntype_mer)
		    Ntype_mer = block_type[i];
		}
	    }
	  Ntype_mer++;
      ...
      for (pol_number = 0; pol_number < Npol_comp; pol_number++)
	{
	  ...
	  if (Nmer[pol_number] > nmer_max)
	    nmer_max = Nmer[pol_number];
	}
      ...
      Bonds =
	(int ***) array_alloc (3, Npol_comp, nmer_max, NBOND_MAX,
			       sizeof (int));
      pol_sym_tmp =
	(int ***) array_alloc (3, Npol_comp, nmer_max, NBOND_MAX,
			       sizeof (int));
      ...
      for (pol_number = 0; pol_number < Npol_comp; ++pol_number)
	{
	  ...
	  for (iseg = 0; iseg < Nmer[pol_number]; iseg++)
	    {
	      if (Proc == 0)
		fscanf (fp4, "%d", &Nbond[pol_number][iseg]);
	      MPI_Bcast (&Nbond[pol_number][iseg], 1, MPI_INT, 0,
			 MPI_COMM_WORLD);
	      for (ibond = 0; ibond < Nbond[pol_number][iseg]; ibond++)
		{
		  if (Proc == 0)
		    {
		      fscanf (fp4, "%d  %d", &Bonds[pol_number][iseg][ibond],
			      &pol_sym_tmp[pol_number][iseg][ibond]);
		      fprintf (fp2, "%d  ", Bonds[pol_number][iseg][ibond]);
		    }
		  MPI_Bcast (&Bonds[pol_number][iseg][ibond], 1, MPI_INT, 0,
			     MPI_COMM_WORLD);
		  MPI_Bcast (&pol_sym_tmp[pol_number][iseg][ibond], 1,
	      ...
	      nbond_tot[pol_number] += Nbond[pol_number][iseg];
	    }
	}
      if (Proc == 0)
	fclose (fp4);
      ...
      for (pol_number = 0; pol_number < Npol_comp; ++pol_number)
	{
      ...
      if (Proc == 0)
	printf ("The total number of g equations will be %d\n", Ngeqn_tot);
      for (pol_number = 0; pol_number < Npol_comp; ++pol_number)
	if (Proc == 0)
	  printf ("The start unknown for polymer %d is %d \n", pol_number,
		  Geqn_start[pol_number]);
      ...
      if (Proc == 0)
	{
      ...
      MPI_Bcast (Cr_rad_hs, NCOMP_MAX * NCOMP_MAX, MPI_DOUBLE, 0,
		 MPI_COMM_WORLD);
...
*/
  ...
  if (Nwall_type > 0)
    Vext_membrane =
      (double **) array_alloc (2, Nwall_type, Ncomp, sizeof (double));
  if (Proc == 0)
    {
      read_junk (fp, fp2);
      for (i = 0; i < Nwall_type; i++)
      ...
      for (i = 0; i < Nwall_type; i++)
	{
	  for (j = 0; j < Ncomp; j++)
	    if (!Lsemiperm[i][j])
	      Vext_membrane[i][j] = VEXT_MAX;
	}
    }
  if (Nwall_type > 0)
    MPI_Bcast (*Vext_membrane, Nwall_type * Ncomp, MPI_DOUBLE, 0,
	       MPI_COMM_WORLD);
  ...
  if (Proc == 0)
    {
      read_junk (fp, fp2);
      if (!Sten_Type[POLYMER_CR] || Ntype_mer == 1)
	for (icomp = 0; icomp < Ncomp; ++icomp)
	  {
	    fscanf (fp, "%lf", &Rho_b[icomp]);
	    fprintf (fp2, "%f  ", Rho_b[icomp]);
	    if (Density_ref > 0.0)
	      Rho_b[icomp] /= Density_ref;
	  }
      else
	{
	  ...
	  for (i = 0; i < Ntype_mer; i++)
		...
		{
		  Rho_b[i] +=
		    (double) Nmer_t[j][i] * rho_tmp[j] / (double) Nmer[j];
		}
  ...
  if (Type_coul != -1)
	  ...
	  for (iwall_type = 0; iwall_type < Nwall_type; ++iwall_type)
	    {
	      ...
	      if (Type_bc_elec[iwall_type] != 0)
		Ipot_wf_c = COULOMB;
	    }
	}
      if (Nwall_type > 0)
	MPI_Bcast (Type_bc_elec, NWALL_MAX_TYPE, MPI_INT, 0, MPI_COMM_WORLD);
      ...
      if (Proc == 0)
	{
      ...
      ncharge = 0;
      if (Nlocal_charge > 0)
	ncharge = Nlocal_charge;
      else if (Nlocal_charge < 0)
	ncharge = 2;
      ...
      else
	{
	  ...
	  Charge_x =
	    (double **) array_alloc (2, Ndim, ncharge, sizeof (double));
	  if (Proc == 0)
	    {
	      for (i = 0; i < ncharge; i++)
		{
	  ...
	  MPI_Bcast (*Charge_x, Ndim * ncharge, MPI_DOUBLE, 0,
		     MPI_COMM_WORLD);
	}
      logical = FALSE;
      for (iwall_type = 0; iwall_type < Nwall_type; iwall_type++)
	if (Type_bc_elec[iwall_type] == ATOMIC_CHARGE)
	  logical = TRUE;
	...
	fprintf (fp2, "not relevant  ncharge=0 and walls are not atoms");
    }
  else
    {
      if (Proc == 0)
  ...
  if (Ipot_ff_c == COULOMB)
	...
	{
	  Temp_elec =
	    4.0 * PI * KBOLTZ * Temp * Dielec_ref * EPSILON_0 * Length_ref *
	    1.e-10 / (E_CONST * E_CONST);
	  if (Proc == 0)
	    printf
	      ("\t WARNING :: CHECK THAT Length_ref WAS GIVEN IN ANGSTROM UNITS ... CHARGED SYSTEM\n");
	}
      else			/* make some assumptions */
	Temp_elec =
	  4 * PI * KBOLTZ * 298.0 * KAPPA_H2O * EPSILON_0 * 4.25e-10 /
	  (E_CONST * E_CONST);

      if (Proc == 0)
	printf ("\t Temp_elec=%9.6f\n", Temp_elec);
      ...
      else
	{
	  if (Proc == 0)
	    {
	      fscanf (fp, "%lf %lf %lf", &Dielec_bulk, &Dielec_pore,
		      &Dielec_X);
	      fprintf (fp2, "%f  %f  %f ", Dielec_bulk, Dielec_pore,
		       Dielec_X);
	      if (Dielec_ref > 0.0)
		Dielec_bulk /= Dielec_ref;
	      if (Dielec_ref > 0.0)
	...
	{
	  Dielec_wall =
	    (double *) array_alloc (1, Nwall_type, sizeof (double));
	  if (Proc == 0)
	    {
	      for (iwall_type = 0; iwall_type < Nwall_type; iwall_type++)
		{
		  ...
		  if (Dielec_ref > 0.0)
		    Dielec_wall[iwall_type] /= Dielec_ref;
		}
	    }
      ...
      if (Proc == 0)
	{
	  read_junk (fp, fp2);
	  fprintf (fp2,
		   "CHARGED FLUID (DIELECTRIC) INPUT NOT RELEVENT FOR THIS RUN\n");
	  fprintf (fp2, "not relevant");
	  for (i = 0; i < 2; i++)
	    {
	      read_junk (fp, fp2);
  ...
  if (Lsteady_state || Sten_Type[POLYMER_CR])
      ...
      if (Proc == 0)
	{
	  ...
	  for (i = 0; i < Nseg_IC; i++)
	    {
	      fscanf (fp, "%lf  %lf  %lf", &Pore_rad_L_IC[i],
		      &Pore_rad_R_IC[i], &Lseg_IC[i]);
	      fprintf (fp2, "%f  %f  %f  ", Pore_rad_L_IC[i],
		       Pore_rad_R_IC[i], Lseg_IC[i]);
	      if (Length_ref > 0.0)
		{
  ...
  if (Iguess1 == STEP_PROFILE
      || (Iguess1 >= CHOP_RHO && Iguess1 <= CHOP_RHO_STEP))
    {
      if (Proc == 0)
	{
	  read_junk (fp, fp2);
      ...
      if (Proc == 0)
	{
      ...
      MPI_Bcast (Rho_step, NSTEPS_MAX * NCOMP_MAX, MPI_DOUBLE, 0,
		 MPI_COMM_WORLD);
    }
  else
    {
  ...
  if (Proc == 0)
    {
      read_junk (fp, fp2);
      fscanf (fp, "%d  %d  %d %d ", &Lper_area, &Lcount_reflect, &Lprint_gofr,
	      &Lprint_pmf);
      fprintf (fp2, "%d  %d  %d  %d", Lper_area, Lcount_reflect, Lprint_gofr,
	       Lprint_pmf);
    }
  ...
  if (Mesh_coarsening == 2 && Lauto_size && Proc == 0)
	...
	{
	  Size_x[idim] =
	    maxpos[idim] - minpos[idim] + 2.0 * (Rmax_zone[0] +
						 Sigma_ff[0][0]);
	  printf ("  %9.6f\n", Size_x[idim]);
	}
    }
  ...
  if (Proc == 0)
    {
      read_junk (fp, fp2);
      fscanf (fp, "%d", &itmp);
      if (itmp == CONT_SCALE_RHO || itmp == CONT_SCALE_EPSW
	  || itmp == CONT_SCALE_EPSWF || itmp == CONT_SCALE_EPSFF
	  || itmp == CONT_SCALE_CHG)
	{
  ...
  if (Proc == 0)
    fprintf (fp2, "%d  %g", Loca.cont_type1, Scale_fac);
  ...
  if (Proc == 0 && Loca.method != -1)
      ...
      if (Loca.cont_type1 == CONT_TEMP && Temp > 0.0)
	{
      ...
      else if (Loca.cont_type1 == CONT_RHO_0
	       || Loca.cont_type1 == CONT_RHO_ALL
	       || Loca.cont_type1 == CONT_LOG_RHO_0
	       || Loca.cont_type1 == CONT_LOG_RHO_ALL
	       || Loca.cont_type1 == CONT_SCALE_RHO)
	{
	  if (Type_poly >= 0)
	    {
	      printf
		("WARNING: density continuation may be incorrect with polymer code\n");
	      printf
		("...... you really need a new c(r) for the hard chain part\n");
	      printf ("...... for each new density.\n");
	    }
	  if (Density_ref > 0.0 && Loca.cont_type1 != CONT_SCALE_RHO)
	    dtmp /= Density_ref;
	}
      else if (Loca.cont_type1 == CONT_EPSW_0
	       || Loca.cont_type1 == CONT_EPSW_ALL
	       || Loca.cont_type1 == CONT_EPSWF00
	       || Loca.cont_type1 == CONT_EPSWF_ALL_0
	       || Loca.cont_type1 == CONT_EPSFF_00
	       || Loca.cont_type1 == CONT_EPSFF_ALL)
	{
	  if (Temp > 0.0)
	    dtmp /= Temp;
	}
	...
	{
	  printf
	    ("ERROR: Can't do continuation in Eps_wf when the Mix_type is 0\n");
	  exit (-1);
	}
    }
  ...
  if (Loca.cont_type1 == CONT_SCALE_RHO)
    {
      for (icomp = 0; icomp < Ncomp; icomp++)
	Rho_b[icomp] *= Scale_fac;
    }
  else if (Loca.cont_type1 == CONT_SCALE_EPSW)
    {
      if (Mix_type == 0)
	for (iwall_type = 0; iwall_type < Nwall_type; iwall_type++)
	  Eps_w[iwall_type] *= Scale_fac;
      else
	{
	  for (i = 0; i < Nwall_type; i++)
	    {
	      for (j = 0; j < Nwall_type; j++)
		Eps_ww[i][j] *= Scale_fac;
	    }
	}
    }
	...
	{
	  for (iwall_type = 0; iwall_type < Nwall_type; iwall_type++)
	    Eps_wf[icomp][iwall_type] *= Scale_fac;
	}
    }
  ...
  if (Proc == 0)
    fprintf (fp2, "%d  %g", Loca.cont_type2, Scale_fac);
...
error_check (void)
{
  ...
  if (Type_attr > 0 && Type_attr < -1)
	...
	("\nSorry, your choice for the type of attractions is not yet available\n");
      printf ("Type_attr: %d\n", Type_attr);
      printf
	("Reset Type_attr to 0 for the strict mean field approximation\n");
      exit (-1);
    }
...
/*             exit(-1);*/
  ...
  if (Iguess1 == 6 && Iliq_vap != 3 && Lsteady_state == FALSE
