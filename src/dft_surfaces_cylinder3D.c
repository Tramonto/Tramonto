...
/****************************************************************************/
void surface_cylinder3D_inSurfaceTest(int iwall,int iwall_type,int loc_inode,int flag_setup_Xwall,
                                  ...
                                  int *logical_inwall, int *logical_nearWallDielec)
{
  ...
  *logical_inwall=FALSE;
  ...
  radius=sgeom_iw->radius+dist_adjustments-roff;   /* apply periodic functions, roughness, linear functions in radial direction only */
  ...
  if (Ipot_ff_c==COULOMB && r12 < radius+Dielec_X && x12<halflength+Dielec_X) *logical_nearWallDielec  = TRUE;
   ...
   if (x12<halflength) *delr=r12-radius;
