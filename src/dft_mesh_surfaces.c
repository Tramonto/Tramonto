...
void els_planar (int, int, int, int **, double *, int **, int ***, int *,
		 int ***, double **);
void els_finite_planar (int, int, int, int **, double *, int **, int ***,
			int *, int ***, double **);
void els_spheres (int, int, int, int **, double *, int **, int ***, int *,
		  int ***, double **);
void els_cyls_3D (int, int, int, int **, double *, int **, int ***, int *,
		  int ***, double **);
void els_cyls_cos_3D (int, int, int, int **, double *, int **, int ***, int *,
		      int ***, double **);
void els_atomic_centers (int, int, int, int **, double *, int **, int ***,
			 int *, int ***, double **);
void els_cyl_pores (int, int, int, int **, double *, int **, int ***, int *,
		    int ***, double **);
void els_slit_pore_2D (int, int, int, int **, double *, int **, int ***,
		       int *, int ***, double **);
void els_cyl_pore_3D (int, int, int, int **, double *, int **, int ***, int *,
		      int ***, double **);
void els_cone_pore_2D (int, int, int, int **, double *, int **, int ***,
		       int *, int ***, double **);
void els_cone_pore_3D (int, int, int, int **, double *, int **, int ***,
		       int *, int ***, double **);
void flag_wall_el (int, int, int, int, int **, int **, int ***, int ***);
void find_wall_images (int, int *, double **, double *);

void
setup_surface (FILE * fp2, int *nelems_f,
	       int **nelems_w_per_w,
	       int **elems_f,
	       ...
	       int *fast_fill_elem_TF, int ***el_type)
{
  ...
  int itype, iwall, ilist, iel_box, izone, **L_wall, idim, loc_inode,
    real_wall, *imagetype, flag;
  double *x_min;
  ...
  Touch_domain_boundary =
    (int ****) array_alloc (4, Nwall, Nlists_HW, Ndim, 2, sizeof (int));
  flag = FALSE;
  ...
  if (flag)
    {
      X_wall =
	(double **) array_alloc (2, Nnodes_per_proc, Nwall, sizeof (double));
      X_wall2 =
	(double **) array_alloc (2, Nnodes_per_proc, Nwall, sizeof (double));
      for (loc_inode = 0; loc_inode < Nnodes_per_proc; loc_inode++)
	{
	  for (iwall = 0; iwall < Nwall; iwall++)
	    X_wall[loc_inode][iwall] = -999.0;
	  for (iwall = 0; iwall < Nwall; iwall++)
	    X_wall2[loc_inode][iwall] = -999.0;
	}
    }
  ...
  for (iel_box = 0; iel_box < Nelements_box; iel_box++)
    {
      ...
      if (Coarser_jac != 5)
	elem_zones[iel_box] = Nzone - 1;
      else
	elem_zones[iel_box] = Nzone - 2;
      x_min[iel_box] = 1000.;
      for (ilist = 0; ilist < Nlists_HW; ilist++)
	{
	  ...
	  for (iwall = 0; iwall < Nwall; iwall++)
	    el_type[iwall][ilist][iel_box] = FLUID_EL;
	}
    }
	  ...
	  if (Surface_type[itype] == smooth_planar_wall)
	      ...
	      for (idim = 0; idim < Ndim; idim++)
		{
		  if (idim != Orientation[itype])
		    Xtest_reflect_TF[Link[iwall]][idim] = FALSE;
		}
	    }
	  if (Surface_type[itype] == finite_cyl_3D
	      || Surface_type[itype] == cyl_periodic_3D)
	    {
  ...
  image_pos = (double **) array_alloc (2, Nwall * POW_INT (3, Ndim),
				       Ndim, sizeof (double));
  imagetype =
    (int *) array_alloc (1, Nwall * POW_INT (3, Ndim), sizeof (double));
  ...
  for (iwall = 0; iwall < nwall_max; iwall++)
	...
	case smooth_planar_wall:
	  els_planar (iwall, real_wall, itype, L_wall, x_min,
		      ...
		      fast_fill_elem_TF, el_type, image_pos);
	  break;
	...
	{
	  printf
	    ("Proc: %d in setup surfaces:: iwall=%d of nwall_max=%d Nnodes_wall_box: %d \n",
	...
	{
	  printf
	    ("Proc: %d is done with setup surfaces:: iwall=%d of nwall_max=%d Nnodes_wall_box=%d\n",
	     Proc, iwall, nwall_max, Nnodes_wall_box);
	}
      else
	icount++;
 ...
 */
void
find_wall_images (int idim, int *image, double **image_pos, double *pos)
{
  int iside, jdim;
  ...
  for (iside = 0; iside < 2; iside++)
      ...
      if (Type_bc[idim][iside] == PERIODIC || Type_bc[idim][iside] == REFLECT)
	{
	  ...
	  if (fabs (shift) > 0.000000001)
		...
		{
		  if (jdim == idim)
		    node_image[jdim] = pos[jdim] + sign * shift;
		  else
		    node_image[jdim] = pos[jdim];
		}
	      ;
	      for (jdim = 0; jdim < Ndim; jdim++)
...
void
flag_wall_el (int inode, int ilist, int iwall, int iel_box, int **L_wall,
	      int **nelems_w_per_w, int ***elems_w_per_w, int ***el_type)
{
  int ijk[3], idim, nadd, i, new_wall, new_entry;
  int imax[3], ix[3], iw, ijk_box[3], inode_box, inode_tmp_box,
    ijk_tmp_box[3], index;
  imax[0] = imax[1] = imax[2] = 1;

  new_wall = TRUE;
  for (i = 0; i < Nwall_owners[ilist][iel_box]; i++)
    if (Link[iwall] == Link[i])
      new_wall = FALSE;
  if (new_wall)
    Wall_owners[ilist][iel_box][Nwall_owners[ilist][iel_box]++] = Link[iwall];
      ...
      for (ix[2] = 0; ix[2] < imax[2]; ix[2]++)
	{
	  for (ix[1] = 0; ix[1] < imax[1]; ix[1]++)
	      ...
	      for (ix[0] = 0; ix[0] < imax[0]; ix[0]++)
		{
		  ...
		  for (idim = 0; idim < Ndim; idim++)
		    {
		      ...
		      if (ijk_tmp_box[idim] == Nodes_x[idim]
			  && Type_bc[idim][1] == PERIODIC && Pflag[idim])
			{
			  ijk_tmp_box[idim] = 0;
			}
		    }
		  ...
		  if (Index_wall_nodes[inode_tmp_box] == -1)
		  ...
		  if (Nnodes_wall_box > 0)
			...
			{
			  if (Wall_touch_node[index][iw] == iwall
			      && List_wall_node[index][iw] == ilist)
			    new_entry = FALSE;
			}
		    }
		  if (new_entry)
		    {
		      ...
		      List_wall_node[index][Nwall_touch_node[index]++] =
			ilist;
		    }
...
void
els_spheres (int iwall, int real_wall, int itype, int **L_wall, double *x_min,
  ...
  for (ilist = 0; ilist < Nlists_HW; ilist++)
      ...
      for (iel_box = 0; iel_box < Nelements_box; iel_box++)
	{
	  ...
	  if (Ipot_ff_c == COULOMB
	      && Surface_type[WallType[iwall]] == point_surface
	      && nelems_w_per_w[ilist][iwall] == 1)
	    L1el_charge = TRUE;
	  ...
	  if (ilist == Nlists_HW - 1)
	    {
	      delr = r12 - radius;
	      if (delr > 0.0)
		x_min[iel_box] = min (delr, x_min[iel_box]);
	      else
		x_min[iel_box] = 0.0;
	    }
...
void
els_cyls_3D (int iwall, int real_wall, int itype, int **L_wall, double *x_min,
	     int **nelems_w_per_w, int ***elems_w_per_w,
	     int *fast_fill_elem_TF, int ***el_type, double **image_pos)
{
...
void
els_cyls_cos_3D (int iwall, int real_wall, int itype, int **L_wall,
  ...
  for (ilist = 0; ilist < Nlists_HW; ilist++)
      ...
      for (iel_box = 0; iel_box < Nelements_box; iel_box++)
	{
	  ...
	  radius = WallParam[itype] + WallParam_2[itype] *
	    cos (2 * PI * (xtest[idim] - WallPos[idim][iwall]) /
		 WallParam_3[itype]);
...
void
els_atomic_centers (int iwall, int real_wall, int itype, int **L_wall,
		    double *x_min, int **nelems_w_per_w, int ***elems_w_per_w,
		    int *fast_fill_elem_TF, int ***el_type,
		    double **image_pos)
{
...
void
els_slit_pore_2D (int iwall, int real_wall, int itype, int **L_wall,
		  double *x_min, int **nelems_w_per_w, int ***elems_w_per_w,
		  int *fast_fill_elem_TF, int ***el_type, double **image_pos)
{
      ...
      for (iel_box = 0; iel_box < Nelements_box; iel_box++)
	{
	  ...
	  switch (Orientation[itype])	/* xtest = center of element */
	    ...
	    case 1:
	      ...
	      break;
	    case 0:
	      xtest[0] = node_pos[1] + 0.5 * Esize_x[1];
	      dim[0] = 1;
	      xtest[1] = node_pos[0] + 0.5 * Esize_x[0];
	      dim[1] = 0;
	      break;
	    }
...
void
els_cyl_pore_3D (int iwall, int real_wall, int itype, int **L_wall,
		 double *x_min, int **nelems_w_per_w, int ***elems_w_per_w,
		 int *fast_fill_elem_TF, int ***el_type, double **image_pos)
{
      ...
      for (iel_box = 0; iel_box < Nelements_box; iel_box++)
	{
	  ...
	  switch (Orientation[itype])	/* xtest = center of element */
	    ...
	    case 2:
	      ...
	      break;
	    case 1:
	      xtest[0] = node_pos[2] + 0.5 * Esize_x[2];
	      dim[0] = 2;
	      xtest[1] = node_pos[0] + 0.5 * Esize_x[0];
	      dim[1] = 0;
	      xtest[2] = node_pos[1] + 0.5 * Esize_x[1];
	      dim[2] = 1;
	      break;
	    case 0:
	      xtest[0] = node_pos[1] + 0.5 * Esize_x[1];
	      dim[0] = 1;
	      xtest[1] = node_pos[2] + 0.5 * Esize_x[2];
	      dim[1] = 2;
	      xtest[2] = node_pos[0] + 0.5 * Esize_x[0];
	      dim[2] = 0;
	      break;
	    }
...
void
els_cone_pore_2D (int iwall, int real_wall, int itype, int **L_wall,
		  double *x_min, int **nelems_w_per_w, int ***elems_w_per_w,
		  int *fast_fill_elem_TF, int ***el_type, double **image_pos)
{
  ...
  double node_pos[3];
  double r12, delr,
    radius, radius1, radius2, sten, x12, length, r12new, x12new, radiusnew;
  double roff = 0.00000000001;
  ...
  for (ilist = 0; ilist < Nlists_HW; ilist++)
      ...
      for (iel_box = 0; iel_box < Nelements_box; iel_box++)
	{
	  ...
	  radius =
	    radius2 + (radius2 - radius1) * (x12 - 0.5 * length) / length;
	  radius += roff;
	  ...
	  if (Ipot_wf_n[iwall] == VEXT_1D_XMIN)
		...
		case 0:
	      ...
	      radiusnew =
		radius2 + (radius2 - radius1) * (x12new -
						 0.5 * length) / length;
	      radiusnew += roff;
	      x12new = fabs (x12new);
...
void
els_cone_pore_3D (int iwall, int real_wall, int itype, int **L_wall,
		  double *x_min, int **nelems_w_per_w, int ***elems_w_per_w,
		  int *fast_fill_elem_TF, int ***el_type, double **image_pos)
{
  int iel, iel_box, inode, idim, ilist, dim[3];
  ...
  double r12_sq_sum, r12, delr, dx,
    radius, radius1, radius2, sten, x12, length;
  double roff = 0.00000000001;
...
void
els_cyl_pores (int iwall, int real_wall, int itype, int **L_wall,
	       double *x_min, int **nelems_w_per_w, int ***elems_w_per_w,
	       int *fast_fill_elem_TF, int ***el_type, double **image_pos)
{
...
void
els_planar (int iwall, int real_wall, int itype,
	    ...
	    int *fast_fill_elem_TF, int ***el_type, double **image_pos)
{
  ...
  for (ilist = 0; ilist < Nlists_HW; ilist++)
      ...
      for (iel_box = 0; iel_box < Nelements_box; iel_box++)
	{
	  ...
	  if (Orientation[itype] == 0)
	    idim = 0;
	  else if (Orientation[itype] == 1)
	    idim = 1;
	  else
	    idim = 2;
	  ...
	  if (ilist == Nlists_HW - 1)
	    {
	      ...
	      if (delx > 0.0)
		x_min[iel_box] = min (delx, x_min[iel_box]);
	      else
		x_min[iel_box] = 0.0;
	    }
...
void
els_finite_planar (int iwall, int real_wall, int itype,
		   int **L_wall, double *x_min,
		   int **nelems_w_per_w, int ***elems_w_per_w,
		   int *fast_fill_elem_TF, int ***el_type, double **image_pos)
{
  int iel, iel_box, inode, idim, ilist, in_wall, in_sten, near_wall, npos;
  ...
  for (ilist = 0; ilist < Nlists_HW; ilist++)
      ...
      for (iel_box = 0; iel_box < Nelements_box; iel_box++)
	{
	  ...
	  if (Ndim == 3)
	    length[2] = 2.0 * WallParam_3[itype];
	  if (Lhard_surf && ilist != Nlists_HW - 1)
	    {
	      length[0] += diam;
	      length[1] += diam;
	  ...
	  for (idim = 0; idim < Ndim; idim++)
	    {
	      ...
	      if (x12[idim] > 0.5 * length[idim])
		in_wall = FALSE;
	      if (x12[idim] > 0.5 * length[idim] + sten[idim])
		in_sten = FALSE;
	      if (Ipot_ff_c == COULOMB && x12[idim] >
		  0.5 * length[idim] + Dielec_X)
		near_wall = FALSE;
	    }
	  ...
	  if (ilist == Nlists_HW - 1)
	      ...
	      for (idim = 0; idim < Ndim; idim++)
		{
		  if (idim == 0)
		    ...
		    xtest[idim] = x12[idim] - WallParam_3[itype];

		  if (xtest[idim] > 0)
		    xsave[npos++] = xtest[idim];
		}
