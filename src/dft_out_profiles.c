...
void
collect_x_old (double *x, int flag)
{
  ...
  if (Proc == 0)
	...
	{
	  for (iunk = 0; iunk < Nunk_per_node; iunk++)
	    X_old[index[i] * Nunk_per_node + iunk] =
	      unk_global[i * Nunk_per_node + iunk];
	}
...
collect_vext_old ()
{
  ...
  for (i = 0; i < Num_Proc; i++)
    {
      comm_comp_proc_tmp[i] = 0;
      comm_offset_comp_tmp[i] = 0;
    }
  if (Proc == 0)
    {
      for (i = 0; i < Num_Proc; i++)
	{
	  ...
	  comm_offset_comp_tmp[i] =
	    Comm_offset_unk[i] * Ncomp / Nunk_per_node;
	}
    }
...
void
print_profile (char *output_file4)
{
  int icomp, i, inode, ijk[3], idim, nunk_print, npol =
    0, ipol, iseg, itype_mer;
...
/*               if (icomp<Ncomp) fprintf(ifp,"%22.17f\t", X_old[inode*Nunk_per_node+icomp]/Rho_b[icomp]);*/
		...
		fprintf (ifp, "%22.17f\t",
			 X_old[inode * Nunk_per_node + icomp + Ncomp]);
	      else if (icomp < 2 * Ncomp)
		{		/* now print fields */
		  tmp = X_old[inode * Nunk_per_node + icomp - Ncomp];
		  if (tmp >
		      0.0 /*DENSITY_MIN *//*Rho_b[icomp]*exp(-VEXT_MAX) */ )
		    {
		      tmp = -log (tmp);
		  ...
		  if (Type_poly == 2)
		    {
		      ...
		      fprintf (fp6, "%22.17f\t",
			       X_old[inode * Nunk_per_node +
				     icomp] * X_old[inode * Nunk_per_node +
						    itype_mer]);
		    }
		  else
		    fprintf (fp6, "%22.17f\t",
      ...
      if (Ipot_ff_c == 1 && !Sten_Type[POLYMER_CR])
	{
	  for (icomp = 0; icomp < Ncomp; icomp++)
	    fprintf (ifp, "%20.15f\t",
		     Rho_b[icomp] * exp (-Charge_f[icomp] *
					 X_old[inode * Nunk_per_node + Ncomp +
					       Nrho_bar] -
	...
	fprintf (ifp, "\n");
      if ((Ndim == 1 && ijk[0] == Nodes_x[0] - 1) || (Ndim == 2 &&
						      ...
						      && ijk[1] ==
						      Nodes_x[1] - 1)
	  || (Ndim == 3 && ijk[0] == Nodes_x[0] - 1
	      && ijk[1] == Nodes_x[1] - 1 && ijk[2] == Nodes_x[2] - 1))
	fprintf (ifp, "\n");
    }				/* loop over all nodes  */
  fclose (ifp);
  if (Sten_Type[POLYMER_CR])
...
void
print_gofr (char *output_file6)
{
  int icomp, i, inode, ijk[3], idim, nunk_print, npol =
    0, itype_mer, iwall, iunk;
  ...
  for (iwall = 0; iwall < Nwall; iwall++)
    {				/*compute g(r) for different atoms
      ...
      for (inode = 0; inode < Nnodes; inode++)
	{
	  ...
	  for (idim = 0; idim < Ndim; idim++)
	    {
	      rsq =
		rsq + (ijk[idim] * Esize_x[idim] -
		       (WallPos[idim][iwall] +
			0.5 * Size_x[idim])) * (ijk[idim] * Esize_x[idim] -
						(WallPos[idim][iwall] +
						 0.5 * Size_x[idim]));
	    }
	  if (rsq > 0.0)
	  ...
	  for (icomp = 0; icomp < nunk_print; icomp++)
		...
		{
		  if (Sten_Type[POLYMER_CR])
		    iunk += Ncomp;	/*rhos are after fields */
...
void
print_zeroTF (int **zero_TF, char *output_file)
{
...
void
print_Nodes_to_zone (int *node_to_zone, char *output_file)
{
...
void
print_charge_surf (double **charge_w_sum, char *output_file)
{
  int i, icount;
  ...
  for (idim = 0; idim < Ndim; idim++)
      ...
      if (Proc == 0)
	{
	  ...
	  for (i = 1; i < Num_Proc; i++)
	    {
	      comm_offset_icount[i] =
		comm_offset_icount[i - 1] + comm_icount_proc[i - 1];
	    }
	}
...
void
print_charge_vol (double *charge_els, char *output_file)
{
  ...
  for (loc_inode = 0; loc_inode < Nnodes_per_proc; loc_inode++)
    {
      ...
      logical = FALSE;
      for (idim = 0; idim < Ndim; idim++)
	if (ijk[idim] == Nodes_x[idim] - 1 && Type_bc[idim][1] != PERIODIC)
	  logical = TRUE;
  ...
  MPI_Gather (&icount, 1, MPI_INT, comm_icount_proc, 1, MPI_INT, 0,
	      MPI_COMM_WORLD);
...
void
print_vext (double **vext, char *output_file)
{
...
print_rho_bar: This routine collects arrays of the rho_bar type
               and prints them out  */
void
print_rho_bar (struct RB_Struct *rho_bar, char *output_file)
{
  ...
  for (isten = 0; isten < imax; isten++)
	  ...
	  if (B2L_1stencil[inode_box] >= 0)
	    {
	      ...
	      if (isten == 0)
		unk_loc[loc_inode] = point->S0;
	      else if (isten == 1)
		unk_loc[loc_inode] = point->S1;
	      else if (isten == 2)
		unk_loc[loc_inode] = point->S2;
	      else if (isten == 3)
		unk_loc[loc_inode] = point->S3;
	      else if (isten == 4)
		unk_loc[loc_inode] = point->V1[0];
	      else if (isten == 5)
		unk_loc[loc_inode] = point->V2[0];
	      else if (isten == 6)
		unk_loc[loc_inode] = point->V1[1];
	      else if (isten == 7)
		unk_loc[loc_inode] = point->V2[1];
	      else if (isten == 8)
		unk_loc[loc_inode] = point->V1[2];
	      else if (isten == 9)
		unk_loc[loc_inode] = point->V2[2];
	    }
	  else
	    unk_loc[loc_inode] = 0.0;
	}
      ...
      if (Proc == 0)
	{
	  for (inode = 0; inode < Nnodes; inode++)
	    {
	      if (isten == 0)
		unk[index[inode]].S0 = unk_global[inode];
	      else if (isten == 1)
		unk[index[inode]].S1 = unk_global[inode];
	      else if (isten == 2)
		unk[index[inode]].S2 = unk_global[inode];
	      else if (isten == 3)
		unk[index[inode]].S3 = unk_global[inode];
	      else if (isten == 4)
		unk[index[inode]].V1[0] = unk_global[inode];
	      else if (isten == 5)
		unk[index[inode]].V2[0] = unk_global[inode];
	      else if (isten == 6)
		unk[index[inode]].V1[1] = unk_global[inode];
	      else if (isten == 7)
		unk[index[inode]].V2[1] = unk_global[inode];
	      else if (isten == 8)
		unk[index[inode]].V1[2] = unk_global[inode];
	      else if (isten == 9)
		unk[index[inode]].V2[2] = unk_global[inode];
	    }
	  safe_free ((void *) &unk_global);
	  safe_free ((void *) &index);
	}
...
void
print_time_histogram (int *hist, int *niters)
{
