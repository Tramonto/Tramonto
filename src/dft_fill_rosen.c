...
void  load_Jacobian(double *, struct RB_Struct, int, int, 
                                        int *, int, int);
                       ...
                       int resid_only_flag)
{
  ...
  double resid_start;
  if (fill_flag != MSR_PREPROCESS) resid_start=resid[loc_i];
      ...
      if (jnode_box >= 0 ) {
         ...
         if (fill_flag != MSR_PREPROCESS) {
             ...
             if (sten_type == DELTA_FN) {
                ...
                if (!resid_only_flag) {
                  if (Type_func == 0)
                     tmp = d2phi_drb2_delta(rho_bar[loc_jnode], weight, offset, 
                                            Inv_rad[icomp], Inv_4pir[icomp], 
                                            Inv_4pirsq[icomp], sign, fill_flag);
                  else
                     tmp = d2phi_drb2_delta2(rho_bar[loc_jnode], weight, offset, 
                                            Inv_rad[icomp], Inv_4pir[icomp], 
                                            Inv_4pirsq[icomp], sign, fill_flag);
                }
             }
               ...
               if (!resid_only_flag) {
                 if (Type_func == 0)
                   tmp = d2phi_drb2_theta(rho_bar[loc_jnode], weight, offset, fill_flag);
                 else
                   tmp = d2phi_drb2_theta2(rho_bar[loc_jnode], weight, offset, fill_flag);
               }
             }
         ...
         if (!resid_only_flag) {
           if (Matrix_fill_flag == FULL_MSR_FILL)
              ...
              load_Jac_Save_Jacobian(mat_row, tmp, jnode_box, jzone, bindx_tmp,
                                     fill_flag, jac_weights_hs, jac_columns_hs);
         }
         t_lj += MPI_Wtime();
      }
                       ...
                       struct RB_Struct *dphi_drb_bulk_right_ptr)
{
 ...
 if (B2L_1stencil[inode_box] != -1) {
  ...
  if (Iliq_vap == 3 || Lsteady_state == TRUE){
     ...
     for (i=0;i<imax; i++){
        if (Lsteady_state == TRUE) rho = Rho_b_LBB[i];
        else                       rho = Rho_coex[1];
        ...
        if (Lsteady_state == TRUE) rho = Rho_b_RTF[i];
        else                       rho = Rho_coex[0];
    ...
    if (fill_flag == FULL_MSR_FILL) {
      ...
      for (idim = 0; idim<Ndim; idim++)
         tmp.V2[idim] = - weight * (rb1v[idim]*inv_one_m_rb3_sq
                      + rb2*rb2v[idim]*inv_one_m_rb3_3rd/(2*PI) );
    }
  }
...
void load_Jac_Save_Jacobian(double *mat_row, struct RB_Struct tmp,
                   int jnode_box, int jzone, int *bindx_tmp, int fill_flag,
                   double ***jac_weights_hs, int ***jac_columns_hs)
{
...

{
  ...
  for (jcomp=0; jcomp<Ncomp; jcomp++) {
    if (Nlists_HW == 1 || Nlists_HW == 2) jlist = 0;
    else if (Nlists_HW > 2)  jlist = jcomp;
    ...
    switch (fast_fill_TF_j) {
       case CHECK_NONE:
         fast_fill_Jacobian(DELTA_FN, jcomp, jzone, jnode_box, 
                       ...
                       mat_row, bindx_tmp,fill_flag);   break;
       case CHECK_HW:
                        ...
                        double *mat_row, int *bindx_tmp,int fill_flag)
{
  ...
  for (isten = 0; isten < sten->Length; isten++) {
    ...
    knode_box = jnode_box;
    switch (Ndim) {
      case 3:   knode_box += offset[0] + offset[1] * Nodes_x_box[0]
                                       + offset[2] * Nodes_plane_box; break;
      case 2:   knode_box += offset[0] + offset[1] * Nodes_x_box[0]; break;
      case 1:   knode_box += offset[0];
    }
           ...
           if (fill_flag != MSR_PREPROCESS)
             if (Nodes_2_boundary_wall[jlist][knode_box]!=-1)
                 weight = HW_boundary_weight
                          (jcomp,jlist,sten->HW_Weight[isten], knode_box, reflect_flag);
