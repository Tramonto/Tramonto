...

{
 ...
 /*************************** BEGIN EXECUTION *******************************/
  ...
  calc_rhs_continuation(TP_CONT_SOL2, x, b, NULL, NULL, NULL,
		        chi->bif_param, NULL, cgi->numUnks, cgi->numOwnedUnks);
  ...
  calc_rhs_continuation(TP_CONT_SOL3, x, e, a, cpi->scale_vec, x_tmp,
		 chi->bif_param, chi->y_vec, cgi->numUnks, cgi->numOwnedUnks);
  calc_rhs_continuation(TP_CONT_SOL3, x, f, a, cpi->scale_vec, x_tmp,
		 chi->bif_param, chi->z_vec, cgi->numUnks, cgi->numOwnedUnks);
  if (cpi->mass_x) {
