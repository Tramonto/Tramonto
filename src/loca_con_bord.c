...
static double scalar_perturbation (double param);
...
int
arc_length_bordering_alg (double *x, double *delta_x,
 ...
 */
{
 ...
 /*************************** BEGIN EXECUTION *******************************/
  ...
  calc_rhs_continuation (ARC_CONT_SOL2, x, dx2, NULL, NULL, NULL,
			 cgi->param, NULL, cgi->numUnks, cgi->numOwnedUnks);
  ...
  al_eq_soln_contrib =
    ...
    - scaled_dot_prod (cpi->x_old, cpi->x_tang, cpi->scale_vec,
		       cgi->numOwnedUnks);
  ...
  y = (arc_length_equation -
       scaled_dot_prod (cpi->x_tang, delta_x, cpi->scale_vec,
			cgi->numOwnedUnks)) / (cpi->dp_ds -
					       scaled_dot_prod (cpi->x_tang,
								dx2,
								cpi->
								scale_vec,
								cgi->
  ...
  if (cgi->printproc)
    {
      printf ("\n\t\tFraction of arc-step due to soln, param = %g %g\n",
	      al_eq_soln_contrib / cpi->arc_step,
	      al_eq_param_contrib / cpi->arc_step);
...
int
turning_point_alg (double *x, double *delta_x, struct con_struct *con,
		   double reltol, double abstol)
/*
...
#endif
  int i;
  double param_update, r_update, vecnorm, gnum_unks, tmp;
...
#ifdef SCALE_TP
  b_big = 1.0 / ltransnorm (b, phi);
  /*SCALED*/ a_big = -ltransnorm (a, phi) * b_big;
  /*SCALED*/ for (i = 0; i < cgi->numUnks; i++)
    a[i] += a_big * b[i];
  /*SCALED*/ for (i = 0; i < cgi->numUnks; i++)
    b[i] *= b_big;
  /*SCALED*/
    ...
    calc_rhs_continuation (TP_CONT_SOL3, x, c, a, phi, x_tmp,
			   con->turning_point_info.bif_param, y,
			   cgi->numUnks, cgi->numOwnedUnks);
...
#ifdef SCALE_TP
  d_big = 1.0 / ltransnorm (d, phi);
  /*SCALED*/ c_big = -ltransnorm (c, phi) * d_big;
  /*SCALED*/ for (i = 0; i < cgi->numUnks; i++)
    c[i] += c_big * d[i];
  /*SCALED*/ for (i = 0; i < cgi->numUnks; i++)
    d[i] *= d_big;
...
*/
  ...
  param_update =
    fabs (dt_p) / (reltol * fabs (con->turning_point_info.bif_param) +
		   abstol);
  ...
  if (cgi->printproc)
    {
      printf ("\n\tbif_parameter (%g) unscaled and scaled update = %g %g\n",
	      con->turning_point_info.bif_param, dt_p, param_update);
...
int
pitchfork_alg (double *x, double *delta_x, struct con_struct *con,
	       double reltol, double abstol)
/*
...
/* Seems to work better as 1 for PF calcs      */
 ...
 /*************************** BEGIN EXECUTION *******************************/
  ...
  if (first)
      ...
      if (tmp == 0.0)
	{
	  if (cgi->printproc)
	    {
	      printf
		("ERROR in pitchfork alg: Asymmetric vector must be supplied\n");
	    }
	  exit (-1);
	}
      for (i = 0; i < cgi->numUnks; i++)
  ...
  calc_rhs_continuation (TP_CONT_SOL3, x, d, a, cpi->scale_vec, x_tmp,
			 con->pitchfork_info.bif_param, phi, cgi->numUnks,
			 cgi->numOwnedUnks);
  ...
  d_eps =
    -eps + ((ipx + ipa) * lte + ipb * (1.0 - ltd)) / (ipb * ltf - ipc * lte);
...
/*
if (cgi->printproc) printf("writing x, n, psi, delta_x, delta_n\n");
...
*/
  ...
  if (cgi->printproc)
    {
      printf ("\n\tbif_parameter (%g) unscaled and scaled update = %g %g\n",
	      con->pitchfork_info.bif_param, dt_p, param_update);
...
int
hopf_alg (double *x, double *delta_x, struct con_struct *con,
	  double reltol, double abstol)
/*
 ...
 */
{
  ...
  int i;
  double x_update, param_update, gnum_unks, tmp, tmp2, alpha, beta;
 ...
 /*************************** BEGIN EXECUTION *******************************/
  ...
  if (first)
      ...
      if ((tmp == 0.0) || (tmp2 == 0.0))
	{
	  if (cgi->printproc)
	    {
	      printf
		("ERROR in Hopf alg: complex vector pair must be supplied\n");
	    }
	  exit (-1);
	}
      ...
      else if (chi->mass_flag == 5)
	{
	  ...
	  for (i = 0; i < cgi->numUnks; i++)
	    x_tmp[i] = x[i] + scalar_perturbation (x[i]);
	  ...
	  dt_p = scalar_perturbation (chi->bif_param);
	  ...
	  if (cgi->printproc)
		...
		printf
	      ...
	      if (cpi->mass_param == 1)
		printf
		  ("\tMass Matrix is a Function of Bifurcation Parameter!\n");
	      else
		printf
		  ("\tMass Matrix is Independent of Bifurcation Parameter!\n");
	    }
	}
      else
	{
  ...
  calc_rhs_continuation (TP_CONT_SOL2, x, b, NULL, NULL, NULL,
			 chi->bif_param, NULL, cgi->numUnks,
			 cgi->numOwnedUnks);
  ...
  calc_rhs_continuation (HP_CONT_SOL3, x, c, chi->z_vec, chi->y_vec,
			 x_tmp, chi->bif_param, d, cgi->numUnks,
			 cgi->numOwnedUnks);
  ...
  calc_rhs_continuation (TP_CONT_SOL3, x, e, a, cpi->scale_vec, x_tmp,
			 chi->bif_param, chi->y_vec, cgi->numUnks,
			 cgi->numOwnedUnks);
  calc_rhs_continuation (TP_CONT_SOL3, x, f, a, cpi->scale_vec, x_tmp,
			 chi->bif_param, chi->z_vec, cgi->numUnks,
			 cgi->numOwnedUnks);
  if (cpi->mass_x)
    {
  ...
  if (cpi->mass_param)
    {
      vec_copy (x, M_1);
      vec_copy (x, M_2);
      calc_rhs_continuation (HP_CONT_DMDPARAM, x, M_1, a, cpi->scale_vec,
			     x_tmp, chi->bif_param, chi->z_vec, cgi->numUnks,
			     cgi->numOwnedUnks);
      calc_rhs_continuation (HP_CONT_DMDPARAM, x, M_2, a, cpi->scale_vec,
			     x_tmp, chi->bif_param, chi->y_vec, cgi->numUnks,
			     cgi->numOwnedUnks);
      for (i = 0; i < cgi->numUnks; i++)
	{
	  g[i] += M_1[i] * chi->omega;
  ...
  if (cgi->printproc)
    {
      ...
      printf
	("\tVariable     Scaled Update (<1)  Unscaled Update  New Value\n");
      printf
	("\t***********************************************************\n");
      printf ("\tX vector     %e\n", x_update);
...
int
phase_transition_alg (double *x, double *delta_x,
 ...
 */
{
 ...
 /*************************** BEGIN EXECUTION *******************************/
  ...
  dt_p = scalar_perturbation (con->phase_transition_info.bif_param);
  ...
  eps = PERTURB * sqrt (dp (x, x) / (dp (a, a) + PERTURB)
			+ dp (x2, x2) / (dp (c, c) + PERTURB));
  ...
  eps = PERTURB * sqrt (dp (x, x) / (dp (b, b) + PERTURB)
			+ dp (x2, x2) / (dp (d, d) + PERTURB));
...
void
calc_rhs_continuation (int rhs_type, double *x, double *resid_vector,
...
{
  double dc_p, dc_p1, *resid_delta;
  int i;

  /* Allocate and initialize resid_delta */

  init_vec (resid_vector);
  ...
  if (rhs_type == CONT_TANGENT)
    {
      ...
      dc_p = scalar_perturbation (param);
...
/*****************************************************************************/
int
continuation_hook (double *x, double *delta_x, void *con_void,
		   double reltol, double abstol)
{
