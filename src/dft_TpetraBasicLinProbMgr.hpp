...
protected:
  ...
  {
    //printf("Max GlobalIndex = %d, Max LocalIndex = %d\n", globalRowMap_->getMaxGlobalIndex(), globalRowMap_->getMaxLocalIndex());
    //printf("numprocs = %d\n", comm_->getSize());
    return(globalRowMap_->getLocalElement(ownedToSolverGID(ownedPhysicsID,ownedNode)));
  }
  /*  inline LocalOrdinal ownedToSolverLID(LocalOrdinal ownedPhysicsID, LocalOrdinal ownedNode) const { 
      if (groupByPhysics_) 
  ...
  boxToSolverGID
  (GlobalOrdinal boxPhysicsID, LocalOrdinal boxNode) const 
  {
    if (groupByPhysics_) 
