...
/****************************************************************************/
void
put_row_in_msr (int i_box, int loc_i, int *bindx,
		int *bindx_tmp, int **bindx_2d,
		double *val, double *mat_row, int fill_flag, int *first_time)
{
...
/* following needed for ideal gas */
  ...
  if (fill_flag == MSR_PREPROCESS)
	...
	if (bindx_tmp[j])
      ...
      bindx_2d[loc_i] =
	(int *) array_alloc (1, nonzeros_in_row, sizeof (int));
...
/****************************************************************************/
void
put_zero_in_msr (int loc_i, int **bindx_2d)
{
  int nonzeros_in_row;
...
void
put_euler_lag_in_msr (int i_box, int loc_i, int **bindx_2d)
{
  int nonzeros_in_row;
  /* following needed for ideal gas */
  int ijk_box[3], icomp;
  ...
  if (Lsteady_state)
	...
	imu = Ncomp;
      if (Ipot_ff_c == COULOMB)
	bindx_2d[loc_i][1] = B2G_unk[i_box + imu];
      else
	bindx_2d[loc_i][0] = B2G_unk[i_box + imu];
    }
...
/*put_coarse_in_msr: put coarsened rows in msr ... we always 
                    know where the nonzeros are found !!*/
void
put_coarse_in_msr (int mesh_coasen_flag, int i_box, int loc_i, int **bindx_2d)
{
...
/*put_1Dsolution_in_msr: put 1D solution rows in msr ... we always 
                    know where the nonzeros are found !!*/
void
put_1Dsolution_in_msr (int mesh_coasen_flag, int i_box,
...
/*put_transport_in_msr: put row in msr for poisson term for which we always 
                    know where the nonzeros are found !!*/
void
put_transport_in_msr (int i_box, int loc_i, int **bindx_2d)
{
	  ...
	  for (ksten = minimum[2]; ksten <= maximum[2]; ksten++)
	    {
	      ...
	      jnode_box = offset_to_node_box (ijk_box, offset, reflect_flag);
	      if (jnode_box >= 0 &&
		  reflect_flag[0] + reflect_flag[1] + reflect_flag[2] ==
		  FALSE)
		{
		  node_to_ijk (node_box_to_node (inode_box), ijk);
		  if (!(ijk[Grad_dim] * Esize_x[Grad_dim] <= X_const_mu) &&
		      !(Size_x[Grad_dim] -
			ijk[Grad_dim] * Esize_x[Grad_dim] <= X_const_mu))
		    {
		      if (!(isten == 0 && jsten == 0 && ksten == 0))
			bindx_2d[loc_i][k++] =
			  B2G_unk[jnode_box * Nunk_per_node + icomp];
		    }
		}
	    }
    }
	  ...
	  for (ksten = minimum[2]; ksten <= maximum[2]; ksten++)
		  ...
		  FALSE)
		{
		  ...
		  if (!(ijk[Grad_dim] * Esize_x[Grad_dim] <= X_const_mu) &&
			...
			irho = icomp - Ncomp;
		      bindx_2d[loc_i][k++] =
			B2G_unk[jnode_box * Nunk_per_node + irho];
		      if (!(isten == 0 && jsten == 0 && ksten == 0))
			bindx_2d[loc_i][k++] =
			  B2G_unk[jnode_box * Nunk_per_node + icomp];
		    }
...
/*put_poisson_in_msr: put row in msr for poisson term for which we always 
                    know where the nonzeros are found !!*/
void
put_poisson_in_msr (int i_box, int loc_i, int **bindx_2d)
{
		...
		if (!(isten == 0 && jsten == 0 && ksten == 0 && j == Ncomp))
		  {
			...
			if (j == Ncomp)
		      ...
		      {
			if (j == Ncomp)
			  jtmp = 2 * Ncomp + Ngeqn_tot;
			else
			  jtmp = Ncomp + j;
		      }
		    bindx_2d[loc_i][k++] =
		      B2G_unk[jnode_box * Nunk_per_node + jtmp];
		  }
	      }
	}
      ...
      for (isten = -2; isten < 3; isten += 4)
	{
	  if (isten != 0)
		...
		if (Type_poly == -1)
		  {
		    bindx_2d[loc_i][k++] =
		      B2G_unk[jnode_box * Nunk_per_node + Ncomp + Nrho_bar];
		  }
		else
		  {
		    bindx_2d[loc_i][k++] =
		      B2G_unk[jnode_box * Nunk_per_node + 2 * Ncomp +
			      Ngeqn_tot];
		  }
	    }
	}
