...
int
con_lib (struct con_struct *con, struct set_con_struct *set_con)
/*****************************************************************************
*
*  Input Variables:
*
******************************************************************************/
{
  ...
  if (con->method != PHASE_TRANSITION_CONTINUATION &&
    ...
    {
      printf
	("ERROR: Second solution vector required for Phase Transition alg"
	 "and pitchfork algs\n");
      exit (-1);
    }
  ...
  for (n = 0; n <= set_con->max_steps; n++)
	...
	print_cont_step1 (order, con);
      ...
      num_newt_conv =
	nonlinear_solver_conwrap (con->x, (void *) con, con->step_num);
	   ...
	   */
	      ...
	      if (n < set_con->max_steps)
		{
		  ...
		  if (order == 1)
			...
			con->x[i] =
			  con->x_old[i] + con->step * con->x_tang[i];
		    }
		  else
		    {
		      ...
		      for (i = 0; i < con->numUnks; i++)
			con->x[i] =
			  con->x_old[i] + con->arc_step * con->x_tang[i];
		    }
		}
	      ...
	      else
		{
	      ...
	      if (con->printproc)
		print_cont_step_fail (order, con);
	    }
	}
      ...
      else
	{
	  ...
	  if (order == 2)
	    con->step = con->param - con->param_old;

	  if (con->printproc)
	    print_cont_step2 (order, con);
	  ...
	  if (con->step_num == 0)
		...
		{
		  if (order == 2)
		    con->arc_step *= simple_step_control (num_newt_conv,
							  set_con->
							  max_newton_its,
							  set_con->step_ctrl);
		  else
		    con->step *= simple_step_control (num_newt_conv,
						      set_con->max_newton_its,
						      set_con->step_ctrl);
		}
	       ...
	       */
		       ...
		       && fabs (con->step) < fabs (set_con->first_step))
		{
		  if (order == 2)
		    con->arc_step *= simple_step_control (num_newt_conv,
							  set_con->
	  ...
	  if (n < set_con->max_steps)
		...
		case 1:
		  ...
		  calc_rhs_continuation (CONT_TANGENT, con->x_tang, NULL,
					 NULL, NULL, con->delta_param,
					 con->param, NULL, con->numUnks,
					 con->numOwnedUnks);
		...
		case 2:
		  ...
		  calc_scale_vec_conwrap (con->x, con->scale_vec,
					  con->numUnks);
...
/*****************************************************************************/
static void
print_cont_step1 (int order, struct con_struct *con)
/*
...
static void
print_cont_step2 (int order, struct con_struct *con)
/*
...
static void
print_cont_step_fail (int order, struct con_struct *con)
/*
