...
void
setup_wall_wall_potentials (int **nelems_w_per_w, int ***elems_w_per_w)
...
/* setup_lj_atomic: In this routine we base wall-wall potentials on the
                            ...
                            surfaces.  */
void
setup_lj_atomic (int iwall, int jwall)
{
  ...
  Uww[iwall][jwall] +=
    uLJ12_6_cut (r, Sigma_ww[WallType[iwall]][WallType[jwall]],
...
/* setup_coulomb_atomic: In this routine we base wall-wall potentials on the
                            ...
                            surfaces.  */
void
setup_coulomb_atomic (int iwall, int jwall)
{
  ...
  for (idim = 0; idim < Ndim; idim++)
    {
      if ((WallPos[idim][iwall] == -0.5 * Size_x[idim]
	   && Type_bc[idim][0] == REFLECT)
	  || (WallPos[idim][iwall] == 0.5 * Size_x[idim]
	      && Type_bc[idim][1] == REFLECT))
	faci *= 2.0;
      if ((WallPos[idim][jwall] == -0.5 * Size_x[idim]
	   && Type_bc[idim][0] == REFLECT)
	  || (WallPos[idim][jwall] == 0.5 * Size_x[idim]
	      && Type_bc[idim][1] == REFLECT))
	facj *= 2.0;
    }
  ...
  Uww[iwall][jwall] +=
    uCOULOMB (r, faci * Elec_param_w[iwall], facj * Elec_param_w[jwall]);
...
void
setup_ww_integrated (int iwall, int *nelems_w_per_w, int **elems_w_per_w,
		     int L_LJ, int L_COULOMB)
{
  ...
  double max_cut, **image_pos, node_pos[3], node_pos_w[3],
    point[3], node_pos_w2[3], u_ww, r_center_sq, node_pos_jw[3];
  ...
  for (iwall = 0; iwall < Nwall - 1; iwall++)
      ...
      for (wall_el = 0; wall_el < nelems_w_per_w[ilist]; wall_el++)
	{
	  if (Proc == 0 && Iwrite == VERBOSE)
	    printf
	      ("iwall: %d  of Nwall: %d wall_el: %d  of nelems_w_per_w: %d\n",
	  ...
	  for (jwall = iwall + 1; jwall < Nwall; jwall++)
	    {
	      if (Link[iwall] != Link[jwall])
		{
		  for (jwall_el = 0; jwall_el < nelems_w_per_w[ilist];
		       jwall_el++)
		    {
		      jnode_llb =
			element_to_node (elems_w_per_w[ilist][jwall_el]);
			...
			{
			  pos_jw[idim] =
			    node_pos_jw[idim] + 0.5 * Esize_x[idim];
			  jimage_pos[0][idim] = pos_jw[idim];
			}
		      jimage = 1;

		      iside = -1;
		      if (ijk_j[0] == 0 && Type_bc[0][0] == IN_WALL)
			iside = 0;
		      else if (ijk_j[0] == Nodes_x[0] - 1
			       && Type_bc[0][1] == IN_WALL)
			iside = 1;

		      if (iside != -1)
			find_images2 (0, max_cut, &jimage, jimage_pos, pos_jw,
				      iwall, iside);
			  ...
			  for (idim = 0; idim < Ndim; idim++)
			    {
			      r_center_sq +=
				(node_pos_w[idim] -
				 jimage_pos[imj][idim]) * (node_pos_w[idim] -
							   jimage_pos[imj]
							   [idim]);
			    }
			  if (r_center_sq < 4.0)
			      ...
			      for (jg = 0; jg < ngpj; jg++)
				{
				  for (ig = 0; ig < ngpi; ig++)
				      ...
				      if (Uww[iwall][jwall] < VEXT_MAX)
					{
					  ...
					  find_images (0, max_cut, &image,
						       image_pos, node_pos_w,
						       point);

					  if (Ndim > 1)
					    {
					      image_x = image;
					      for (iel_y = 0; iel_y < image_x;
						   iel_y++)
						{
						  for (idim = 0; idim < Ndim;
						       idim++)
						    node_pos_w2[idim] =
						      image_pos[iel_y][idim];
						  find_images (1, max_cut,
							       &image,
							       image_pos,
							       node_pos_w2,
							       point);
					  ...
					  if (Ndim == 3)
					    {
					      image_xy = image;
					      for (iel_z = 0;
						   iel_z < image_xy; iel_z++)
						{
						  for (idim = 0; idim < Ndim;
						       idim++)
						    node_pos_w2[idim] =
						      image_pos[iel_z][idim];
						  find_images (2, max_cut,
							       &image,
							       image_pos,
							       node_pos_w2,
							       point);
