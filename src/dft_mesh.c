...
void surf_el_to_list (int, int, int *, int, int, int, int, double, double);
...
/********************** BEGIN EXECUTION ************************************/
void
set_up_mesh (char *output_file1, char *output_file2)
{
...
/*set up arrays on processor 0 that are needed for communications
  ...
  for (i = 0; i < Num_Proc; i++)
    Comm_node_proc[i] = Comm_unk_proc[i] = Comm_offset_node[i] =
      Comm_offset_unk[i] = 0;
  ...
  if (Proc == 0)
	...
	Comm_unk_proc[i] = Comm_node_proc[i] * Nunk_per_node;
      ...
      Comm_offset_node[0] = 0;
      Comm_offset_unk[0] = 0;
      for (i = 1; i < Num_Proc; i++)
	{
	  Comm_offset_node[i] =
	    Comm_offset_node[i - 1] + Comm_node_proc[i - 1];
  ...
  if (L1D_bc || Load_Bal_Flag == LB_WEIGHTS || (Load_Bal_Flag >= LB_TIMINGS
						&& Mesh_coarsening != FALSE))
    {
      ...
      if (Iwrite == VERBOSE)
	printf ("Proc: %d Nodes_per_proc: %d\n", Proc, Nnodes_per_proc);
      ...
      if (Proc == 0)
	{
	  ...
	  for (i = 1; i < Num_Proc; i++)
		...
		Comm_offset_node[i - 1] + Comm_node_proc[i - 1];
	      Comm_offset_unk[i] =
		Comm_offset_unk[i - 1] + Comm_unk_proc[i - 1];
	    }
	}
  ...
  if (L1D_bc)
      ...
      for (i = 0; i < Nnodes_box; i++)
	{
	  ...
	  for (idim = 0; idim < Ndim; idim++)
	    if (Mesh_coarsen_flag[i] != FLAG_1DBC
		|| (ijk[idim] != 0 && idim != Grad_dim))
	      flag = FALSE;
	  if (flag)
	    {
	      count++;
	      ...
	      if (Mesh_coarsening != FALSE && Nodes_to_zone[i] > 0)
		{
		  ...
		  if (ijk[0] % coarse_fac)
		    Mesh_coarsen_flag[i] = -1;
		  else if (ijk[1] % coarse_fac)
		    Mesh_coarsen_flag[i] = -2;
		  else if (ijk[2] % coarse_fac)
		    Mesh_coarsen_flag[i] = -3;
		}
	    }
	}
    }
...
void
control_mesh (FILE * fp1, char *output_file2, int print_flag)
{
  int **elems_f, ***elems_w_per_w;

  int *nelems_f, **nelems_w_per_w;
  int ***el_type;
  ...
  int count_zero, count_zero_all, count_coarse_resid, count_coarse_r_all,
    ...
    count_check_hw, count_check_hw_all, count_check_both,
    count_check_both_all, count_check_bc, count_check_bc_all;
  ...
  if (Nwall != 0)
      ...
      if (Lhard_surf && Ipot_ff_n != IDEAL_GAS)
	{
	  ...
	  if (Ncomp > 1)
		...
		{
		  if (Sigma_ff[icomp][icomp] != sigma_test)
		    flag = TRUE;
		}
	      if (flag == TRUE)
		Nlists_HW = Ncomp + 1;
      ...
      elems_f =
	(int **) array_alloc (2, Nlists_HW, Nelements_box, sizeof (int));

      nelems_f = (int *) array_alloc (1, Nlists_HW, sizeof (int));

      elems_w_per_w =
	(int ***) array_alloc (3, Nlists_HW, Nelements_box, Nwall,
			       sizeof (int));
      nelems_w_per_w =
	(int **) array_alloc (2, Nlists_HW, Nwall, sizeof (int));

      elem_zones = (int *) array_alloc (1, Nelements_box, sizeof (int));
      for (iel_box = 0; iel_box < Nelements_box; iel_box++)
	elem_zones[iel_box] = 0;

      fast_fill_elem_TF =
	(int *) array_alloc (1, Nelements_box, sizeof (int));
      Wall_elems =
	(int **) array_alloc (2, Nlists_HW, Nelements_box, sizeof (int));
      el_type =
	(int ***) array_alloc (3, Nwall, Nlists_HW, Nelements_box,
			       sizeof (int));
      if (Ipot_ff_c == COULOMB)
	{
	  Dielec = (double *) array_alloc (1, Nelements_box, sizeof (double));
	  for (iel = 0; iel < Nelements_box; iel++)
	    Dielec[iel] = Dielec_bulk;
	}
      ...
      for (idim = 0; idim < Ndim; idim++)
	{
	  if (Type_bc[idim][0] == PERIODIC || Type_bc[idim][0] == REFLECT ||
	      Type_bc[idim][1] == PERIODIC || Type_bc[idim][1] == REFLECT)
	    nwall_max *= 3;
	}
      Wall_owners =
	(int ***) array_alloc (3, Nlists_HW, Nelements_box, nwall_max,
			       sizeof (int));
      ...
      if (Nwall != 0)
	{
	  ...
	  Index_wall_nodes =
	    (int *) array_alloc (1, Nnodes_box, sizeof (int));
	  Nodes_wall_box = (int *) array_alloc (1, Nnodes_box, sizeof (int));
	  Nwall_touch_node =
	    (int *) array_alloc (1, Nnodes_box, sizeof (int));
	  Wall_touch_node =
	    (int **) array_alloc (2, Nnodes_box, Nwall * Nlists_HW,
				  sizeof (int));
	  List_wall_node =
	    (int **) array_alloc (2, Nnodes_box, Nwall * Nlists_HW,
				  sizeof (int));
	  ...
	  for (inode_box = 0; inode_box < Nnodes_box; inode_box++)
	    Nwall_touch_node[inode_box] = 0;
	  for (inode_box = 0; inode_box < Nnodes_box; inode_box++)
	    Index_wall_nodes[inode_box] = -1;
      ...
      if (Imain_loop == 0 && Proc == 0)
	{
	  fprintf (fp1,
		   "\n---------------------------------------------------------------\n");
	  fprintf (fp1,
		   " Have set up elements for the  selected surface geometry\n");
	  ...
	  for (ilist = 0; ilist < Nlists_HW; ilist++)
		...
		fprintf (fp1,
			 "\t iwall: %d \t nelems_w_per_w[ilist][iwall]: %d\n",
	...
	setup_external_field_n (nelems_w_per_w, elems_w_per_w);
      if (Nwall > 1 && Lprint_pmf)
	setup_wall_wall_potentials (nelems_w_per_w, elems_w_per_w);
      ...
      for (icomp = 0; icomp < Ncomp; icomp++)
	{
	  ...
	  for (i = 0; i < Nnodes_per_proc; i++)
	    {
	      ...
	      if (Zero_density_TF[L2B_node[i]][icomp])
		count_zero++;
	      if (Mesh_coarsening != FALSE || L1D_bc)
		if (Mesh_coarsen_flag[L2B_node[i]] < 0)
		  count_coarse_resid++;
	      if (Nodes_to_zone[L2B_node[i]] > 0 || Coarser_jac > 0)
		count_coarse_jac++;
	      if (Fast_fill_TF[L2B_node[i]] == CHECK_NONE)
		count_fast++;
	      else if (Fast_fill_TF[L2B_node[i]] == CHECK_HW)
		count_check_hw++;
	      else if (Fast_fill_TF[L2B_node[i]] == CHECK_BC)
		count_check_bc++;
	      else if (Fast_fill_TF[L2B_node[i]] == CHECK_BOTH)
		count_check_both++;
	    }
	  ...
	  if (icomp == 0)
	    {
	      count_coarse_r_all =
		AZ_gsum_int (count_coarse_resid, Aztec.proc_config);
	      count_coarse_jac_all =
		AZ_gsum_int (count_coarse_jac, Aztec.proc_config);
	      count_fast_all = AZ_gsum_int (count_fast, Aztec.proc_config);
	      count_check_hw_all =
		AZ_gsum_int (count_check_hw, Aztec.proc_config);
	      count_check_bc_all =
		AZ_gsum_int (count_check_bc, Aztec.proc_config);
	      count_check_both_all =
		AZ_gsum_int (count_check_both, Aztec.proc_config);
	      if (Proc == 0 && Iwrite != NO_SCREEN && print_flag)
		{
		  ...
		  printf ("Total number of nodes in calculation = \t %d\n",
			  Nnodes);
		  printf
		    ("Number of coarsened residual nodes = \t %d \t or %g percent\n",
		     count_coarse_r_all, 100. * count_coarse_r_all / Nnodes);
		  printf
		    ("Number of coarsened jacobian nodes = \t %d \t  or %g percent \n",
		     count_coarse_jac_all,
		     100. * count_coarse_jac_all / Nnodes);
		  printf
		    ("Number of fast fill nodes = \t \t %d \t or %g percent\n",
		     count_fast_all, 100. * count_fast_all / Nnodes);
		  printf
		    ("Number of check boundary nodes = \t %d \t or %g percent\n",
		     count_check_bc_all, 100. * count_check_bc_all / Nnodes);
		  printf
		    ("Number of check hard wall nodes = \t %d \t or %g percent \n",
		     count_check_hw_all, 100. * count_check_hw_all / Nnodes);
		  printf
		    ("Number of check both (bc and hw) nodes = \t %d \t or %g percent \n",
		     count_check_both_all,
		     100. * count_check_both_all / Nnodes);
	  ...
	  if (Proc == 0 && Iwrite != NO_SCREEN && print_flag)
	    {
	      printf
		("Number of zero density nodes for icomp: %d = \t %d \t or %g percent \n",
    ...
    {				/* if zero walls */
      ...
      Wall_elems =
	(int **) array_alloc (2, Nlists_HW, Nelements_box, sizeof (int));
      Nodes_2_boundary_wall =
	(int **) array_alloc (2, Nlists_HW, Nnodes_box, sizeof (int));
      Zero_density_TF =
	(int **) array_alloc (2, Nnodes_box, Ncomp + 1, sizeof (int));
      Dielec = (double *) array_alloc (1, Nelements_box, sizeof (double));
  ...
  if (Ipot_ff_n != IDEAL_GAS)
      ...
      for (icomp = 0; icomp < Ncomp; icomp++)
	{
	  if (Sigma_ff[icomp][icomp] < 1.e-6)
	    {
	      printf
		("ERROR : icomp=%d Sigma_ff[%d][%d]=%9.6f --- check particle sizes!\n",
...
setup_basic_domain (FILE * fp1)
{
      ...
      if (round_to_int (fmod (Size_x[idim], Esize_x[idim])) == 0)
	{
	  ...
	  if (Proc == 0 && Imain_loop == 0)
	    fprintf (fp1, "   %d \t    %d \t \t %d \t \t \t %d \t \t %d\n",
		     idim, Nodes_x[idim], Nnodes, Elements_x[idim],
		     Nelements);
	}
      else
	{
	  if (Proc == 0 && Imain_loop == 0)
	    {
	      ...
	      fprintf (fp1, "Esize_x: %lf\tSize_x: %lf\n", Esize_x[idim],
		       Size_x[idim]);
	      fprintf (fp1,
		       "idim: %d \t fmod(Size_x[idim],Esize_x[idim]): %g\n",
  ...
  for (idim = 0; idim < Ndim; idim++)
	...
	{
	  if (jdim != idim)
	    Area_surf_el[idim] *= Esize_x[jdim];
	}
    }
...
setup_basic_box (FILE * fp1)
{
  ...
  if (L1D_bc)
    ijk_1D = (int) X_1D_bc / Esize_x[Grad_dim] + 1;
  ...
  for (idim = 0; idim < Ndim; idim++)
      ...
      if (L1D_bc && idim != Grad_dim)
	{
	  if (Min_IJK[Grad_dim] <= ijk_1D ||
	      Max_IJK[Grad_dim] >= Nodes_x[Grad_dim] - ijk_1D)
	    Min_IJK_box[idim] = 0;
	}
      ...
      Pflag[idim] = FALSE;
      if (Type_bc[idim][0] == PERIODIC && (max_cut > 0.5 * Size_x[idim] ||
					   (Min_IJK[idim] == 0
					    && Max_IJK[idim] ==
					    Nodes_x[idim] - 1)))
	Pflag[idim] = TRUE;
	  ...
	  && Min_IJK_box[idim] < 0)
	Min_IJK_box[idim] = 0;
      if ((Type_bc[idim][1] != PERIODIC || Pflag[idim])
	  && Max_IJK_box[idim] >= Nodes_x[idim])
  ...
  for (idim = 0; idim < Ndim; idim++)
    {
	  ...
	  Max_IJK_box[idim] == Nodes_x[idim] - 1)
	Elements_x_box[idim] = Nodes_x_box[idim];
      else
	Elements_x_box[idim] = Nodes_x_box[idim] - 1;
  ...
  if (Proc == 0)
    fprintf (fp1, " Proc: %d  Nunknowns_box: %d \n", Proc, Nunknowns_box);

  if (Iwrite == VERBOSE)
    {
      for (idim = 0; idim < Ndim; idim++)
	fprintf (fp2, "Proc=%d Pflag[%d]=%d\n", Proc, idim, Pflag[idim]);
      fprintf (fp2, "Proc: %d  Nnodes_per_proc: %d  Nnodes_box: %d", Proc,
	       Nnodes_per_proc, Nnodes_box);
      for (idim = 0; idim < Ndim; idim++)
	fprintf (fp2, "Min_IJK[%d]: %d  Max_IJK[%d]:%d ", idim, Min_IJK[idim],
		 idim, Max_IJK[idim]);
      for (idim = 0; idim < Ndim; idim++)
	fprintf (fp2, "Min_IJK_box[%d]: %d  Max_IJK_box[%d]:%d \n", idim,
		 Min_IJK_box[idim], idim, Max_IJK_box[idim]);
    }
...
void
initialize_fast_fill_flag (void)
{
...
void
boundary_setup (char *output_file1)
{
  FILE *fp1 = NULL;
...
boundary_free (void)
{
  ...
  if (Nwall != 0)
	...
	{
	  if (Vol_charge_flag)
	    safe_free ((void *) &Charge_vol_els);
	  if (Surf_charge_flag)
	    safe_free ((void *) &Charge_w_sum_els);
	}
...
void
setup_zeroTF_and_Node2bound (FILE * fp1, int ***el_type)
{
  int reflect_flag[3], **n_fluid_els, iwall,
    iel, idim, ilist, loc_node_el,
    inode, inode_box, iel_box, icomp, iside, periodic_flag, wall_flag;
  ...
  n_fluid_els =
    (int **) array_alloc (2, nonat_count, Nlists_HW, sizeof (int));
  n_el_in_box =
    (int **) array_alloc (2, nonat_count, Nlists_HW, sizeof (int));
		  ...
		  for (ilist = 0; ilist < Nlists_HW; ilist++)
		      ...
		      if (iel >= 0)
			{
			  ...
			  if (iel_box >= 0)
				...
				if (el_type[nonatomic_walls[jw]][ilist]
				    [iel_box] == WALL_EL)
				  wall_flag = TRUE;
			      if (wall_flag)
				countw[iw][ilist]++;
		      ...
		      else if (iel == -1)
			{
			  ...
			  if (Touch_domain_boundary[iwall][ilist][idim]
			      [iside])
			    {
			      n_el_in_box[iw][ilist]++;
			      countw[iw][ilist]++;
		  ...
		  if (Zero_density_TF[inode_box][ilist])
		    {
		      ...
		      count = 0;
		      for (loc_node_el = 0; loc_node_el < Nnodes_per_el_V;
			   loc_node_el++)
			{

			  iel =
			    node_to_elem (inode, loc_node_el, reflect_flag);
			  if (iel >= 0)
			    {
			      iel_box = el_to_el_box (iel);
		      ...
		      if (count >= 2)
			{
			  ...
			  for (i = 1; i < count; i++)
			    {
			      if (Wall_owners[ilist][els_one_owner[i]][0] !=
				  test)
				all_equal = FALSE;
			    }
			  if (all_equal == FALSE)
		      ...
		      if (all_equal == TRUE && reflect)
			{
			  ...
			  if (test == TRUE)
			    {	/* node proves to _not_ be located on a center line */
			      Nodes_2_boundary_wall[ilist][inode_box] = -2;
			      if (ilist < Nlists_HW - 1)
				Zero_density_TF[inode_box][ilist] = FALSE;
			    }
  ...
  if (Imain_loop == 0 && Proc == 0 && Iwrite == VERBOSE)
    {
      ...
      fprintf (fp1, "Proc: %d \n", Proc);
      fprintf (fp1,
	       "Have set up arrays Wall_elems, and Nodes_2_boundary_wall\n");
...
setup_zeroTF_and_Node2bound_new: This routine take the information about which elements
                    ...
                    lists of wall nodes and fluid nodes for each wall list */
void
setup_zeroTF_and_Node2bound_new (FILE * fp1, int ***el_type)
{
  int reflect_flag[3], *n_fluid_els, iwall,
    iel, idim, ilist, loc_node_el,
    inode, inode_box, iel_box, icomp, iside, periodic_flag, wall_flag, jwall;
  int *n_el_in_box, *countw;
  int test, all_equal, ijk[3], reflect, count, *els_one_owner, i;
  int dim_reflect[3], ndim_reflect, wall_chk;
  int link_chk, ilink;
			  ...
			  if (ilist == List_wall_node[index][jw])
			    {
			      ...
			      if (el_type[jwall][ilist][iel_box] == WALL_EL)
				wall_flag = TRUE;
			    }
			}
		      if (wall_flag)
		  ...
		  if (all_equal == TRUE && reflect)
		      ...
		      for (i = 0; i < ndim_reflect; i++)
			{
			  ...
			  if (Xtest_reflect_TF[link_chk][idim])
			    {
			      for (ilink = 0;
				   ilink < Nwall_this_link[link_chk]; ilink++)
				{
				  wall_chk = Link_list[link_chk][ilink];
				  if (ijk[idim] == 0
				      && Type_bc[idim][0] == REFLECT
	  ...
	  for (inode_box = 0; inode_box < Nnodes_box; inode_box++)
	    {
	      ...
	      if (Nodes_2_boundary_wall[ilist][inode_box] != -1)
		fprintf (fp1,
			 "ilist: %d   inode: %d  ijk: %d %d  iwall: %d\n",
...
boundary_properties (FILE * fp1)
{
  ...
  Nelems_S =
    (int **) array_alloc (2, Nlists_HW, Nnodes_per_proc, sizeof (int));
  Surf_normal =
    (int ***) array_alloc (3, Nlists_HW, Nnodes_per_proc, 2 * Nnodes_per_el_V,
			   sizeof (int));
  Surf_elem_type =
    (int **) array_alloc (2, Nnodes_per_proc, 2 * Nnodes_per_el_V,
			  sizeof (int));
  S_area =
    (double ***) array_alloc (3, Nlists_HW, Nwall, Ndim, sizeof (double));
	  ...
	  for (iel_s = 0; iel_s < Nnodes_per_el_S; iel_s++)
	    {
	      ...
	      if (ilist == Nlists_HW - 1)
		Surf_elem_type[loc_inode][iel_s] = -1;
	    }
	}

      for (iwall = 0; iwall < Nwall; iwall++)
	for (idim = 0; idim < Ndim; idim++)
	  S_area[ilist][iwall][idim] = 0.0;
    }
  ...
  for (ilist = 0; ilist < Nlists_HW; ilist++)
      ...
      for (loc_inode = 0; loc_inode < Nnodes_per_proc; loc_inode++)
	{
	  ...
	  inode_box = node_to_node_box (inode);
	  flag_fluid = FALSE;
	  if (Nodes_2_boundary_wall[ilist][inode_box] >= 0)
	    {			/* wall-fluid
				   boundary nodes */
	      flag = -1;
		  ...
		  for (i = 0; i < Ndim - 1; i++)
		    {
		      ...
		      if ((iel[j] >= 0 && iel[j + 1] >= 0) && ((Wall_elems
								[ilist]
								[iel_box[j]]
								!= -1
								&&
								...
								-1)))
			{
			  ...
			  surf_el_to_list (loc_inode, ilist, iel_box, el,
					   type, normal, idim, esize1,
					   esize2);
			}

		      /* check for surface element in top/bottom (on right elements) */

		      if ((iel[j] >= 0 && iel[j + 2] >= 0) && ((Wall_elems
								[ilist]
								[iel_box[j]]
								!= -1
								&&
								...
								-1)))
			{
		      ...
		      if ((iel[j + 1] >= 0 && iel[j + 3] >= 0) && ((Wall_elems
								    [ilist]
								    [iel_box
								     [j +
								      1]] !=
								    ...
								    -1)))
			{
		      ...
		      if ((iel[j + 2] >= 0 && iel[j + 3] >= 0) && ((Wall_elems
								    [ilist]
								    [iel_box
								     [j +
								      2]] !=
		  ...
		  for (i = 0; i < Ndim - 1; i++)
		    {
		      ...
		      if ((iel[j] >= 0 && iel[j + 1] >= 0 &&
			   ...
			   Wall_elems[ilist][iel_box[j]] !=
			   Wall_elems[ilist][iel_box[j + 1]])
			  || (iel[j] == flag && iel[j + 1] >= 0
			      && Nwall_owners[ilist][iel_box[j + 1]] == 1
			      && fabs (0.5 * Size_x[0] -
				       WallPos[0][Wall_elems[ilist]
						  [iel_box[j + 1]]]) >
			      0.00000001) || (iel[j] >= 0
					      && iel[j + 1] == flag
					      &&
					      Nwall_owners[ilist][iel_box[j]]
					      == 1
					      &&
					      fabs (WallPos[0]
						    [Wall_elems[ilist]
					      ...
					      0.00000001))
			{
		      ...
		      if ((iel[j] >= 0 && iel[j + 2] >= 0 &&
			   ...
			   Wall_elems[ilist][iel_box[j]] !=
			   Wall_elems[ilist][iel_box[j + 2]])
			  || (iel[j] == flag && iel[j + 2] >= 0
			      && Nwall_owners[ilist][iel_box[j + 2]] == 1
			      && fabs (0.5 * Size_x[1] -
				       WallPos[1][Wall_elems[ilist]
						  [iel_box[j + 2]]]) >
			      0.00000001) || (iel[j] >= 0
					      && iel[j + 2] == flag
					      &&
					      ...
					      0.00000001))
			{
		      ...
		      if ((iel[j + 1] >= 0 && iel[j + 3] >= 0 &&
			   ...
			   Wall_elems[ilist][iel_box[j + 1]] !=
			   Wall_elems[ilist][iel_box[j + 3]])
			  || (iel[j + 1] == flag && iel[j + 3] >= 0
			      && Nwall_owners[ilist][iel_box[j + 3]] == 1
			      && fabs (0.5 * Size_x[1] -
				       WallPos[1][Wall_elems[ilist]
						  [iel_box[j + 3]]]) >
			      0.00000001) || (iel[j + 1] >= 0
					      && iel[j + 3] == flag
					      &&
					      Nwall_owners[ilist][iel_box
								  [j + 1]] ==
					      1
					      &&
					      fabs (WallPos[1]
		  ...
		  if (Ndim == 3)
		      ...
		      for (i = 0; i < 4; i++)
			{
			  ...
			  if ((iel[i] >= 0 && iel[j] >= 0 &&
			       ...
			       Nwall_owners[ilist][iel_box[j]] == 1 &&
			       Wall_elems[ilist][iel_box[i]] !=
			       Wall_elems[ilist][iel_box[j]])
			      || (iel[i] == flag && iel[j] >= 0
				  && Nwall_owners[ilist][iel_box[j]] == 1
				  && fabs (0.5 * Size_x[2] -
					   WallPos[2][Wall_elems[ilist]
						      [iel_box[j]]]) >
				  0.00000001) || (iel[i] >= 0
						  && iel[j] == flag
						  &&
			      ...
			      if (iel[i] >= 0)
				{
				  el = i;
				  normal = -3;
				  surf_el_to_list (loc_inode, ilist, iel_box,
						   el, type, normal, idim,
						   esize1, esize2);
				}
			      if (iel[j] >= 0)
	  ...
	  for (idim = 0; idim < Ndim; idim++)
	    {
	      ...
	      S_area_tot[ilist][iwall] =
		AZ_gsum_double (s_area_tot_proc, Aztec.proc_config);
	      S_area[ilist][iwall][idim] =
		AZ_gsum_double (S_area[ilist][iwall][idim],
				Aztec.proc_config);
	    }
	}
    }				/* end of loop over lists */
  ...
  if (Proc == 0 && Imain_loop >= 0)
    {
      fprintf (fp1,
	       "\n---------------------------------------------------------------\n");
      fprintf (fp1,
	       "Done calculating surface normals of boundary elements : Iloop: %d\n",
	       Imain_loop);
      fprintf (fp1, "\nProc: %d\n", Proc);
    }
  ...
  for (ilist = 0; ilist < Nlists_HW; ilist++)
      ...
      for (loc_inode = 0; loc_inode < Nnodes_per_proc; loc_inode++)
	{
	  ...
	  if (iwall != -1)
	    {			/* proceed if this is a boundary node */
	      node_to_ijk (inode, ijk);
	      if (Imain_loop >= 0)
		{
      ...
      for (idim = 0; idim < Ndim; idim++)
	{
	  ...
	  if (Proc == 0)
	    fprintf (fp1,
		     "ilist: %d Summing surface normals idim: %d yields: %d\n",
  ...
  if (Proc == 0 && Imain_loop >= 0)
      ...
      for (ilist = 0; ilist < Nlists_HW; ilist++)
	{
	  ...
	  for (iwall = 0; iwall < Nwall; iwall++)
	    {
	      fprintf (fp1,
		       "\t iwall: %d \t S_area_tot[ilist][iwall]: %9.6f\n",
		       iwall, S_area_tot[ilist][iwall]);
	      for (idim = 0; idim < Ndim; idim++)
		fprintf (fp1,
			 "\t\t idim: %d \t S_area[ilist][iwall][idim]: %9.6f\n",
...
void
find_local_els (int inode, int *iel, int *iel_box, int flag)
{
  ...
  for (idim = 0; idim < Ndim; idim++)
    {
      if ((Type_bc[idim][0] == REFLECT || Type_bc[idim][0] == LAST_NODE)
	  && ijk[idim] == 0)
	{
	  if (idim == 0)
	    {
		...
		iel[2] = iel[3] = iel[6] = iel[7] = flag;
	    }
	  else if (idim == 2)
	    iel[4] = iel[5] = iel[6] = iel[7] = flag;
	}
      else if ((Type_bc[idim][1] == REFLECT || Type_bc[idim][0] == LAST_NODE)
	       && ijk[idim] == Nodes_x[idim] - 1)
	{
	  if (idim == 0)
	    {
		...
		iel[0] = iel[1] = iel[4] = iel[5] = flag;
	    }
	  else if (idim == 2)
	    iel[0] = iel[1] = iel[2] = iel[3] = flag;
	}
    }
...
void
surf_el_to_list (int loc_inode, int ilist, int *iel_box,
		 ...
		 double esize1, double esize2)
{
  ...
  for (i = 0; i < Nwall_owners[ilist][iel_box[el]]; i++)
      ...
      if (jwall < Nwall)
	{
	  ...
	  if (Ndim > 1)
	    Surf_elem_type[loc_inode][Nelems_S[ilist][loc_inode]] = type;
	  Surf_elem_to_wall[ilist][loc_inode][Nelems_S[ilist][loc_inode]] =
	    jwall;
	  Nelems_S[ilist][loc_inode]++;

	  if (Ndim == 1)
	    S_area[ilist][jwall][idim] += 1.0;
	  else if (Ndim == 2)
	    S_area[ilist][jwall][idim] += esize1 / Nnodes_per_el_S;
	  else if (Ndim == 3)
	    S_area[ilist][jwall][idim] += esize1 * esize2 / Nnodes_per_el_S;
...
setup_surface_charge (FILE * fp1)
{
  ...
  for (iwall_type = 0; iwall_type < Nwall_type; iwall_type++)
    {
      if (Type_bc_elec[iwall_type] == CONST_CHARGE)
	ncharge_s++;
      else if (Type_bc_elec[iwall_type] == ATOMIC_CHARGE)
	ncharge_v++;
    }
  ...
  if (ncharge_s > 0)
    {
      ...
      for (loc_inode = 0; loc_inode < Nnodes_per_proc; loc_inode++)
	for (idim = 0; idim < Ndim; idim++)
	  Charge_w_sum_els[loc_inode][idim] = 0.0;
    }
  if (ncharge_v > 0 || Nlocal_charge != 0)
    {
      Charge_vol_els =
	(double *) array_alloc (1, Nelements_box, sizeof (double));
      for (iel = 0; iel < Nelements_box; iel++)
  ...
  if (Iwrite == VERBOSE)
    {
      if (Proc == 0)
	printf ("PRINTING CHARGE DISTRIBUTIONS: ncharge_v=%d ncharge_s=%d\n",
		ncharge_v, ncharge_s);
      if (ncharge_v > 0 || Nlocal_charge != 0)
	print_charge_vol (Charge_vol_els, "dft_charge_vol.dat");
      if (ncharge_s > 0)
	print_charge_surf (Charge_w_sum_els, "dft_charge_surf.dat");
    }
...
void
bc_setup_const_charge (int iwall, int loc_inode)
{
	  ...
	  for (jdim = 0; jdim < Ndim; jdim++)
	      ...
	      if (jdim != idim)
		{
		  if ((ijk[jdim] == 0 && Type_bc[jdim][0] >= REFLECT) ||
		      (ijk[jdim] == Nodes_x[jdim] - 1
		       && Type_bc[jdim][1] >= REFLECT))
		    {
		      weight *= 2.0;
		    }
		}
...
/*setup_linear_grad_of_charge: in this routine we set up a linear
      ...
      points*/
void
setup_linear_grad_of_charge (void)
{
  ...
  for (i = 0; i < Ndim; i++)
	...
	idim = i;
	Nlocal_charge =
	  1 + (int) (fabs (Charge_x[i][1] - Charge_x[i][0]) / Esize_x[i]);
	break;
      }
  charge_1 = Charge[0];
  charge_2 = Charge[1];
  ...
  Charge_x =
    (double **) array_alloc (2, Ndim, Nlocal_charge, sizeof (double));


  for (i = 0; i < Nlocal_charge; i++)
    {
      for (j = 0; j < Ndim; j++)
	Charge_x[j][i] = charge_x_1[j];

      Charge[i] =
	(charge_1 * (Nlocal_charge - 1 - i) + charge_2 * i) / (Nlocal_charge -
							       1);
      Charge_x[idim][i] =
	(charge_x_1[idim] * (Nlocal_charge - 1 - i) +
...
/*setup_volume_charge1:  This routine sets up charges on a per element
                    ...
                    charged. */
void
setup_volume_charge1 (int iwall)
{
  ...
  nmax = Nwall;
  for (idim = 0; idim < Ndim; idim++)
    esize_x_min = min (Esize_x[idim], esize_x_min);

  elems = (int *) array_alloc (1, Nelements_box, sizeof (int));


  if (Charge_type_atoms == POINT_CHARGE ||
      Sigma_ww[WallType[iwall]][WallType[iwall]] <= esize_x_min)
    r = 0.6 * esize_x_min;
  else if (Surface_type[WallType[iwall]] == atomic_centers)
    r = 0.5 * Sigma_ww[WallType[iwall]][WallType[iwall]];
  else
    r = WallParam[WallType[iwall]];

  for (idim = 0; idim < Ndim; idim++)
    x[idim] = WallPos[idim][iwall];

  nelems = 0;
  nelems_unique = 0;
  els_charge_spheres (r, x, &nelems, &nelems_unique, elems,
		      Charge_type_atoms);
  ...
  if (Proc == 0 && Iwrite == VERBOSE)
    {
      printf
	("iwall: %d size=%g has a charge of: %g being smeared over %d elements\n",
...
setup_volume_charge2 (void)
{
  ...
  for (i = 0; i < nmax; i++)
      ...
      if (Charge_type_local == BACKGROUND)
	{
	  ...
	  for (iel = 0; iel < Nelements_box; iel++)
	    Charge_vol_els[iel] += charge_per_el;
	  if (Proc == 0 && Iwrite != NO_SCREEN)
	    printf
	      ("a uniform background charge of %g total is being smeared over every element\n",
	       Charge[i]);
	}
      else
	{
...
void
els_charge_spheres (double radius, double *x, int *nelems,
		    int *nelems_unique, int *elems, int charge_type)
{
  ...
  for (iel_box = 0; iel_box < Nelements_box; iel_box++)
	...
	{
	case 3:
	  xtest[2] = node_pos[2] + 0.5 * Esize_x[2];
	case 2:
	  xtest[1] = node_pos[1] + 0.5 * Esize_x[1];
	default:
	  xtest[0] = node_pos[0] + 0.5 * Esize_x[0];
	}
      ...
      if (r12 <= radius && charge_type != POINT_CHARGE)
	{
	  ...
	  if ((B2L_node[inode_box] != -1) && !image_flag)
	    (*nelems_unique)++;
	}
  ...
  if ((*nelems == 0 && r12min < esize_x_min) || charge_type == POINT_CHARGE)
    {
      (*nelems) = 1;
      elems[0] = iel_save;
      if ((B2L_node[inode_save] != -1) && !image_flag_save)
	(*nelems_unique) = 1;
    }
}
...
void
zones_el_to_nodes (int *elem_zones)
{
  ...
  if (Iwrite == VERBOSE)
    print_Nodes_to_zone (Nodes_to_zone, "dft_zones.dat");
}
...
void
fast_fill_el_to_nodes (int *fast_fill_el_TF)
{
 ...
 */
{
  ...
  for (i = 0; i < Nnodes_box; i++)
      ...
      if (Mesh_coarsening != FALSE && Nodes_to_zone[i] > 0)
	{
	  ...
	  if (B2L_node[i] >= 0)
	    if (Mesh_coarsen_flag[i] < 0)
	      count_coarse++;
	}
      if (L1D_bc)
	{
	  if (ijk[Grad_dim] * Esize_x[Grad_dim] <= X_1D_bc + 0.00000001 ||
	      ijk[Grad_dim] * Esize_x[Grad_dim] >=
	      Size_x[Grad_dim] - (X_1D_bc + 0.00000001))
	    {
  ...
  if (L1D_bc)
    {
      ...
      if (Proc == 0 && Iwrite != NO_SCREEN)
	printf
	  (" %d nodes of %d total will be set to the 1D boundary region\n",
	   count, Nnodes);
    }
  else
    {
      ...
      if (Proc == 0 && Iwrite != NO_SCREEN)
	printf (" %d nodes of %d total will be coarsened\n", count_coarse,
		Nnodes);
    }
  ...
  for (inode_box = 0; inode_box < Nnodes_box; inode_box++)
	...
	case OPTION_VARY:
	  if (nodepos[idim] <= -0.5 * Size_x[idim] + X_const_mu + 0.00000001)
	    Area_IC[inode_box] = PI * Pore_rad_L_IC[0] * Pore_rad_L_IC[0];
	  else if (nodepos[idim] >=
		   0.5 * Size_x[idim] - X_const_mu - 0.00000001)
	    Area_IC[inode_box] =
	      PI * Pore_rad_R_IC[Nseg_IC - 1] * Pore_rad_R_IC[Nseg_IC - 1];
	  else
	    {
	      ...
	      for (i = 0; i < Nseg_IC; i++)
		{
		  ...
		  if (nodepos[idim] > xleft - 0.00000001 &&
			...
			(Pore_rad_R_IC[i] - Pore_rad_L_IC[i]) / (xright -
								 xleft);
...
/*  if (Load_Bal_Flag == LB_LINEAR || Ndim != 2) flag = AZ_linear;
  ...
  switch (Az_solver)
    {
    case 1:
      Aztec.options[AZ_solver] = AZ_cg;
      break;
    case 2:
      Aztec.options[AZ_solver] = AZ_tfqmr;
      break;
    case 3:
      Aztec.options[AZ_solver] = AZ_cgs;
      break;
    case 4:
      Aztec.options[AZ_solver] = AZ_bicgstab;
      break;
    default:
      Aztec.options[AZ_solver] = AZ_gmres;
    }
  switch (Az_scaling)
    {
    case 1:
      Aztec.options[AZ_scaling] = AZ_Jacobi;
      break;
    case 2:
      Aztec.options[AZ_scaling] = AZ_sym_row_sum;
      break;
    case -1:
      Aztec.options[AZ_scaling] = AZ_none;
      break;
    default:
      Aztec.options[AZ_scaling] = AZ_row_sum;
    }
  switch (Az_preconditioner)
    {
    case 1:
      Aztec.options[AZ_precond] = AZ_Jacobi;
      break;
    case 2:
      Aztec.options[AZ_precond] = AZ_sym_GS;
      break;
    case 3:
      Aztec.options[AZ_precond] = AZ_ls;
      break;

    case 4:
      Aztec.options[AZ_precond] = AZ_dom_decomp;
      Aztec.options[AZ_subdomain_solve] = AZ_ilut;
      Aztec.params[AZ_ilut_fill] = Az_ilut_fill_param;
      break;

    case -1:
      Aztec.options[AZ_precond] = AZ_none;
      break;
    default:
      Aztec.options[AZ_precond] = AZ_dom_decomp;
  ...
  if (Iwrite == NO_SCREEN)
    Aztec.options[AZ_output] = 0;	/* no output */
  else
    Aztec.options[AZ_output] = 10;	/* lots of output */
  if (Iwrite == NO_SCREEN)
    Aztec.options[AZ_output] = AZ_none;
...
void
MY_read_update (int *N_update, int *update[], int proc_config[],
		int N, int *nodes_x, int chunk, int input_option)
/*******************************************************************************
...
*******************************************************************************/
{
  ...
  if (input_option == AZ_box)
      ...
      if (Ndim == 2)
	{
	  ...
	  if (proc_x == 0)
	    proc_x = 1;
	  if ((nprocs % proc_x != 0) && (nprocs % (proc_x + 1) == 0))
	    proc_x++;
