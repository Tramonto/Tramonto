 ...
 */
{
 ...
 /*************************** BEGIN EXECUTION *******************************/
  ...
  al_eq_soln_contrib =
    ...
    - scaled_dot_prod (con->x_old, con->x_tang, con->scale_vec,
		       con->numOwnedUnks);
  ...
  y = (arc_length_equation -
       scaled_dot_prod (con->x_tang, delta_x, con->scale_vec,
			con->numOwnedUnks)) / (con->dp_ds -
					       scaled_dot_prod (con->x_tang,
								dx2,
								con->
								scale_vec,
								con->
  ...
  if (con->printproc)
    {
      printf ("\n\t\tFraction of arc-step due to soln, param = %g %g\n",
	      al_eq_soln_contrib / con->arc_step,
	      al_eq_param_contrib / con->arc_step);
...
/* Seems to work better as 0 for TP calcs      */
 ...
 /*************************** BEGIN EXECUTION *******************************/
...
#ifdef SCALE_TP
  b_big = 1.0 / ltransnorm (b, con->scale_vec, con->numOwnedUnks);
  /*SCALED*/
    a_big = -ltransnorm (a, con->scale_vec, con->numOwnedUnks) * b_big;
  /*SCALED*/ for (i = 0; i < con->numUnks; i++)
    a[i] += a_big * b[i];
  /*SCALED*/ for (i = 0; i < con->numUnks; i++)
    b[i] *= b_big;
  /*SCALED*/
  ...
  calc_rhs_continuation (TP_CONT_SOL3, c, a, con->scale_vec, x_tmp, dt_p,
			 con->tp_param, r_vec, con->numUnks,
			 con->numOwnedUnks);
...
#ifdef SCALE_TP
  d_big = 1.0 / ltransnorm (d, con->scale_vec, con->numOwnedUnks);
  /*SCALED*/
    c_big = -ltransnorm (c, con->scale_vec, con->numOwnedUnks) * d_big;
  /*SCALED*/ for (i = 0; i < con->numUnks; i++)
    c[i] += c_big * d[i];
  /*SCALED*/ for (i = 0; i < con->numUnks; i++)
    d[i] *= d_big;
...
#endif
    dt_p = ((1.0 -
	     AGS_option * ltransnorm (r_vec, con->scale_vec,
				      con->numOwnedUnks)) - ltransnorm (c,
									con->
									scale_vec,
									con->
									numOwnedUnks))
  ...
  /*SCALED*/
    ...
    {
      vecnorm =
	0.00001 / fabs (ltransnorm (c, con->scale_vec, con->numOwnedUnks));
      if (con->printproc)
  ...
  if (first)
      ...
      if (tmp == 0.0)
	{
	  if (con->printproc)
	    {
	      printf
		("ERROR in pitchfork alg: Asymmetric vector must be supllied\n");
	    }
	  exit (-1);
	}
      for (i = 0; i < con->numUnks; i++)
	psi[i] = con->x_tang[i] / tmp;
      first = FALSE;
...
double
ltransnorm (double *x, double *scale_vec, int n)
{
  int i;
  double sum = 0;
...
void
calc_rhs_continuation (int rhs_type, double *x, double *ab_vec,
   ...
   */
  ...
  else if (rhs_type == TP_CONT_SOL3)
    {
      ...
      dc_p1 = PERTURB * (PERTURB +
			 sqrt (dp (x, x, numOwnedUnks) /
			       (dp (ab_vec, ab_vec, numOwnedUnks) +
				PERTURB)));

      printf ("TP3: x=%g, ab=%g, del=%g\n\n", dp (x, x, numOwnedUnks),
	      dp (ab_vec, ab_vec, numOwnedUnks), dc_p1);
      for (i = 0; i < numUnks; i++)
