...
/***************************************************************************/
void
gather_global_vec (double *loc_vec, int *loc_index, int N_loc,
		   double *global_vec)
/*
 ...
 */
{
  ...
  if (Proc == 0)
    {
      ...
      loc_index_jproc =
	(int *) array_alloc (1, max_proc_num_nodes, sizeof (int));
      vec_jproc =
	(double *) array_alloc (1, max_proc_num_nodes, sizeof (double));

      for (i = 0; i < N_loc; i++)
	global_vec[loc_index[i]] = loc_vec[i];
    }
  ...
  if (Proc == 0)
      ...
      for (from_p = Num_Proc - 1; from_p > 0; from_p--)
	{
	  ...
	  if (md_wrap_write
	      ((void *) &flag, sizeof (int), from_p, flagtype,
	       &st_write) != 0)
	    {
	  ...
	  read_flag =
	    md_wrap_wait ((void *) &N_jproc, sizeof (int), &from_p, &itype,
			  &cflag, &st_wait);
	  ...
	  if (N_jproc * sizeof (double) > max_length)
	    {
	      ...
	      if (n_msgs * max_length / sizeof (double) != N_jproc)
		n_msgs++;
	    }
	  else
	    n_msgs = 1;
	  ...
	  for (imsg = 0; imsg < n_msgs; imsg++)
	    {
	      ...
	      itype = INT_VEC_LENGTH + from_p;
	      read_flag =
		md_wrap_wait ((void *) &msg_length, sizeof (int), &from_p,
			      &itype, &cflag, &st_wait);
	  ...
	  for (i = 0; i < N_jproc; i++)
	    if (fabs (vec_jproc[i]) > 1.0e-10)
	      global_vec[loc_index_jproc[i]] = vec_jproc[i];
  ...
  else
    {
      ...
      read_flag =
	md_wrap_wait ((void *) &flag, sizeof (int), &zero, &flagtype, &cflag,
		      &st_wait);
      printf ("after md_wrap_wait: proc: %d \n", Proc);

      if (md_wrap_write
	  ((void *) &N_loc, sizeof (int), 0, ltype + Proc, &st_write) != 0)
	{
	  (void) fprintf (stderr, "%s%d\n", yo_error, Proc);
      ...
      if (N_loc * sizeof (double) > max_length)
	{
	  ...
	  if (n_msgs * max_length / sizeof (double) != N_loc)
	    n_msgs++;
	}
      else
	n_msgs = 1;
      ...
      for (imsg = 0; imsg < n_msgs; imsg++)
	{
	  ...
	  if (n_msgs == 1)
	    msg_length = N_loc;
	  else if (imsg < n_msgs - 1)
	    msg_length = max_length / sizeof (double);
	  else
	    msg_length = N_loc - (n_msgs - 1) * max_length / sizeof (double);

	  if (md_wrap_write
	      ((void *) &msg_length, sizeof (int), 0, ltype + Proc,
	       &st_write) != 0)
	    {
	      (void) fprintf (stderr, "%s%d\n", yo_error, Proc);
	      (void) fprintf (stderr, "write failed: N_loc\n");
...
/***************************************************************************/
void
gather_global_vec_int (int *loc_vec, int *loc_index, int N_loc,
		       int *global_vec)
/*
 * This routine was created by modifying an MPSalsa routine by the same name.
 *
 ...
 */
{
  ...
  if (Proc == 0)
	  ...
	  if (N_jproc * sizeof (int) > max_length)
	    {
	      ...
	      if (n_msgs * max_length / sizeof (int) != N_jproc)
		n_msgs++;
	    }
	  else
	    n_msgs = 1;
      ...
      if (N_loc * sizeof (int) > max_length)
	{
	  ...
	  if (n_msgs * max_length / sizeof (int) != N_loc)
	    n_msgs++;
	}
      else
	n_msgs = 1;
      ...
      for (imsg = 0; imsg < n_msgs; imsg++)
	{
	  ...
	  if (n_msgs == 1)
	    ...
	    msg_length = max_length / sizeof (int);
	  else
	    msg_length = N_loc - (n_msgs - 1) * max_length / sizeof (int);
...
void
share_global_int_vec (int vec_length, int *int_vec)
{
  int j;
  ...
  if (Proc & nprocs_small)
      ...
      for (imsg = 0; imsg < n_msgs; imsg++)
	{
	  ...
	  if (n_msgs == 1)
	    ...
	    msg_length = size - (n_msgs - 1) * max_length;
	  ...
	  if (Proc < partner)
	      ...
	      for (imsg = 0; imsg < n_msgs; imsg++)
		{
		  ...
		  if (md_write ((char *) &(int_vec[istart]),
		    ...
		    {
		      printf
			("2 WRITE ERROR ... see share_global_int_vec.\n");
		      exit (-1);
		    }
		  result = md_read ((char *) &flag_done_read,
				    sizeof (int), &partner, &type2, &cflag);
  ...
  if (Proc & nprocs_small)
      ...
      for (imsg = 0; imsg < n_msgs; imsg++)
	{
	  ...
	  if (md_read
	      ((char *) &(int_vec[istart]), msg_length, &partner, &type,
	       &cflag) != msg_length)
	    {
...
void
share_global_double_vec (int vec_length, double *double_vec)
{
  int j;
  int nprocs;			/* total number of processors */
  int size;
  ...
  if (Proc & nprocs_small)
      ...
      for (imsg = 0; imsg < n_msgs; imsg++)
	{
      ...
      for (j = 0; j < vec_length; j++)
	{
	  if (double_vec[j] == UNINIT_VEC && tmp[j] != UNINIT_VEC)
	    double_vec[j] = tmp[j];
	}
    }
	  ...
	  if (Proc < partner)
	      ...
	      for (imsg = 0; imsg < n_msgs; imsg++)
		{
		  ...
		  if (md_write ((char *) &(double_vec[istart]),
		    ...
		    {
		      printf
			("2 WRITE ERROR ... see share_global_double_vec.\n");
		      exit (-1);
		    }
		  result = md_read ((char *) &flag_done_read,
				    sizeof (int), &partner, &type2, &cflag);
	      ...
	      for (imsg = 0; imsg < n_msgs; imsg++)
		{
		  ...
		  if (md_read ((char *) &(tmp[istart]),
		    ...
		    {
		      printf
			("2 READ ERROR ... see share_global_double_vec.\n");
		      exit (-1);
		    }
		  result = md_write ((char *) &flag_done_read,
				     sizeof (int), partner, type2, &cflag);
