...
setup_vext_coulomb_vol ()
{
  ...
  reflect_flag[0] = FALSE;
  reflect_flag[1] = FALSE;
  reflect_flag[2] = FALSE;
  ...
  charge_el_vol_global =
    (double *) array_alloc (1, Nelements, sizeof (double));
  charge_el_vol_loc =
		      ...
		      for (i = 0; i < image; i++)
			{
			  ...
			  for (idim = 0; idim < Ndim; idim++)
				...
				image_pos[i][idim] - 0.5 * Esize_x[idim];
			      r_center_sq +=
				(pos[idim] - node_pos_f[idim]) * (pos[idim] -
								  node_pos_f
								  [idim]);
			    }
			  if (r_center_sq < 4.0)
			  ...
			  Vext_coul[loc_inode][icomp] +=
			    integrate_potential (FLAG_COULOMB,
						 Charge_f[icomp],
						 charge_el_vol_global[iel],
						 1., ngp, 1, gp, NULL, gw,
						 NULL, pos, node_pos_f);
			}
		    }
		  if (Type_coul == LIKE_LJ)
		    Vext[loc_inode][icomp] += Vext_coul[loc_inode][icomp];
		}
