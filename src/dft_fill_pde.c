...
/****************************************************************************/
void
set_fem_weights (double **wt_laplace_ptr, double **wt_source_ptr)
/* This routine sets the fem weights for the laplace and source terms */
{
  ...
  if (*wt_laplace_ptr == NULL)
    {
      *wt_laplace_ptr =
	(double *) array_alloc (1, num_nodes, sizeof (double));
  ...
  if (Ndim == 1)
	...
	(*wt_laplace_ptr)[8] = -(dy_x + dx_y) / 6.0;
      ...
      /* left and right */
      (*wt_laplace_ptr)[3] = (*wt_laplace_ptr)[5] =
	(-2.0 * dy_x + dx_y) / 3.0;
      /* center */
	...
	2.0 * (-2 * dyz_x + dxz_y + dxy_z) / 9.0;
      /* +- iy */
      (*wt_laplace_ptr)[10] = (*wt_laplace_ptr)[16] =
	2.0 * (dyz_x - 2.0 * dxz_y + dxy_z) / 9.0;
      /* +- iz */
      (*wt_laplace_ptr)[4] = (*wt_laplace_ptr)[22] =
	2.0 * (dyz_x + dxz_y - 2.0 * dxy_z) / 9.0;
      /* +- ix and +- iy */
	...
	= (*wt_laplace_ptr)[24] = (*wt_laplace_ptr)[26] =
	-(dyz_x + dxz_y + dxy_z) / 36.0;
...
/****************************************************************************/
void
set_fem_1el_weights (double **wt_lp_1el_ptr, double **wt_s_1el_ptr,
		     int ***elem_permute)
/* This routine sets the fem weights for the laplace and source terms */
{
  int iln;
  ...
  if (*wt_lp_1el_ptr == NULL)
    {
      *wt_lp_1el_ptr =
	(double *) array_alloc (1, Nnodes_per_el_V, sizeof (double));
      *wt_s_1el_ptr =
	(double *) array_alloc (1, Nnodes_per_el_V, sizeof (double));
      *elem_permute =
	(int **) array_alloc (2, Nnodes_per_el_V, Nnodes_per_el_V,
			      sizeof (int));
    }
  ...
  if (Ndim == 1)
      ...
      for (iln = 0; iln < 4; iln++)
	{
	  ...
	  (*elem_permute)[iln][1] = (*elem_permute)[iln + 4][1 + 4] =
	    (13 - iln) % 4;
	  (*elem_permute)[iln][2] = (*elem_permute)[iln + 4][2 + 4] =
	    (iln + 6) % 4;
      ...
      for (iln = 4; iln < 8; iln++)
	{
	  ...
	  (*elem_permute)[iln][2] = (*elem_permute)[iln - 4][2 + 4] =
	    (iln + 6) % 4 + 4;
	  (*elem_permute)[iln][3] = (*elem_permute)[iln - 4][3 + 4] =
	    7 - (iln - 4);
	}
    }
}
...
/****************************************************************************/
void
load_polarize_poissons_eqn (int i_box, int inode_box, int loc_i, int *ijk_box,
			    double *mat_row,
 ...
 * handled in the main fill routine. */
  ...
  if (iwall != -1 && Type_bc_elec[WallType[iwall]] == CONST_POTENTIAL)
      ...
      for (iln = 0; iln < Nnodes_per_el_V; iln++)
	{
	  ...
	  elem =
	    node_to_elem (node_box_to_node (inode_box), iln, reflect_flag);
		  ...
		  for (idim = 0; idim < Ndim; idim++)
		    {
		      ...
		      offset[idim] =
			off_ref[((nodes_volm_el +
				  iln) % nodes_volm_el) /
				nodes_surf_el][((nodes_volm_el +
						 jln) % nodes_volm_el) /
					       nodes_surf_el];
		    }
		  ...
		  if (jnode_box >= 0)
		      ...
		      if (fill_flag != MSR_PREPROCESS)
			{
			  ...
			  for (icomp = 0; icomp < Ncomp; icomp++)
			    {
			      if (!Zero_density_TF[jnode_box][icomp])
				in_wall = FALSE;
			    }
			  if (in_wall)
			    {
			      ...
			      resid[loc_i] +=
				(1.0 +
				 pol_wall) * wt_lp_1el[isten] * x[loc_j];
			      mat_row[loc_j] +=
				(1.0 + pol_wall) * wt_lp_1el[isten];
			    }
			  else
			    {
						    ...
						    [el_box]]][icomp]))
			{
			  ...
			  if (Charge_f[icomp] != 0.0)
			      ...
			      if (fill_flag != MSR_PREPROCESS)
				{
				  loc_j = B2L_unknowns[j_box];
				  resid[loc_i] -=
				    wt_s_1el[isten] * KAPPA_H2O *
				    Charge_f[icomp] * x[loc_j] * 4.0 * PI /
				    Temp_elec;
				  mat_row[loc_j] -=
				    wt_s_1el[isten] * KAPPA_H2O *
				    Charge_f[icomp] * 4.0 * PI / Temp_elec;
				}
			      else
				bindx_tmp[j_box] = TRUE;
	      ...
	      for (icomp = 0; icomp < Ncomp; icomp++)
		{
		  if (!Zero_density_TF[inode_box][icomp] && Lpolarize[icomp])
		      ...
		      for (jln = 0; jln < Nnodes_per_el_V; jln++)
			{
			  ...
			  jnode_box =
			    offset_to_node_box (ijk_box, offset, junk2);
		      ...
		      if (!in_wall)
			{
			  if (fill_flag != MSR_PREPROCESS)
			      ...
			      if (Ndim == 1)
				{
				  ...
				  resid[loc_i] +=
				    wt * Pol[icomp] * (psi_0 - psi_1) * tmp;

				  mat_row[loc_j_rho[0]] +=
				    wt * Pol[icomp] * (psi_0 - psi_1);
				  mat_row[loc_j_rho[1]] +=
				    wt * Pol[icomp] * (psi_0 - psi_1);
				  mat_row[loc_j_psi[0]] +=
				    wt * Pol[icomp] * tmp;
...
/****************************************************************************/
void
load_poissons_eqn (int i_box, int inode_box, int loc_i, int *ijk_box,
		   double *mat_row,
	  ...
	  if (elem >= 0)
	      ...
	      for (jln = 0; jln < Nnodes_per_el_V; jln++)
		{
		  ...
		  if (Type_poly == -1)
		    ...
		    j_box = jnode_box * Nunk_per_node + 2 * Ncomp + Ngeqn_tot;
		  /*
		   * add in Laplace term
		   */
		  if (jnode_box >= 0)
		      ...
		      if (fill_flag != MSR_PREPROCESS)
			{
			  loc_j = B2L_unknowns[j_box];
			  resid[loc_i] +=
			    Dielec[el_box] * wt_lp_1el[isten] * x[loc_j];
		  ...
		  for (icomp = 0; icomp < Ncomp; icomp++)
						    ...
						    [el_box]]][icomp]))
			{
			  ...
			  if (Charge_f[icomp] != 0.0)
				...
				j_box =
				  jnode_box * Nunk_per_node + Ncomp + icomp;
			      if (fill_flag != MSR_PREPROCESS)
				{
				  loc_j = B2L_unknowns[j_box];
				  resid[loc_i] -=
				    wt_s_1el[isten] * Charge_f[icomp] *
				    x[loc_j] * 4.0 * PI / Temp_elec;
				  mat_row[loc_j] -=
				    wt_s_1el[isten] * Charge_f[icomp] * 4.0 *
				    PI / Temp_elec;
				}
			      else
				bindx_tmp[j_box] = TRUE;
...
/* load_poisson_bc: Load the Boundary condiditions associated with surfaces
                    of known charge.                                     */
void
load_poisson_bc (double *resid, int inode_box, int loc_inode, int loc_i)
{
  int iwall, idim;
  double charge_i;
  ...
  if (Nodes_2_boundary_wall[Nlists_HW - 1][inode_box] != -1)
      ...
      if (Type_bc_elec[WallType[iwall]] == CONST_CHARGE)
	{
	  ...
	  for (idim = 0; idim < Ndim; idim++)
	    charge_i +=
	      Charge_w_sum_els[loc_inode][idim] * Area_surf_el[idim];
...
void
basis_fn_calc (double **phi, double ***grad_phi, double *evol)
{
  int iln, igp;
  double gp[2];
  ...
  if (Ndim == 3)
    ...
    {
      if (Proc == 0)
	printf ("ERROR basis_fn_calc: Only for 3D\n");
      exit (-1);
    }
}
...
/************************************************************************/
void
load_nonlinear_transport_eqn (int i_box, int inode_box, int loc_i,
			      int *ijk_box, double *mat_row, double *resid,
			      double *x, int *bindx_tmp, int fill_flag,
			      int iunk)
{
  ...
  int j_box_mu[8], j_box_rho[8], loc_j_mu[8], loc_j_rho[8], jnode_box, flag,
    count_flag;
  ...
  if (first)
    {
      ...
      if (Ndim == 3)
	basis_fn_calc (phi, grad_phi, &evol);
      first = FALSE;
    }
  ...
  /* check if you are in well-mixed bulk region */
  ...
  else if (Size_x[Grad_dim] - ijk[Grad_dim] * Esize_x[Grad_dim] <=
	   X_const_mu + 0.0000001)
    {
      if (fill_flag != MSR_PREPROCESS)
	{
      ...
      for (iln = 0; iln < Nnodes_per_el_V; iln++)
	{
	  ...
	  if (elem >= 0 && (Wall_elems[ilist][el_to_el_box (elem)] == -1 ||
			    Lsemiperm[WallType
				      [Wall_elems[ilist]
				       [el_to_el_box (elem)]]][icomp]))
	    {

	      if (Ndim == 1)
		{
		  if (iln == 0)
		    wt = 1.0 / (Esize_x[0] * 6.0);
		  else if (iln == 1)
		    wt = -1.0 / (Esize_x[0] * 6.0);
		...
		{
		  printf
		    ("WARNING : zero density nodes in all elements !!!\n");
	      ...
	      if (flag == FALSE)
		{
		  if (fill_flag != MSR_PREPROCESS)
		      ...
		      if (Ndim == 1)
			{
			  ...
			  mat_row[loc_j_rho[0]] +=
			    wt * (mu_0 - mu_1) * (2.0 * area_0 + area_1);
			  mat_row[loc_j_rho[1]] +=
			    wt * (mu_0 - mu_1) * (area_0 + 2.0 * area_1);
			  mat_row[loc_j_mu[0]] += wt * tmp;
			  mat_row[loc_j_mu[1]] -= wt * tmp;

			  /* add in a bulk convection term */
			  if (iln == 0)
			    tmp = (2.0 * area_0 + area_1) / 6.0;
			  else
			    tmp = (area_0 + 2.0 * area_1) / 6.0;
...
/*if (Proc==4 && inode_box==L2B_node[26]) printf("jln=%d\n",jln);*/
				  ...
				  grad_mu[0] +=
				    grad_phi[jln][igp][0] * x[loc_j_mu[jln]];
				  grad_mu[1] +=
				    grad_phi[jln][igp][1] * x[loc_j_mu[jln]];
				  grad_mu[2] +=
				    grad_phi[jln][igp][2] * x[loc_j_mu[jln]];
				}
			      grad_mu_dot_grad_phi =
				grad_mu[0] * grad_phi[iln][igp][0] +
				grad_mu[1] * grad_phi[iln][igp][1] +
				grad_mu[2] * grad_phi[iln][igp][2];

			      resid[loc_i] +=
				evol * rho * grad_mu_dot_grad_phi;
			      for (jln = 0; jln < 8; jln++)
				{
				  mat_row[loc_j_rho[jln]] +=
				    evol * phi[jln][igp] *
				    grad_mu_dot_grad_phi;
				  mat_row[loc_j_mu[jln]] +=
				    evol * rho * (grad_phi[jln][igp][0] *
						  grad_phi[iln][igp][0] +
						  grad_phi[jln][igp][1] *
						  grad_phi[iln][igp][1] +
						  grad_phi[jln][igp][2] *
...
/****************************************************************************/
void
load_linear_transport_eqn (int i_box, int inode_box, int loc_i, int *ijk_box,
