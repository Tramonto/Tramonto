...
void
thermodynamics (char *output_file1, int print_flag)
{
  int icomp;
  ...
  if (!Sten_Type[POLYMER_CR])
      ...
      if (Ncomp == 1 && Ipot_ff_n == 2 && Iliq_vap != -2)
	{
	  ...
	  if (Proc == 0 && Iwrite != NO_SCREEN)
		...
		{
		  printf
		    ("For kT/epsilon=:%9.6f  the coexistence densities are:\n",
		     1.0 / Eps_ff[0][0]);
      ...
      else
	{
	  ...
	  if (Lsteady_state)
		...
		{
		  Betap =
		    betap_hs + calc_att_properties (Betamu_att, Rho_b_LBB);
		  for (icomp = 0; icomp < Ncomp; icomp++)
	      ...
	      else
		{
		  ...
		  for (icomp = 0; icomp < Ncomp; icomp++)
		    Betamu_LBB[icomp] =
		      betamu_hs[icomp] + Charge_f[icomp] * Elec_pot_LBB;
		}
		...
		{
		  Betap =
		    betap_hs + calc_att_properties (Betamu_att, Rho_b_RTF);
		  for (icomp = 0; icomp < Ncomp; icomp++)
...
double
calc_hs_properties (double *betamu_hs, double *rho)
{
  int icomp;
...
calc_att_properties: In this routine calculate the strict mean field
                     ...
                     chemical potential */
double
calc_att_properties (double *betamu_att, double *rho)
{
  int icomp, jcomp;
  double betap_att;
...
int_stencil_bulk: this routine sums the appropriate stencil to get
                  ...
                  equation. */
double
int_stencil_bulk (int sten_type, int icomp, int jcomp)
{
  int izone, isten;
...
double
deltaC_MSA (double r, int i, int j)
{
...
/*  printf("\t r: %9.6f icomp: %d  jcomp: %d  kappa: %9.6f  B: %9.6f  Sigma_ff: %9.6f\n",
  ...
  if (r == 0.0)
    printf ("trouble with deltaC term .... r=0");
...
double
deltaC_MSA_int (double r, int i, int j)
{
...
/* pot_parameters: calculate the cross terms (sigmaij,epsilonij,cutoffij)
   for this potential */
void
pot_parameters (char *output_file1)
{
  int i, j, iw, jw, printproc = FALSE;
  double cut = 0.0;
  FILE *fp2 = NULL;
  if (Proc == 0 && output_file1 != NULL)
    printproc = TRUE;
  else
    printproc = FALSE;
  if (printproc)
  ...
  if (printproc)
    fprintf (fp2, "****** SUMMARY OF POTENTIAL PARAMETERS ************\n");
	  ...
	  if (printproc)
	    {
	      ...
	      fprintf (fp2,
		       "   Sigma_ff: %9.6f  Cut_ff: %9.6f  Eps_ff: %9.6f\n",
	  ...
	  if (printproc)
	    {
	      ...
	      fprintf (fp2,
		       "   Sigma_ww: %9.6f  Cut_ww: %9.6f  Eps_ww: %9.6f\n",
	  ...
	  if (printproc)
	    {
	      ...
	      fprintf (fp2,
		       "   Sigma_wf: %9.6f  Cut_wf: %9.6f  Eps_wf: %9.6f\n",
  ...
  while (fabs (resid[0]) > 1e-10 || fabs (resid[1]) > 1e-10)
    {
      ...
      Jac[1][0] =
	dmu_drho_hs (&(Rho_coex[0])) + dmu_drho_att (&(Rho_coex[0]));
      Jac[1][1] =
	dmu_drho_hs (&(Rho_coex[1])) + dmu_drho_att (&(Rho_coex[1]));
	...
	{
	  printf
	    ("problems locating coexistence....more than 100 iterations");
	  exit (-1);
	}
    }
...
dp_drho_hs (double *rho)
{
  ...
  dp_drho =
    (1.0 / pi6) * (xsi0 / (rho[0] * y1) - xsi0 * dy1_drho / (y1 * y1) +
		   (3.0 * xsi1 * xsi2 / y2) * (2.0 / rho[0] - dy2_drho / y2) +
		   (3.0 * xsi2 * xsi2 * xsi2 / y3) * (3.0 / (rho[0]) -
						      dy3_drho / y3));
  return (dp_drho);
}
...
dmu_drho_hs (double *rho)
{
  ...
  dmu_drho = -dy1_drho / y1 +
    ...
    * (1.0 / rho[0] - 0.5 * dy2_drho / y2) + (1.0 / rho[0]);
...
void
print_thermo (char *output_file1, double betap_hs, double *betamu_hs)
{
  ...
  if (Ipot_ff_n != IDEAL_GAS)
    {
      ...
      fprintf (fp2,
	       "excess hard sphere chemical potentials  (units of kT)\n\n");
      for (icomp = 0; icomp < Ncomp; ++icomp)
