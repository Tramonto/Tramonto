...
{
  int loc_inode, inode_box, ijk_box[3], loc_i, i;
...
setup_polymer_G (double *x)
{
  ...
  int loc_j, iunk, poln, iseg, ibond;
      ...
      for (poln = 0; poln < Npol_comp; poln++)
	{
	  ...
	  for (iseg = 0; iseg < Nmer[poln]; iseg++)
	      ...
	      for (ibond = 0; ibond < Nbond[poln][iseg]; ibond++)
		{
		  if (loc_inode == 400)
		    printf
		      ("setting up initial guess for polymer number=%d seg number=%d  bond number=%d\n",
		       poln, iseg, ibond);

		  loc_i =
		    Aztec.update_index[iunk + Nunk_per_node * loc_inode];
		  x[loc_i] = 0.;

		  /* TREAT THE END SEGMENTS */
		  if (loc_inode == 400)
		    printf ("Bonds[][][]=%d\n", Bonds[poln][iseg][ibond]);
		  if (Bonds[poln][iseg][ibond] == -1)
			...
			Aztec.update_index[itype_mer +
		      ...
		      x[loc_i] = x[loc_j];
		      if (loc_inode == 400)
			printf ("end segment Boltz term=%g\n", x[loc_j]);
		      if (Type_poly == 2)
			x[loc_i] = 1.0;
		    }
		  else
		      ...
		      if (!Zero_density_TF[inode_box][jtype_mer])
			{
			  ...
			  for (isten = 0; isten < sten->Length; isten++)
				...
				{	/* (-1 in bulk) */
				  if (Lhard_surf)
				    {
				      if (Nodes_2_boundary_wall[jlist]
					  [jnode_box] != -1)
					weight =
					  HW_boundary_weight (jtype_mer,
							      jlist,
							      sten->
							      HW_Weight
							      [isten],
...
/*if (loc_inode==400 && Bonds[poln][iseg][ibond]==1) printf ("weight from stencil=%9.6f  old x[loc_i]=%g\n",weight,x[loc_i]);*/
				  x[loc_i] += weight * x[loc_j];
/*if (loc_inode==400 && Bonds[poln][iseg][ibond]==1) printf ("new x[loc_i]=%g\n",x[loc_i]);*/
				}
			    }

/*if (loc_inode==400) printf("before multiplication with Boltz term x[loc_i]=%g\n",x[loc_i]);*/
			  loc_j = Aztec.update_index[itype_mer + Nunk_per_node * loc_inode];	/*Boltz */
			  x[loc_i] *= x[loc_j];
