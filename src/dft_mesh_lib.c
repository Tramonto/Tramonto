...
int
offset_to_node (int *inode_ijk, int *offset_node, int *reflect_flag)
{
  int i, inode_sten[3];
  ...
  for (i = 0; i < Ndim; i++)
      ...
      if (inode_sten[i] < 0)
	{
	  switch (Type_bc[i][0])
	    {
	    case IN_BULK:
	      if ((Nwall == 0 && Iliq_vap == 3) || Lsteady_state == TRUE)
		return (-3);
	      else
		return (-1);
	    case IN_WALL:
		...
		{
		  if (Type_bc[i][1] == IN_WALL)
		    return (-2);
		  else if (Type_bc[i][1] == IN_BULK)
		    return (-1);
		  else
		    {
		      printf
			("ERROR: You are reflecting beyond the domain\n");
		      exit (-1);
		    }
		}
	    case LAST_NODE:
      ...
      else if (inode_sten[i] >= Nodes_x[i])
	{
	  switch (Type_bc[i][1])
	    ...
	    case REFLECT:
	      ...
	      if (inode_sten[i] < 0)
		{
		  if (Type_bc[i][0] == IN_WALL)
		    return (-2);
		  else if (Type_bc[i][0] == IN_BULK)
		    return (-1);
		  else
		    {
		      printf
...
void
ijk_to_ijk_box (int *ijk, int *ijk_box)
{
  int idim;
...
/* ijk_to_ijk_box_no_bound: converts ijk to ijk_box, with no special attention to
                   periodic boundary conditions */
void
ijk_to_ijk_box_no_bound (int *ijk, int *ijk_box)
{
  int idim;
...
/* ijk_box_to_ijk: converts ijk_box to ijk, with special attention to
                   periodic boundary conditions */
void
ijk_box_to_ijk (int *ijk_box, int *ijk)
{
  int idim;
...
int
offset_to_node_box (int *ijk_box, int *offset, int *reflect_flag)
{
		  ...
		  Max_IJK_box[i] == Max_IJK[i])
		{
		  switch (Type_bc[i][1])
		    ...
		    case REFLECT:
		      ...
		      printf
			("Check domain size and maximum stencil size \n");
		      exit (-1);
		    }
		}
	      break;
      ...
      else if (ijk_sten_box[i] >= Nodes_x_box[i])
	{
	  switch (Type_bc[i][1])
		...
		ijk_sten_box[i] -= Nodes_x[i];
	      break;
	    case LAST_NODE:
	      ijk_sten_box[i] = Nodes_x_box[i] - 1;
	      break;
	    }
	}
    }
  ...
  switch (Ndim)
    ...
    case 1:
      return (*ijk_sten_box);
    case 2:
      return (*ijk_sten_box + ijk_sten_box[1] * (*Nodes_x_box));
    case 3:
      return (*ijk_sten_box + ijk_sten_box[1] * (*Nodes_x_box)
...
element_to_node (int ielement)
{
  ...
  if (Ndim == 1)
    ...
    inode =
      ielement + (ielement / Elements_x[0]) * (Nodes_x[0] - Elements_x[0]);
  else if (Ndim == 3)
    {
...
void
element_to_nodes (int ielement, int *nodes)
{
...
/****************************************************************************/
int
node_to_elem (int inode_all, int local_node, int *reflect_flag)
/*
 ...
 */
{
  ...
  if ((local_node / 2 == (local_node + 1) / 2) == reflect_flag[0])
    (ijk[0])--;

  if (Ndim > 1)
    if ((local_node - 4 * (local_node / 4) < 2) == reflect_flag[1])
      (ijk[1])--;
  ...
  for (idim = 0; idim < Ndim; idim++)
      ...
      if (ijk[idim] == -1)
	{
	  ...
	  else if (Type_bc[idim][0] == REFLECT)
	    ijk[idim] = 0;
	  else if (Type_bc[idim][0] == IN_BULK)
	    iel = -2;
	  else if (Type_bc[idim][0] == IN_WALL)
	    iel = -1;
	  else if (Type_bc[idim][0] == LAST_NODE)
	    ijk[idim] = 0;
	}
      else if (ijk[idim] == Elements_x[idim])
	{
	  if (Type_bc[idim][1] == PERIODIC)
	    ...
	    ijk[idim] = Elements_x[idim] - 1;
	  else if (Type_bc[idim][1] == IN_BULK)
	    iel = -2;
	  else if (Type_bc[idim][1] == IN_WALL)
	    iel = -1;
	  else if (Type_bc[idim][1] == LAST_NODE)
	    ijk[idim] = Elements_x[idim] - 1;
	}
    }
  ...
  if (iel >= 0)
    {
      iel = ijk[0];
      if (Ndim > 1)
	iel += ijk[1] * Elements_x[0];
      if (Ndim == 3)
	iel += ijk[2] * Elements_plane;
    }
...
/****************************************************************************/
int
node_to_elem_return_dim (int inode_all, int local_node, int *reflect_flag,
 ...
 */
{
  ...
  else if ((local_node / 2 == (local_node + 1) / 2)
	   && (reflect_flag[0] == TRUE))
    {
      (ijk[0])--;
    }
  ...
  if (Ndim > 1)
    {
      if ((local_node - 4 * (local_node / 4) >= 2)
	  && (reflect_flag[1] == FALSE))
	{
	  (ijk[1])--;
	}
      else if ((local_node - 4 * (local_node / 4) < 2)
	       && (reflect_flag[1] == TRUE))
	{
	  (ijk[1])--;
	}
...
/****************************************************************************/
int
node_to_elem_v2 (int inode_all, int local_node)
/*
 * node_to_elem gives the element that has node inode_all as
 * its local node local_node
 * local node numbering given by "set_offsets_and_weights"
 ...
 */
{
  ...
  if (local_node / 2 != (local_node + 1) / 2)
    (ijk[0])--;
...
void
node_to_position (int inode, double *NodePos)
{
  int Node_0th_plane;
  ...
  if (Ndim == 3)
    {
      ...
      NodePos[1] =
	-0.5 * Size_x[1] + Esize_x[1] * (Node_0th_plane / Nodes_x[0]);

      NodePos[2] = -0.5 * Size_x[2] + Esize_x[2] * (inode / Nodes_plane);
    }
  else if (Ndim == 2)
    {
      ...
      NodePos[1] = -0.5 * Size_x[1] + Esize_x[1] * (inode / Nodes_x[0]);
    }
  else if (Ndim == 1)
    {
      NodePos[0] = -0.5 * Size_x[0] + Esize_x[0] * (inode);
...
int
position_to_node (double *NodePos)
{
  int inode_calc;
  ...
  if (Ndim == 3)
    {
      inode_calc =
	((NodePos[2] +
	  0.5 * Size_x[2]) / Esize_x[2]) * Nodes_x[0] * Nodes_x[1] +
...
int
map_0th_plane (int i, int Nplane)
{
  int mapping = 0;
  if (Ndim == 2)
    mapping = i;
...
/* node_box_to_ijk_box:  Given a node number, find the i,j,k node numbers, the offsets
                   into each dimension */
void
node_box_to_ijk_box (int node_box, int *ijk_box)
{
  ...
  switch (Ndim)
    {
      /* If 3D, put the plane number in ijk[2] */
    case 3:
      ijk_box[2] = node_box / (Nodes_plane_box);
      node_box -= ijk_box[2] * Nodes_plane_box;

      /* Now in a plane, put the line number in ijk[1] */
    case 2:
      ijk_box[1] = node_box / (*Nodes_x_box);
...
ijk_to_node (int *ijk)
{
  switch (Ndim)
    ...
    case 1:
      return (*ijk);
    case 2:
      return (*ijk + ijk[1] * (*Nodes_x));
    case 3:
      return (*ijk + ijk[1] * (*Nodes_x) + ijk[2] * Nodes_plane);
    }
  return 0;
}
...
int
ijk_box_to_node_box (int *ijk_box)
{
  switch (Ndim)
    {
    case 1:
      return (*ijk_box);
    case 2:
      return (*ijk_box + ijk_box[1] * (*Nodes_x_box));
    case 3:
      return (*ijk_box + ijk_box[1] * (*Nodes_x_box)
...
int
node_box_to_node (int inode_box)
{
  int ijk_box[3], ijk[3];
...
int
node_to_node_box_no_bound (int inode)
{
...
/* element_box_to_node_box:   
        ...
        or the lower left back corner (3D).   */
int
element_box_to_node_box (int iel_box)
{
...
/****************************************************************************/
int
node_box_to_elem_box (int inode_box, int local_node)
/*
...
/****************************************************************************/
int
node_box_to_elem_box_reflect (int inode_box, int local_node,
			      int *reflect_flag)
/*
 * node_box_to_elem_box: gives the element that touches inode_box
 *                       at the local_node corner.  All in box units.
 */
{
  ...
  switch (Ndim)
      ...
      if ((local_node < 4) == reflect_flag[2])
	{
	  ...
	  if (ijk_box[2] == -1)
	    {
	      if (Type_bc[2][0] == REFLECT)
		ijk_box[2] = 0;
	      else if (Type_bc[2][0] == LAST_NODE)
		ijk_box[2] = 0;
	      else if (Type_bc[2][0] == PERIODIC && Pflag[2])
		ijk_box[2] = Elements_x[2] - 1;
	      else
		return (-1);	/* out of domain -- no contribution */
	    }
	}
      else if (ijk_box[2] == Elements_x_box[2])
	{
	  if (Type_bc[2][1] == REFLECT)
	    ijk_box[2] = Elements_x_box[2] - 1;
	  else if (Type_bc[2][1] == LAST_NODE)
	    ijk_box[2] = Elements_x_box[2] - 1;
	  else if (Type_bc[2][1] == PERIODIC && Pflag[2])
	    ijk_box[2] = 0;
	  else
	    return (-1);	/* out of domain -- no contribution */
	}
      ...
      if ((local_node % 4 < 2) == reflect_flag[1])
	{
	  ...
	  if (ijk_box[1] == -1)
	    {
	      if (Type_bc[1][0] == REFLECT)
		ijk_box[1] = 0;
	      else if (Type_bc[1][0] == LAST_NODE)
		ijk_box[1] = 0;
	...
	{
	  if (Type_bc[1][1] == REFLECT)
	    ijk_box[1] = Elements_x_box[1] - 1;
	  else if (Type_bc[1][1] == LAST_NODE)
	    ijk_box[1] = Elements_x_box[1] - 1;
	  else if (Type_bc[1][1] == PERIODIC && Pflag[1])
	    ijk_box[1] = 0;
	  else
	    return (-1);	/* out of domain -- no contribution */
	}
      ...
      if ((local_node % 2 == 0) == reflect_flag[0])
	{
	  ...
	  if (ijk_box[0] == -1)
	    {
	      if (Type_bc[0][0] == REFLECT)
		ijk_box[0] = 0;
	      else if (Type_bc[0][0] == LAST_NODE)
		ijk_box[0] = 0;
	...
	{
	  if (Type_bc[0][1] == REFLECT)
	    ijk_box[0] = Elements_x_box[0] - 1;
	  else if (Type_bc[0][1] == LAST_NODE)
	    ijk_box[0] = Elements_x_box[0] - 1;
	  else if (Type_bc[0][1] == PERIODIC && Pflag[0])
	    ijk_box[0] = 0;
	  else
	    return (-1);	/* out of domain -- no contribution */
	}
