...
int
solve_continuation (double *x, double *x2, int polymer_flag)
 ...
 */
{
  ...
  if (Proc == 0)
    con.general_info.printproc = TRUE;
  else
    con.general_info.printproc = FALSE;
  ...
  if (con.general_info.printproc)
    print_final (con.general_info.param, nstep);
...
/*****************************************************************************/
int
nonlinear_solver_conwrap (double *x, void *con_ptr, int step_num)
...
/*****************************************************************************/
int
linear_solver_conwrap (double *x, int jac_flag, double *tmp)
...
/*****************************************************************************/
int
komplex_linear_solver_conwrap (double *c, double *d,
...
void
mass_matrix_fill_conwrap (double *x, double *rhs)
{
}
void
mass_matvec_mult_conwrap (double *x, double *y)
{
}
void
...
void
shifted_matrix_fill_conwrap (double sigma)
{
}
void
shifted_linear_solver_conwrap (double *x, double *y, int jac_flag, double tol)
{
}
void
...
/*****************************************************************************/
void
matrix_residual_fill_conwrap (double *x, double *rhs, int matflag)
...
/*****************************************************************************/
void
matvec_mult_conwrap (double *x, double *y)
...
scale_elec_param (double ratio)
{
  ...
  if (Surf_charge_flag)
	...
	for (idim = 0; idim < Ndim; idim++)
	  Charge_w_sum_els[loc_inode][idim] *= ratio;
    }
  if (Vol_charge_flag)
    for (iel = 0; iel < Nelements_box; iel++)
      Charge_vol_els[iel] *= ratio;
  for (iwall = 0; iwall < Nwall; iwall++)
    Elec_param_w[iwall] *= ratio;

  if (Iwrite == VERBOSE)
    {
      if (Proc == 0)
	printf ("PRINTING CHARGE DISTRIBUTIONS: Scale_fac=%9.6f\n",
		Scale_fac);
      if (Vol_charge_flag)
	...
	if (Sten_Type[isten])
	  {
		...
		|| isten == POLYMER_CR || (isten == DELTA_FN
...
static void
scale_att_stencil_temp (double temp)
{
...
static void
scale_vext_epswf (double ratio, int icomp, int iwall)
...
/*****************************************************************************/
void
assign_parameter_tramonto (int cont_type, double param)
/* Note: Post_processing assumes the cont_type flags are the same as those
   used in Tramonto's own continuation */
{
  int i, j, icomp, jcomp, iw;
  double ratio, temp_save, scale_save, eps_wf_save[NCOMP_MAX][NWALL_MAX_TYPE];
  ...
  switch (cont_type)
      ...
      if (Mix_type == 0)
	{
	  if (Ipot_ff_n == LJ12_6)
	    {
	      for (icomp = 0; icomp < Ncomp; icomp++)
		Eps_ff[icomp][icomp] /= ratio;
	    }
	  for (i = 0; i < Nwall_type; i++)
	    {
	      if (Ipot_wf_n[i] != VEXT_HARD)
		Eps_w[i] /= ratio;
	    }
	  pot_parameters (NULL);
	}
      else if (Mix_type == 1)
	{
	  for (icomp = 0; icomp < Ncomp; icomp++)
	    {
	      for (jcomp = 0; jcomp < Ncomp; jcomp++)
		Eps_ff[icomp][jcomp] /= ratio;
	      for (i = 0; i < Nwall_type; i++)
		Eps_wf[icomp][i] /= ratio;
	    }
	  for (i = 0; i < Nwall_type; i++)
	    {
    ...
    case CONT_RHO_0:
      Rho_b[2] = param;
      Rho_b[0] = (16. / 18.) * (0.75 - Rho_b[2]);
      Rho_b[1] = (2. / 18.) * (0.75 - Rho_b[2]);
      recalculate_stencils ();
      break;
    case CONT_RHO_ALL:
      for (i = 0; i < Ncomp; i++)
      ...
      if (Mix_type == 0)
	{
	  ...
	  for (i = 0; i < Ncomp; i++)
	      ...
	      for (iw = 0; iw < Nwall; iw++)
		{
		  ...
		  ratio =
		    Eps_wf[i][WallType[iw]] / eps_wf_save[i][WallType[iw]];
...
/*now do a special case where we change two of them at once */
  ...
  thermodynamics ("dft_out.lis", TRUE);
}
...
/*****************************************************************************/
void
assign_parameter_conwrap (double param)
...
/*****************************************************************************/
void
assign_bif_parameter_conwrap (double tp_param)
/* Put the call to a routine to assign the continuation parameter here.
 * Input:
...
{
  if (Proc == 0)
    printf ("\tSecond (floating) parameter #%d set to %g\n",
...
/*****************************************************************************/
static double
get_init_param_value (int cont_type)
{
  int i, j;
  switch (cont_type)
    {
	...
	if (Rho_b[i] != Rho_b[0])
	  {
	    printf
	      ("ERROR: need all Rho_b to be the same for CONT_RHO_ALL\n");
	    exit (-1);
	  }
      return Rho_b[0];
	...
	if (Rho_b[i] != Rho_b[0])
	  {
	    printf
	      ("ERROR: need all Rho_b to be the same for CONT_LOG_RHO_ALL\n");
	    exit (-1);
	  }
      return log (Rho_b[0]);
      ...
      if (Mix_type == 0)
	{
	  for (i = 0; i < Nwall_type; i++)
	    if (Eps_w[i] != Eps_w[0])
	      {
		printf
		  ("ERROR: need all Eps_w to be the same for CONT_EPS_W_ALLL\n");
		exit (-1);
	      }
	  return Eps_w[0];
	}
      else
	{
	  for (i = 0; i < Nwall_type; i++)
	    {
	      for (j = 0; j < Nwall_type; j++)
		if (Eps_ww[i][j] != Eps_ww[0][0])
		  {
		    printf
		      ("ERROR: need all Eps_ww to be the same for CONT_EPS_W_ALLL Mix_type=1\n");
		  }
	    }
	  return Eps_ww[0][0];
	}
    ...
    case CONT_EPSWF00:		/*return Eps_wf[0][0]; */
      return Eps_wf[2][0];

    case CONT_EPSWF_ALL_0:
      for (i = 0; i < Ncomp - 1; i++)
	if (Eps_wf[i][0] != Eps_wf[0][0])
	  {
    ...
    case CONT_EPSFF_00:
      return /*Eps_ff[0][0]; */ Eps_ff[0][2];	/*Eps_ff[2][2]; */
    case CONT_EPSFF_ALL:
      for (i = 0; i < Ncomp; i++)
	if (Eps_ff[i][i] != Eps_ff[0][0])
	  {
	    printf
	      ("ERROR: need all Eps_ff[i][i] to be equal for  (CONT_EPSFF_ALL)\n");
	    exit (-1);
	  }
      return Eps_ff[0][0];
    case CONT_SCALE_EPSFF:
...
/*****************************************************************************/
void
calc_scale_vec_conwrap (double *x, double *scale_vec, int numUnks)
...
/*****************************************************************************/
double
gsum_double_conwrap (double sum)
...
/*****************************************************************************/
void
random_vector_conwrap (double *x, int numOwnedUnks)
...
/*****************************************************************************/
void
perturb_solution_conwrap (double *x, double *x_old,
...
/*****************************************************************************/
void
solution_output_conwrap (int num_soln_flag, double *x, double param,
 ...
 */
{
  ...
  for (i = 0; i < Aztec.N_update; i++)
    x_internal[i] = x[Aztec.update_index[i]];
  ...
  if (num_soln_flag == 2 &&
      con->general_info.method == PHASE_TRANSITION_CONTINUATION)
    {
      ...
      for (i = 0; i < Aztec.N_update; i++)
	x_internal[i] = x2[Aztec.update_index[i]];
...
/*****************************************************************************/
double
free_energy_diff_conwrap (double *x, double *x2)
...
static void
print_con_struct (const struct con_struct *con)
...
/*****************************************************************************/
static void
print_final (double param, int step_num)
/*
