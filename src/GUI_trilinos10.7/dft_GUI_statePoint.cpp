  ...
  int idim;
   ...
   RCP<StringVisualDependency> GradDim_Dep = rcp(
       new StringVisualDependency( 
           Functional_List->getEntryRCP("F0_Type_of_Calculation"),
   ...
   RCP<BoolCondition> TempCon1=rcp(
      new BoolCondition(
          Fluid_List->getEntryRCP("F5_Dimensionless_Energy_Entry"),
          false)
   );

   RCP<BoolCondition> TempCon2=rcp(
      new BoolCondition(
          ChargedFluid_List->getEntryRCP("CF8.0: Dimensionless Entry of Electrostatic Potential(s)?"),
          false)
   );
   ...
   RCP<ConditionVisualDependency> EnergyUnit_Dep = rcp(
      new ConditionVisualDependency(
          Temp_orCon, 
          StatePoint_List->getEntryRCP("0: Temperature(K)"))
   );
   ...
   RCP<BoolVisualDependency> DensityUnit_Dep = rcp( 
      new BoolVisualDependency( 
          StatePoint_List->getEntryRCP("1: Dimensionless Density Entry?"),
          StatePoint_List->getEntryRCP("2: Density Conversion Factor"),
          false)
   );
   ...
   RCP<BoolVisualDependency> DielecConstUnit_Dep = rcp( 
       new BoolVisualDependency( 
           ChargedFluid_List->getEntryRCP("CF2: Entry of Relative Dielectric Constant(s)?"),
           ChargedFluid_List->getEntryRCP("CF3: Reference Dielectric Constant"), 
           false)
   );
   ...
   RCP<StringVisualDependency> DielecPore_Dep = rcp(
       new StringVisualDependency( 
           ChargedFluid_List->getEntryRCP("CF1: Type Dielectric Constant(s)"),
           DielecPoreDependents,
           tuple<std::string>("2-Dielec Consts: fluid and wall regions",
                              "3-Dielec Consts: bulk fluid,fluid near wall, wall regions"))
   );
   ...
   RCP<ConditionVisualDependency> Elec_pot1_Dep = rcp(
       new ConditionVisualDependency(
           Charge_andCon, 
           ChargedFluid_List->getEntryRCP("CF8.2: Elec_pot_1"), 
           true)
   );
