...
void
load_mean_field (int sten_type, int loc_i, int icomp, int izone,
  ...
  for (jcomp = 0; jcomp < Ncomp; jcomp++)
      ...
      for (isten = 0; isten < sten->Length; isten++)
	{
	  ...
	  if (fill_flag != MSR_PREPROCESS)
	      ...
	      if (jnode_box >= 0 && !Zero_density_TF[jnode_box][jcomp])
		{
		  if (Lhard_surf)
			...
			weight = HW_boundary_weight
			  (jcomp, jlist, sten->HW_Weight[isten], jnode_box,
			   reflect_flag);
		    }
		...
		{
		  if (jnode_box == -1)
		    resid[loc_i] += sign * weight * Rho_b[jcomp];
		  else if (jnode_box == -3)
		    {
		      if (Lsteady_state == TRUE)
			resid[loc_i] += sign * weight * Rho_b_LBB[jcomp];
		      else
			resid[loc_i] += sign * weight * Rho_coex[1];
		    }
		  else if (jnode_box == -4)
		    {
		      if (Lsteady_state == TRUE)
