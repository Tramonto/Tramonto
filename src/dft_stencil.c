...
static void merge_load_stencil (struct Stencil_Struct *, int **, double *,
				int *, int *);
...
calc_stencils (void)
{
  ...
  int izone, isten, icomp, i, j, k;	/* counters in for loops         */
  ...
  for (izone = 0; izone < Nzone; izone++)
      ...
      if (Coarser_jac == 5 && izone == Nzone - 1)
	{
	  for (idim = 0; idim < Ndim; idim++)
	    {
	      zone_coarseness =
		(int) (Jac_grid / Esize_x[idim] + 0.000000001);
	...
	if (Sten_Type[isten])
	  {
	    ...
	    if (isten == POLYMER_GAUSS)
	      imax = 1;		/* currently a single Gauss bl must
		...
		for (jcomp = 0; jcomp < jmax; jcomp++)
		  {
		    ...
		    for (i = 0; i < Ndim; i++)
		      {
			el_in_radius[i] =
			  (int) ((float) sten_rad / (float) esize_zone[i] +
				 0.999);
			...
			if (isten == U_ATTRACT || isten == THETA_CHARGE	/*|| Matrix_fill_flag>=3 */
			  ...
			  {
			    nsten_max = 2 * el_in_radius[i];
			  }
			if (isten == DELTA_FN || isten == THETA_FN)
			  {
			    if (el_in_radius[i] * zone_coarseness >
				Sten_length_hs[i])
			      Sten_length_hs[i] =
				el_in_radius[i] * zone_coarseness;
			  }
		    ...
		    sten->Length = 0;
		    sten->Offset =
		      (int **) array_alloc (2, max_sten_nodes, Ndim,
					    sizeof (int));
		    sten->Weight =
		      (double *) array_alloc (1, max_sten_nodes,
					      sizeof (double));
		    if (Lhard_surf)
		      sten->HW_Weight = (double **)
			array_alloc (2, max_sten_nodes, Nnodes_per_el_V,
				     sizeof (double));
		    ...
		    /* Allocate temporary index array to keep track of which 
		       ...
		       for not yet set ! */
		    index_sten =
		      (int *) array_alloc (1, max_sten_nodes, sizeof (int));
		    for (i = 0; i < max_sten_nodes; i++)
		      index_sten[i] = -1;
			...
			for (i = -el_in_radius[0]; i < el_in_radius[0]; i++)
			  {
			    ...
			    in_out_on_flag =
			      calc_in_out_on (x_left, x_right, sten_rad);
					...
					if (radius_sq <= 1.000001)
					  {
					    if (isten != POLYMER_CR)
					      weight =
						get_weight_from_stencil
						(isten, icomp, jcomp,
						 radius_sq, sten_rad, ngpu,
						 gpu, gwu);
					    else
					      {
						radius = fabs (point[0]);
				...
				merge_load_stencil (sten, el_offsets,
						    el_weights, el_in_radius,
						    index_sten);
			      }
			  }
			break;
			...
			for (i = -el_in_radius[0]; i < el_in_radius[0]; i++)
			  {
			    ...
			    el_offsets[2][0] = i;
			    el_offsets[3][0] = i + 1;
			    for (j = -el_in_radius[1]; j < el_in_radius[1];
				 j++)
			      {
			...
			for (i = -el_in_radius[0]; i < el_in_radius[0]; i++)
			  {
				...
				for (k = -el_in_radius[2];
				     k < el_in_radius[2]; k++)
				  {
				    ...
				    x_mid[0] =
				      x_left[0] + 0.5 * esize_zone[0];
				    x_mid[1] =
				      x_left[1] + 0.5 * esize_zone[1];
				    x_mid[2] =
				      x_left[2] + 0.5 * esize_zone[2];
 ...
 */
			...
			if (Proc == 0 && Iwrite == VERBOSE)
			  printf ("Renormalize stencil: %d , i=%d,j=%d\n",
				  isten, icomp, jcomp);
...
/****************************************************************************/
void
shorten_stencil (struct Stencil_Struct *sten)
{
/* eliminate small tail contributions from the
  ...
  (&sten_tmp)->Offset =
    (int **) array_alloc (2, sten->Length, Ndim, sizeof (int));
  (&sten_tmp)->Weight =
    (double *) array_alloc (1, sten->Length, sizeof (double));
  if (Lhard_surf)
    (&sten_tmp)->HW_Weight = (double **)
      array_alloc (2, sten->Length, Nnodes_per_el_V, sizeof (double));

  max_len = 0.0;
  for (i = 0; i < sten->Length; i++)
    {
      if (fabs (sten->Weight[i]) > max_len)
	max_len = fabs (sten->Weight[i]);
    }
  ...
  for (i = 0; i < sten->Length; i++)
    {
      ...
      for (j = 0; j < Ndim; j++)
	(&sten_tmp)->Offset[i][j] = sten->Offset[i][j];
      if (Lhard_surf)
	{
      ...
      if (fabs (sten->Weight[i]) > max_len / Jac_threshold)
	count++;
    }
  ...
  for (i = 0; i < length_tmp; i++)
      ...
      if (fabs ((&sten_tmp)->Weight[i]) > max_len / Jac_threshold)
	{
	  ...
	  if (Lhard_surf)
		...
		sten->HW_Weight[sten->Length][k] =
		  (&sten_tmp)->HW_Weight[i][k];
	    }
	  sten->Length++;
	}
    }
  ...
  if (sten->Length != count)
    {
      ...
      printf ("number of entries: %d  expected number %d", sten->Length,
	      count);
      exit (-1);
    }
  ...
  if (Lhard_surf)
    safe_free ((void *) &(&sten_tmp)->HW_Weight);
...
static void
merge_load_stencil (struct Stencil_Struct *sten,
...
ijk_to_isten_index (int *ijk, int *el_in_radius)
{
  ...
  switch (Ndim)
    ...
    case 1:
      return (*ijk_tmp);
    case 2:
      return (*ijk_tmp + ijk_tmp[1] * (*nodes_x));
    case 3:
      return (*ijk_tmp + ijk_tmp[1] * (*nodes_x) + ijk_tmp[2] * nodes_plane);
    }
  return 0;
}
...
static void
renormalize_stencil (struct Stencil_Struct *sten, double vol_sten)
{
  double sum = 0, ratio;
  int i, j;
...
/*      if (Proc==0) printf("\tRenormalizing stencil from %g to known volume of %g\n",
  ...
  else if (Ndim != 3)
    {
      if (Proc == 0)
	printf
	  ("WARNING: Stencil does not add up near correct volume %g %g\n",
	   sum, vol_sten);
    }
  ...
  if (Proc == 0 && Iwrite == VERBOSE)
    printf
      ("\tProc: %d Renormalizing stencil from %g to known volume of %g\n",
       Proc, sum, vol_sten);
  ratio = vol_sten / sum;
...
static void
print_out_stencil (int isten, int izone, int icomp, int jcomp, FILE * ifp)
{
  int i, j;
  struct Stencil_Struct *sten;
  double sum;
  ...
  for (i = 0; i < sten->Length; i++)
      ...
      if (Ndim <= 3 && (sten->Length < 2500 || sten->Offset[i][0] == 0))
	{
	  ...
	  for (j = 0; j < Ndim; j++)
	    {
	      fprintf (ifp, "  %3d  %7.4f", sten->Offset[i][j],
		       Esize_x[j] * sten->Offset[i][j]);
	    }
	  fprintf (ifp, "  %e  ", sten->Weight[i]);
	}
      sum += sten->Weight[i];
      if (Lhard_surf
	  && (Ndim <= 3 && (sten->Length < 2500 || sten->Offset[i][0] == 0)))
	{
	  for (j = 0; j < Nnodes_per_el_V; j++)
	    {
      ...
      if (Ndim < 3 || Ndim == 3 && sten->Length < 2500
	  || sten->Offset[i][0] == 0)
	fprintf (ifp, "\n");
    }
...
void
sort2_int_double_array (int n, int ra[], double rb[], int *rc[], int ndim)
/*
...
static void
sort_stencil (struct Stencil_Struct *sten)
{
  ...
  for (i = 0; i < sten->Length; i++)
    {
      ...
      if (Ndim > 1)
	value[i] += sten->Offset[i][1] * MAX_NODES_DIM;
      if (Ndim > 2)
	value[i] += sten->Offset[i][2] * MAX_NODES_DIM * MAX_NODES_DIM;
    }
...
/****************************************************************************/
static double
calc_sten_rad (int isten, int icomp, int jcomp)
{
  switch (isten)
    {
    case DELTA_FN:
...
/****************************************************************************/
static double
calc_sten_vol (int isten, int i, int j)
{
  ...
  switch (isten)
    ...
    case U_ATTRACT:
      ...
      vol_sten =
	(4.0 / 3.0) * PI * pow (r_min, 3.0) * uLJatt_n_noshift (r_min, i,
								j) -
	(4.0 / 3.0) * PI * pow (r_cut, 3.0) * uLJatt_n_noshift (r_cut, i,
								j) +
...
/****************************************************************************/
void
set_gauss_quad (int ngp, double *gp, double *gw)
{

  if (ngp == 40)
    {
      gp[0] = (0.038772417506050821933 + 1.0) / 2.0;
      gw[0] = 0.077505947978424811264 / 2.0;
      gp[1] = (0.116084070675255208483 + 1.0) / 2.0;
      gw[1] = 0.077039818164247965588 / 2.0;
      gp[2] = (0.192697580701371099716 + 1.0) / 2.0;
      gw[2] = 0.076110361900626242372 / 2.0;
      gp[3] = (0.268152185007253681141 + 1.0) / 2.0;
      gw[3] = 0.074723169057968264200 / 2.0;
      gp[4] = (0.341994090825758473007 + 1.0) / 2.0;
      gw[4] = 0.072886582395804059061 / 2.0;
      gp[5] = (0.413779204371605001525 + 1.0) / 2.0;
      gw[5] = 0.070611647391286779695 / 2.0;
      gp[6] = (0.483075801686178712909 + 1.0) / 2.0;
      gw[6] = 0.067912045815233903826 / 2.0;
      gp[7] = (0.549467125095128202076 + 1.0) / 2.0;
      gw[7] = 0.064804013456601038075 / 2.0;
      gp[8] = (0.612553889667980237953 + 1.0) / 2.0;
      gw[8] = 0.061306242492928939167 / 2.0;
      gp[9] = (0.671956684614179548379 + 1.0) / 2.0;
      gw[9] = 0.057439769099391551367 / 2.0;
      gp[10] = (0.727318255189927103281 + 1.0) / 2.0;
      gw[10] = 0.053227846983936824355 / 2.0;
      gp[11] = (0.778305651426519387695 + 1.0) / 2.0;
      gw[11] = 0.048695807635072232061 / 2.0;
      gp[12] = (0.824612230833311663196 + 1.0) / 2.0;
      gw[12] = 0.043870908185673271992 / 2.0;
      gp[13] = (0.865959503212259503821 + 1.0) / 2.0;
      gw[13] = 0.038782167974472017640 / 2.0;
      gp[14] = (0.902098806968874296728 + 1.0) / 2.0;
      gw[14] = 0.033460195282547847393 / 2.0;
      gp[15] = (0.932812808278676533361 + 1.0) / 2.0;
      gw[15] = 0.027937006980023401098 / 2.0;
      gp[16] = (0.957916819213791655805 + 1.0) / 2.0;
      gw[16] = 0.022245849194166957262 / 2.0;
      gp[17] = (0.977259949983774262663 + 1.0) / 2.0;
      gw[17] = 0.016421058381907888713 / 2.0;
      gp[18] = (0.990726238699457006453 + 1.0) / 2.0;
      gw[18] = 0.010498284531152813615 / 2.0;
      gp[19] = (0.998237709710559200350 + 1.0) / 2.0;
      gw[19] = 0.004521277098533191258 / 2.0;
      gp[20] = (-0.038772417506050821933 + 1.0) / 2.0;
      gw[20] = gw[0];
      gp[21] = (-0.116084070675255208483 + 1.0) / 2.0;
      gw[21] = gw[1];
      gp[22] = (-0.192697580701371099716 + 1.0) / 2.0;
      gw[22] = gw[2];
      gp[23] = (-0.268152185007253681141 + 1.0) / 2.0;
      gw[23] = gw[3];
      gp[24] = (-0.341994090825758473007 + 1.0) / 2.0;
      gw[24] = gw[4];
      gp[25] = (-0.413779204371605001525 + 1.0) / 2.0;
      gw[25] = gw[5];
      gp[26] = (-0.483075801686178712909 + 1.0) / 2.0;
      gw[26] = gw[6];
      gp[27] = (-0.549467125095128202076 + 1.0) / 2.0;
      gw[27] = gw[7];
      gp[28] = (-0.612553889667980237953 + 1.0) / 2.0;
      gw[28] = gw[8];
      gp[29] = (-0.671956684614179548379 + 1.0) / 2.0;
      gw[29] = gw[9];
      gp[30] = (-0.727318255189927103281 + 1.0) / 2.0;
      gw[30] = gw[10];
      gp[31] = (-0.778305651426519387695 + 1.0) / 2.0;
      gw[31] = gw[11];
      gp[32] = (-0.824612230833311663196 + 1.0) / 2.0;
      gw[32] = gw[12];
      gp[33] = (-0.865959503212259503821 + 1.0) / 2.0;
      gw[33] = gw[13];
      gp[34] = (-0.902098806968874296728 + 1.0) / 2.0;
      gw[34] = gw[14];
      gp[35] = (-0.932812808278676533361 + 1.0) / 2.0;
      gw[35] = gw[15];
      gp[36] = (-0.957916819213791655805 + 1.0) / 2.0;
      gw[36] = gw[16];
      gp[37] = (-0.977259949983774262663 + 1.0) / 2.0;
      gw[37] = gw[17];
      gp[38] = (-0.990726238699457006453 + 1.0) / 2.0;
      gw[38] = gw[18];
      gp[39] = (-0.998237709710559200350 + 1.0) / 2.0;
      gw[39] = gw[19];
    }
  else if (ngp == 20)
    {
      gp[0] = (0.076526521133497333755 + 1.0) / 2.0;
      gw[0] = 0.152753387130725850698 / 2.0;
      gp[1] = (0.227785851141645078080 + 1.0) / 2.0;
      gw[1] = 0.149172986472603746788 / 2.0;
      gp[2] = (0.373706088715419560673 + 1.0) / 2.0;
      gw[2] = 0.142096109318382051329 / 2.0;
      gp[3] = (0.510867001950827098004 + 1.0) / 2.0;
      gw[3] = 0.131688638449176626898 / 2.0;
      gp[4] = (0.636053680726515025453 + 1.0) / 2.0;
      gw[4] = 0.118194531961518417312 / 2.0;
      gp[5] = (0.746331906460150792614 + 1.0) / 2.0;
      gw[5] = 0.101930119817240435037 / 2.0;
      gp[6] = (0.839116971822218823395 + 1.0) / 2.0;
      gw[6] = 0.083276741576704748725 / 2.0;
      gp[7] = (0.912234428251325905868 + 1.0) / 2.0;
      gw[7] = 0.062672048334109063570 / 2.0;
      gp[8] = (0.963971927277913791268 + 1.0) / 2.0;
      gw[8] = 0.040601429800386941331 / 2.0;
      gp[9] = (0.993128599185094924786 + 1.0) / 2.0;
      gw[9] = 0.017614007139152118312 / 2.0;
      gp[10] = (-0.076526521133497333755 + 1.0) / 2.0;
      gw[10] = gw[0];
      gp[11] = (-0.227785851141645078080 + 1.0) / 2.0;
      gw[11] = gw[1];
      gp[12] = (-0.373706088715419560673 + 1.0) / 2.0;
      gw[12] = gw[2];
      gp[13] = (-0.510867001950827098004 + 1.0) / 2.0;
      gw[13] = gw[3];
      gp[14] = (-0.636053680726515025453 + 1.0) / 2.0;
      gw[14] = gw[4];
      gp[15] = (-0.746331906460150792614 + 1.0) / 2.0;
      gw[15] = gw[5];
      gp[16] = (-0.839116971822218823395 + 1.0) / 2.0;
      gw[16] = gw[6];
      gp[17] = (-0.912234428251325905868 + 1.0) / 2.0;
      gw[17] = gw[7];
      gp[18] = (-0.963971927277913791268 + 1.0) / 2.0;
      gw[18] = gw[8];
      gp[19] = (-0.993128599185094924786 + 1.0) / 2.0;
      gw[19] = gw[9];
    }

  else if (ngp == 12)
    {
      gp[0] = (0.125233408511469 + 1.0) / 2.0;
      gw[0] = 0.249147045813403 / 2.0;
      gp[1] = (0.367831498998180 + 1.0) / 2.0;
      gw[1] = 0.233492536538355 / 2.0;
      gp[2] = (0.587317954286617 + 1.0) / 2.0;
      gw[2] = 0.203167426723066 / 2.0;
      gp[3] = (0.769902674194305 + 1.0) / 2.0;
      gw[3] = 0.160078328543346 / 2.0;
      gp[4] = (0.904117256370475 + 1.0) / 2.0;
      gw[4] = 0.106939325995318 / 2.0;
      gp[5] = (0.981560634246719 + 1.0) / 2.0;
      gw[5] = 0.047175336386512 / 2.0;
      gp[6] = (-0.125233408511469 + 1.0) / 2.0;
      gw[6] = 0.249147045813403 / 2.0;
      gp[7] = (-0.367831498998180 + 1.0) / 2.0;
      gw[7] = 0.233492536538355 / 2.0;
      gp[8] = (-0.587317954286617 + 1.0) / 2.0;
      gw[8] = 0.203167426723066 / 2.0;
      gp[9] = (-0.769902674194305 + 1.0) / 2.0;
      gw[9] = 0.160078328543346 / 2.0;
      gp[10] = (-0.904117256370475 + 1.0) / 2.0;
      gw[10] = 0.106939325995318 / 2.0;
      gp[11] = (-0.981560634246719 + 1.0) / 2.0;
      gw[11] = 0.047175336386512 / 2.0;
    }
  else if (ngp == 3)
    {
      gp[0] = (0.7745966692 + 1.0) / 2.0;
      gw[0] = 0.555555555555556 / 2.0;
      gp[1] = (0.0000000 + 1.0) / 2.0;
      gw[1] = 0.888888888888888 / 2.0;
      gp[2] = (-0.7745966692 + 1.0) / 2.0;
      gw[2] = 0.555555555555556 / 2.0;
    }
  else if (ngp == 1)
    {
      gp[0] = (0.0000000 + 1.0) / 2.0;
      gw[0] = 2.000000000000000 / 2.0;
    }
  else if (ngp == 5)
    {
      gp[0] = (0.906179845936884 + 1.0) / 2.0;
      gw[0] = 0.236926885056189 / 2.0;
      gp[1] = (0.538469310105683 + 1.0) / 2.0;
      gw[1] = 0.478628670499366 / 2.0;
      gp[2] = (0.00000 + 1.0) / 2.0;
      gw[2] = 0.568888888888890 / 2.0;
      gp[3] = (-0.538469310105683 + 1.0) / 2.0;
      gw[3] = 0.478628670499366 / 2.0;
      gp[4] = (-0.906179845936884 + 1.0) / 2.0;
      gw[4] = 0.236926885056189 / 2.0;
    }
  else if (ngp == 6)
    {
      gp[0] = (0.238619186083197 + 1.0) / 2.0;
      gw[0] = 0.467913934572691 / 2.0;
      gp[1] = (0.661209386466265 + 1.0) / 2.0;
      gw[1] = 0.360761573048139 / 2.0;
      gp[2] = (0.932469514203152 + 1.0) / 2.0;
      gw[2] = 0.171324492379170 / 2.0;
      gp[3] = (-0.238619186083197 + 1.0) / 2.0;
      gw[3] = 0.467913934572691 / 2.0;
      gp[4] = (-0.661209386466265 + 1.0) / 2.0;
      gw[4] = 0.360761573048139 / 2.0;
      gp[5] = (-0.932469514203152 + 1.0) / 2.0;
      gw[5] = 0.171324492379170 / 2.0;
    }
  else if (ngp == 9)
    {
      gp[0] = (0.324253423403809 + 1.0) / 2.0;
      gw[0] = 0.312347077040004 / 2.0;
      gp[1] = (0.613371432700590 + 1.0) / 2.0;
      gw[1] = 0.260610696402935 / 2.0;
      gp[2] = (0.836031107326636 + 1.0) / 2.0;
      gw[2] = 0.180648160694857 / 2.0;
      gp[3] = (0.968160239507626 + 1.0) / 2.0;
      gw[3] = 0.081274388361574 / 2.0;
      gp[4] = (0.000000000000000 + 1.0) / 2.0;
      gw[4] = 0.330239355001260 / 2.0;
      gp[5] = (-0.324253423403809 + 1.0) / 2.0;
      gw[5] = 0.312347077040004 / 2.0;
      gp[6] = (-0.613371432700590 + 1.0) / 2.0;
      gw[6] = 0.260610696402935 / 2.0;
      gp[7] = (-0.836031107326636 + 1.0) / 2.0;
      gw[7] = 0.180648160694857 / 2.0;
      gp[8] = (-0.968160239507626 + 1.0) / 2.0;
      gw[8] = 0.081274388361574 / 2.0;
    }
  else
    {
...
static double
get_weight_from_stencil (int isten, int icomp, int jcomp, double rsq,
			 double R, int ngpu, double *gpu, double *gwu)
{
  double temp, zmax, z, rho, rmin, rlast_nz, r_upp, slope_dr, r_low, zsq,
    rx_low;
  int i, irmin;
  double sigsq;
  ...
  switch (isten)
      ...
      else
	{
	  ...
	  if (Ndim == 1)
	    ...
	    return (0.5 / (sqrt (1.0 - rsq) * PI * sigsq));
	  else
	    return (0.25 / (PI * sigsq));
	}

    case THETA_FN:
      if (Ndim == 1)
	return ((1.0 - rsq) * PI * R * R);
      else if (Ndim == 2)
	return (sqrt (1.0 - rsq) * 2.0 * R);
      else
	return (1.0);
      ...
      if (Ndim < 3)
	{
	  ...
	  if (irmin < Last_nz_cr)
		...
		r_upp = R;
	      ...
	      rx_low = 0.;
	      temp =
		int_cr (rmin, r_upp, slope_dr, icomp, jcomp, irmin, zsq,
			&rx_low);
		  ...
		  if (r_upp < R)
			...
			r_upp = R;
		      slope_dr = (Rism_cr[icomp][jcomp][irmin + 1] -
				  Rism_cr[icomp][jcomp][irmin]);
		      temp +=
			int_cr (r_low, r_upp, slope_dr, icomp, jcomp, irmin,
				zsq, &rx_low);
		    }
		}
	    }
	  if (rlast_nz < R)
	      ...
	      else
		{
	      ...
	      temp +=
		int_cr (r_low, r_upp, slope_dr, icomp, jcomp, Last_nz_cr, zsq,
			&rx_low);
	    }
	  return (temp);
	}
...
double
int_cr (double r_low, double r_upp, double slope_dr, int icomp, int jcomp,
...
*/
  ...
  if (Ndim == 1)
  ...
  else
    {
      ...
      temp =
	2. * (rx_upp - *rx_low) * (Rism_cr[icomp][jcomp][irmin] -
				   slope_dr * irmin) +
	slope_dr * ((r_upp * rx_upp - r_low * *rx_low) +
		    zsq * log ((r_upp + rx_upp) / (r_low + *rx_low))) /
	Deltar_cr;
      *rx_low = rx_upp;
    }
  return temp;
...
/****************************************************************************/
static int
calc_in_out_on (double *x_l, double *x_r, double sten_rad)
{
