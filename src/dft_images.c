 ...
 */
void
find_images (int idim, double cut,
  ...
  for (iside = 0; iside < 2; iside++)
	...
	sign = 1.0;
	  ...
	  (Type_bc[idim][iside] == IN_WALL &&
	   (fabs
	    (node_image[idim] -
	     sign * (0.5 * Size_x[idim] - 0.5 * Esize_x[idim])) < 0.0001)))
	{
	  ...
	  while (!done)
	    {
	      for (jdim = 0; jdim < Ndim; jdim++)
		node_image2[jdim] = node_image[jdim];
 ...
 */
void
find_images_1D (int idim, double cut,
		int *image, double **image_pos,
		double *node_image, double *node_ref)
{
  int iside, done, i, jdim;
 ...
 */
void
find_images_coulomb (int idim, int *image, double **image_pos,
		     double *node_image)
{
  int iside, done, i, jdim;
...
/***********************************************************************/
  ...
  */
void
find_images2 (int idim, double cut,
	      int *image, double **image_pos,
	      double *node_image, int iwall, int iside)
{
  int done, i, jdim;
