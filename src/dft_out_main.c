...
void
post_process (double *x_internal, char *output_file3, int *niters,
	      double *time_save, int loop1, int binodal_flag)
{
  ...
  char *yo = "post_process", *output_file4 = "dft_dens.dat", *output_file5 =
    NULL, *output_file6 = "dft_gofr.dat", *output_flux =
    "dft_flux.dat", *output_file7 = "dft_dens_site.dat", *output_file8 = NULL;
  char filename[20];
...
/*     output_file7 = "dft_dens2_site.dat";*/
  ...
  for (i = 0; i < Aztec.N_update; i++)
    x[Aztec.update_index[i]] = x_internal[i];
...
/*           if (Type_poly !=-1) print_site_densities(output_file7);*/
  ...
  if (Lprint_gofr && Nlink == 1)
    print_gofr (output_file6);
  ...
  if (Proc == 0)
    {
      ...
      if (Loca.method != -1)
	print_cont_variable (Loca.cont_type1, fp);
      if (Loca.method == 3 || Loca.method == 4)
	print_cont_variable (Loca.cont_type2, fp);
      if (Nruns > 1)
	print_cont_variable (0, fp);
    }
  ...
  if (Proc == 0)
	...
	fprintf (fp, "%d  %9.4f  ", *niters, *time_save);
    }
  ...
  for (idim = 0; idim < Ndim; idim++)
    {
      if (!(Type_bc[idim][0] == REFLECT && Type_bc[idim][1] == REFLECT)
	  && Lcount_reflect)
	{
	  ...
	  if (Type_bc[idim][0] == REFLECT)
	    {
	      ...
	      fac_vol *= 2.0;
	      if (WallPos[idim][0] == -0.5 * Size_x[idim])
		fac_area *= 2.0;
	    }
...
/*   if (Ipot_wf_n != LJ12_6_WALL &&  
  ...
  if (!Sten_Type[POLYMER_CR])
    (void) calc_free_energy (fp, x, fac_area, fac_vol, TRUE);

  if (Ipot_ff_c > 0 || Type_coul == LIKE_LJ)
    calc_surface_charge (fp, x, fac_area, fac_vol);
  (void) calc_adsorption (fp, x, fac_area, fac_vol);

  if (Sten_Type[POLYMER_CR])
    calc_free_energy_polymer (fp, x, fac_area, fac_vol);

  if (Lsteady_state && Proc == 0)
...
/*   if (Iwrite) print_time_histogram(Hist_time,niters);*/
  ...
  if (Proc == 0 && Iwrite != NO_SCREEN)
    printf ("post processing took %g secs\n", MPI_Wtime () - t1);
}
	  ...
	  for (i = 0; i < Imax; i++)
	      ...
	      for (iel = 0; iel < Nnodes_per_el_V; iel++)
		{
		  ...
		  if (ielement == -1)
			...
			if (iwall >= 0 && Lsemiperm[WallType[iwall]][jcomp])
			  semiperm = TRUE;
		      if (iwall != -1 && !semiperm)
			nel_hit--;
		    }
	      ...
	      for (idim = 0; idim < Ndim; idim++)
		{
		  if ((Type_bc[idim][0] == REFLECT
		       ...
		       || Type_bc[idim][0] == LAST_NODE) && ijk[idim] == 0)
		    {
		      nel_hit /= 2;
		      nel_hit2 /= 2;
		    }
...
void
print_cont_variable (int cont_type, FILE * fp)
{
  ...
  switch (cont_type)
						      ...
						      Nwall == 2))
	{
	  ...
	  if (Nwall == 1)
	    ...
	    {
	      fprintf (fp, "%11.8f   ",
		       (fabs (WallPos[idim][1] - WallPos[idim][0]) -
			2.0 * WallParam[iwall_type]));
	    }
	}
      else if (Nwall > 0)
	{
    ...
    case CONT_TEMP:
      if (Ipot_ff_c == 0)
	fprintf (fp, "%10.7f   ", Temp);
      else
	fprintf (fp, "%7.4f   ", Temp_elec);
      break;
      ...
      for (i = 0; i < Ncomp; i++)
	{
      ...
      for (i = 0; i < Ncomp; i++)
	fprintf (fp, "%9.6f  ", Rho_b[i] / rhosum);
    ...
    case CONT_EPSW_ALL:
      if (Mix_type == 0)
	fprintf (fp, "%11.8f   ", Eps_w[0]);
      else
	fprintf (fp, "%11.8f   ", Eps_ww[0][0]);
...
/*print_cont_type: Here print the type of the variable that
                     is changing in a given run */
void
print_cont_type (int cont_type, FILE * fp)
{
  ...
  switch (cont_type)
	...
	fprintf (fp, "WALL COORDS:  ");
      else
	fprintf (fp, "DOMAIN SIZE:   ");
	...
	fprintf (fp, "TEMP_ELEC  ");
      break;

    case CONT_SCALE_RHO:
      fprintf (fp, "Scale_fac_rho:  ");
      break;
    case CONT_RHO_0:
    case CONT_RHO_ALL:
    case CONT_LOG_RHO_0:
...
/*         fprintf(fp,"Eps_wf[0][0]:  "); break;*/
      fprintf (fp, "Eps_wf[2][0]:  ");
      break;

    case CONT_SCALE_EPSFF:
      fprintf (fp, "Scale_fac_epsff:  ");
      break;
    case CONT_EPSFF_00:
    case CONT_EPSFF_ALL:
      fprintf (fp, "Eps_ff[0][0]:  ");
      break;

    case CONT_SCALE_CHG:
      fprintf (fp, "Scale_fac_chg:  ");
      break;
